...
{
	char n[MAX_INPUT_LENGTH] = { 0 };
	strcpy(n, namelist);
	char *tok = strtok(n, " ");
	while (tok != NULL) {
		if (starts_with(str, tok)) {
			return 1;
		}
		tok = strtok(NULL, " ");
	}
		   ...
		   bool add)
{
	...
	default:
		sprintf(log_buf,
			"##Unknown apply adjust attempt [%d] to %s (handler.c, affect_modify).",
			loc, GET_NAME(ch));
		log_hd(log_buf);
		break;
...
void char_from_room(struct char_data *ch)
{
	...
	if (ch->in_room == NOWHERE) {
		sprintf(log_buf,
			"##NOWHERE extracting char from room (handler.c,char_from_room)[%s]",
			GET_NAME(ch));
		log_hd(log_buf);
		SUICIDE;
	}
	...
	if (ch->equipment[WEAR_LIGHT])
		if (ch->equipment[WEAR_LIGHT]->obj_flags.type_flag ==
		    ITEM_LIGHT)
			if (ch->equipment[WEAR_LIGHT]->obj_flags.value[2])	/* Light is ON */
				world[ch->in_room]->light--;
	if (world[ch->in_room] && world[ch->in_room]->people && ch == world[ch->in_room]->people)	/* head of list */
	...
	if (ch->specials.stpMount && !MOUNTMOVE
	    && world[ch->specials.was_in_room_pk]->zone != 180) {
...
void char_to_room(struct char_data *ch, int room)
{
	...
	if (!world[room]) {
		send_to_char
		    ("ROOM doesn't exist, please tell Vryce exactly what you just did.\n\r",
		     ch);
		room = 1;
	}
	if (IS_SET(world[room]->room_flags, BLOCKING)) {
		...
		GET_HIT(ch) -=
		    (dice
		     (stpHoloRTemplates[world[room]->holoroom]->hurt_num_die,
		      stpHoloRTemplates[world[room]->holoroom]->hurt_size_die) +
		     stpHoloRTemplates[world[room]->holoroom]->hurt_base);
		if (GET_HIT(ch) < 10)
			GET_HIT(ch) = 10;
		char_to_room(ch, ch->specials.was_in_room_pk);
		return;
			...
			if (!zone_table[world[room]->zone].populated) {
				...
				for (i = descriptor_list; i; i = i->next)
					if (i->character && !i->connected &&
						...
						if (IS_SET
						    (i->character->specials.
						     god_display,
						     GOD_ZONERESET)) {
							sprintf(log_buf,
								"(ZONE) %s, Has been entered.\n\r",
								zone_table[world
									   [room]->
									   zone].
								name);
	...
	if ((IS_SET(world[ch->in_room]->room_flags, NEUTRAL) ||
	    ...
	    (!IS_SET(world[ch->specials.was_in_room_pk]->room_flags, NEUTRAL) &&
	     !IS_SET(world[ch->specials.was_in_room_pk]->room_flags,
		     CHAOTIC))) {
		global_color = 31;
		if (IS_SET(world[ch->in_room]->room_flags, NEUTRAL))
			send_to_char
	...
	if (world[room]->zone == 197 && ch->desc) {
		if (Holo[world[room]->holox - 1][world[room]->holoy] < 256) {
			iMakeHoloRoom(world[room]->holox - 1,
				      world[room]->holoy);
		}
		if (Holo[world[room]->holox + 1][world[room]->holoy] < 256) {
			iMakeHoloRoom(world[room]->holox + 1,
		...
		if (Holo[world[room]->holox][world[room]->holoy - 1] < 256) {
			iMakeHoloRoom(world[room]->holox,
				      world[room]->holoy - 1);
		}
		if (Holo[world[room]->holox][world[room]->holoy + 1] < 256) {
			iMakeHoloRoom(world[room]->holox,
	...
	if (ch->p->stpFreight) {
		if (world[room]->dir_option[NORTH]
		    && world[room]->dir_option[NORTH]->to_room ==
		    ch->p->stpFreight->iLocationRoom)
			FreightAutoFollow(ch, room);
		else if (world[room]->dir_option[SOUTH]
			 && world[room]->dir_option[SOUTH]->to_room ==
			 ch->p->stpFreight->iLocationRoom)
			FreightAutoFollow(ch, room);
		else if (world[room]->dir_option[EAST]
			 && world[room]->dir_option[EAST]->to_room ==
			 ch->p->stpFreight->iLocationRoom)
			FreightAutoFollow(ch, room);
		else if (world[room]->dir_option[WEST]
			 && world[room]->dir_option[WEST]->to_room ==
			 ch->p->stpFreight->iLocationRoom)
			FreightAutoFollow(ch, room);
		else if (world[room]->dir_option[UP]
			 && world[room]->dir_option[UP]->to_room ==
			 ch->p->stpFreight->iLocationRoom)
			FreightAutoFollow(ch, room);
		else if (world[room]->dir_option[DOWN]
			 && world[room]->dir_option[DOWN]->to_room ==
			 ch->p->stpFreight->iLocationRoom)
			FreightAutoFollow(ch, room);
	}
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos)
{
	...
	if (pos < 0 || pos > MAX_WEAR) {
		sprintf(log_buf, "##!(ch->equipment[%d]) on %s(obj-%s)", pos,
...
struct obj_data *unequip_char(struct char_data *ch, int pos)
{
	...
	if (!ch->equipment[pos]) {
		sprintf(log_buf, "## ASSERT unequip_char pos=[%d] ch=[%s]", pos,
			GET_NAME(ch));
		log_hd(log_buf);
		if (!ch->equipment[pos])
			SUICIDE;
...
struct char_data *get_char_room(char *name, int room)
{
	...
	for (i = world[room]->people, j = 1; i && (j <= number);
	     i = i->next_in_room)
...
void obj_to_room(struct obj_data *object, int room)
{
	...
	if ((room < 0) || (room >= MAX_ROOM)) {
		sprintf(log_buf,
			"Room [%d] out of bounds in obj_to_room.  object(%s)",
			room, object->name);
		log_hd(log_buf);
		SUICIDE;
	}
...
void obj_from_container(struct obj_data *obj)
{
	...
	if (this_object->contains == obj) {
		...
		if (this_object) {
			...
			sprintf(log_buf, "## Object #:  %d not in container",
				obj->item_number);
			log_hd(log_buf);
			SUICIDE;
		}
	...
	if (ch->equipment[WEAR_LIGHT])
		if (ch->equipment[WEAR_LIGHT]->obj_flags.type_flag ==
			...
			if (ch->equipment[WEAR_LIGHT]->obj_flags.value[2] > 0) {
				(ch->equipment[WEAR_LIGHT]->obj_flags.
				 value[2])--;
				if (ch->equipment[WEAR_LIGHT]->obj_flags.
				    value[2] == 0) {
					act("You're light source flickers out.",
...
void extract_char(struct char_data *ch, bool pull)
{
	...
	if (pull) {
		if (ch == character_list)
			...
			for (k = character_list; k && k->next != ch;
			     k = k->next) ;
			if (k)
				k->next = ch->next;
			else {
...
struct obj_data *create_money(int amount)
{
	...
	if (amount == 1) {
		...
		if (amount < 10) {
			...
			sprintf(buf, "There is about %d coins",
				10 * (amount / 10));
			new_descr->description = str_dup(buf);
		} else if (amount < 1000) {
			...
			new_descr->description =
			    str_dup("There are A LOT of coins");
	}
	...
	*tar_obj = NULL;
	...
	if (IS_SET(bitvector, FIND_OBJ_EQUIP)) {
		for (found = FALSE, i = 0; i < MAX_WEAR && !found; i++)
			if (ch->equipment[i] && is_abbrev(name, ch->equipment[i]->name)) {
...
void prepare_for_quit(struct char_data *ch, bool pull)
{
	...
	extern struct char_data *combat_list;
	sprintf(log_buf, "EXTRACTED! home_number=%d", ch->specials.home_number);
	strcpy(ch->specials.afk_text, log_buf);
