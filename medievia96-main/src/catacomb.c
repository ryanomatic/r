...
char flag;
    ...
    if (moveables == NULL | moveables->room == NULL)
        return;
	...
	if(moveables->type==WATER_REAPER){
	    global_color=33;
	    send_to_room("The Water Reaper touches the lake and instantly looses its tension.\rIt collapses in a splash and becomes just more water in the lake.\r ",room);
	    global_color=0;
	    moveables=my_free(moveables);
	    return;
	}
	...
	global_color=32;
	send_to_room("ROARING WIND FLOODS THE AREA.....YOU ARE BOMBARDED BY ROCK AND STONE...\rTHE GROUND SHAKES ...  WALLS RUMBLE...  You hear screaming....  then..  \n\r",room);
	if(moveables->age>10){
...
char buf2[MAX_INPUT_LENGTH];
	...
	switch(x){
		case 650:
			...
			obj->iLastDet = -1;
			send_to_char("The wizard makes several motions with his hands and says a short encantment.\rThe room glows with a blinding white light focused on the object. You see\rthe life power slowly seep into the object. \r\n",ch);
...
int caveweed(struct char_data *ch, int cmd, char *arg)
{
    ...
    if(number(1,100)<90)return(FALSE);
...
int room=0;
	...
	if(!IS_NPC(m)){
	   ...
	   if(flag==1){
		...
		global_color=33;
		send_to_room("The huge caveray Slides away...THAT WAS NO FLOOR YOU WERE ON!  OOOOoooooooh!  You fall down into the unknown...\r ",m->in_room);
		send_to_room("The ceiling above opens and people fall through!\r  ",room);	
		global_color=0;
	   }
	   char_from_room(m);
	   char_to_room(m,room);
    ...
    for(m=world[ch->in_room]->people;m;m=next){
		...
		if(!IS_NPC(m)){
		   ...
		   if(flag==1){
				...
				global_color=33;
				send_to_room("The Whirlwind moves Towards you!.....WWWwwWWwWWWWSSHSHSHSsshhSSSHHH!\rOOOOoooooooh!  You fling upward into the unknown...\r  ",m->in_room);
				send_to_room("A LOUD rush of air Pounds into the area and people appear!\r ",room);	
				global_color=0;
			}
	   		char_from_room(m);
	   		char_to_room(m,room);
