		...
		if (victim) {
			...
			if (victim == ch) {
				send_to_char("You hit yourself..OUCH!.\n\r",
					     ch);
				act("$n hits $mself, and says OUCH!", FALSE, ch,
				    0, victim, TO_ROOM);
			} else {
				if (is_in_safe(ch, victim) == TRUE) {
					act("$n tries to MURDER $N in a safe room!", FALSE, ch, 0, victim, TO_ROOM);
					return;
				}
		...
		if (victim) {
			if (victim == ch) {
				...
				if (!victim->specials.fighting
					...
					global_color = 33;
					send_to_char
					    ("You cannot start fighting someone who lost link!\n\r",
					     ch);
					global_color = 0;
					return;
				}
...
void do_backstab(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!IS_NPC(ch) && ch->specials.stpMount) {
		global_color = 33;
		send_to_char
		    ("You think about dis-mounting and backstabing..\n\r", ch);
		global_color = 0;
		return;
	}
	one_argument(argument, name);
	if (strlen(name) == 0 || !(victim = get_char_room_vis(ch, name))) {
	...
	if (IS_SET(victim->denyclass, DENYBACKSTAB)) {
		act("Your attempted backstab had no affect on $M.", TRUE, ch, 0,
		    victim, TO_CHAR);
		return;
	}
	...
	if (!IS_IN_BACK(victim)) {
		send_to_char
		    ("Its to hard to sneak up on a formation unless your target is in the back.\n\r",
		     ch);
		return;
	}
...
void do_order(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*name || !*message)
		...
		if (victim) {
			...
			act("$n gives $N an order.", FALSE, ch, 0, victim,
			    TO_ROOM);
...
void do_flee(struct char_data *ch, char *argument, int cmd)
{
	...
	if (world[ch->in_room]->zone == 198) {
		if (number(0, 100) < 30) {
			...
			if (IS_AFFECTED(ch, AFF_FLYING)) {
				...
				send_to_char
				    ("You try and flee but hit your head on some stalactite!\n\r",
				     ch);
			} else {
				act("$n tries to flee but trips over some stalagmite.", TRUE, ch, 0, 0, TO_ROOM);
				send_to_char
				    ("You try and flee but trip over some stalagmite!",
				     ch);
			}
			global_color = 0;
			return;
	...
	if (!(ch->specials.fighting)) {
		for (i = 0; i < 6; i++) {
			...
			if (CAN_GO(ch, attempt) &&
			    !IS_SET(world[EXIT(ch, attempt)->to_room]->
				    room_flags, DEATH)) {
				if ((!IS_SET
				     (world[ch->in_room]->room_flags, NEUTRAL)
				     && !IS_SET(world[ch->in_room]->room_flags,
						CHAOTIC))
				    &&
				    (IS_SET
				     (world[EXIT(ch, attempt)->to_room]->
				      room_flags, NEUTRAL)
				     ||
				     IS_SET(world[EXIT(ch, attempt)->to_room]->
					    room_flags, CHAOTIC)))
				...
				if (IS_NPC(ch))
					for (rap =
					     world[EXIT(ch, attempt)->to_room]->
					     room_afs; rap; rap = rap->next)
						if (rap->type == RA_SHIELD)
							return;
				act("$n panics, and attempts to flee.",
				    TRUE, ch, 0, 0, TO_ROOM);
				if ((die =
				     do_simple_move(ch, attempt, FALSE)) == 1) {
	...
	for (i = 0; i < 6; i++) {
		attempt = number(0, 5);	/* Select a random direction */
		if (CAN_GO(ch, attempt) &&
		    !IS_SET(world[EXIT(ch, attempt)->to_room]->room_flags,
			    DEATH)) {
			if ((!IS_SET(world[ch->in_room]->room_flags, NEUTRAL)
			     && !IS_SET(world[ch->in_room]->room_flags,
					CHAOTIC))
			    &&
			    (IS_SET
			     (world[EXIT(ch, attempt)->to_room]->room_flags,
			      NEUTRAL)
			     || IS_SET(world[EXIT(ch, attempt)->to_room]->
				       room_flags, CHAOTIC)))
				continue;
			...
			if ((die = do_simple_move(ch, attempt, FALSE)) == 1) {
				...
				if (loose > (GET_EXP(ch) / 10))
					loose = GET_EXP(ch) / 10;
				if ((!IS_NPC(ch))
				    && (IS_NPC(ch->specials.fighting)))
				...
				if (IS_NPC(ch->specials.fighting->master) && IS_NPC(ch->specials.fighting) && (ch->specials.fighting != ch) && (fLeftHp > .5))
...
void do_trap(struct char_data *ch, char *argument, int cmd)
{
	...
	if (world[room]->zone == 1) {
		send_to_char
		    ("Traps have been outlawed inside the city of Medievia.\n\r",
		     ch);
		return;
	}
	...
	if (num_traps >= 3) {
		send_to_char
		    ("Try as you might, you can't fit another trap in this room.\n\r",
		     ch);
		return;
	}
	...
	if (percent > skill) {
		global_color = 33;
		send_to_char
		    ("SNAP, %&!^^@~!!! you almost broke your hand and you broke the trap!....\n\r",
		     ch);
...
void do_meditate(struct char_data *ch, char *argument, int cmd)
{
	...
	if ((GET_CLASS(ch) != CLASS_WARRIOR) && GET_LEVEL(ch) < 33
	    ...
	    && !IS_SET(ch->player.multi_class, MULTI_CLASS_THIEF)) {
		send_to_char
		    ("You better leave all the martial arts to fighters.\n\r",
		     ch);
		return;
	}
	if (ch->specials.fighting)
	...
	if (!world[room]->people || world[room]->people != ch
			...
			for (row = 0; row < 3; row++) {
				for (col = 0; col < 3; col++) {
					if (ch->master->formation[row][col] ==
						...
						if (next_v) {
							sprintf(log_buf,
								"%s gets jostled further back as %s finishes charging.",
								GET_NAME
								(next_v),
								GET_NAME(ch));
							send_to_room(log_buf,
								     ch->
								     in_room);
						}
					}
...
void do_scan(struct char_data *ch, char *argument, int cmd)
{
	...
	act("$n focuses and remains still...scanning the area...", TRUE, ch, 0,
	    0, TO_ROOM);
	*buf = '\0';
	if (check_blind(ch))
		return;
		...
		if (EXIT(ch, door)) {
			...
			if (EXIT(ch, door)->to_room != NOWHERE) {
				if (IS_SET
					...
					global_color = 0;
					if (percent >
					    ch->skills[SKILL_SCAN].learned)
						continue;
					if (world[EXIT(ch, door)->to_room]->
					    people)
						list_char_to_char(world
								  [EXIT
								   (ch,
								    door)->
								   to_room]->people,
...
void do_throw(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!(victim = get_char_room_vis(ch, name))) {
		if (ch->specials.fighting) {
			...
			send_to_char
			    ("You turn about looking for your target...\n\r",
			     ch);
			return;
		}
	}
	if (victim == ch) {
		send_to_char
		    ("Don't you think you are a bit close to throw to?\n\r",
		     ch);
		return;
	}
	if (is_in_safe(ch, victim) == TRUE) {
	...
	if (obj->obj_flags.eq_level > GET_LEVEL(ch)) {
		send_to_char
		    ("You are not experienced enough to use that weapon\r\n",
		     ch);
		return;
	}
	if (IS_SET(victim->denyclass, DENYTHROW)) {
		act("You cannot throw against $N.", TRUE, ch, 0, victim,
		    TO_CHAR);
		return;
	}
	if (!if_allowed_to_attack(ch, victim))
...
void do_bash(struct char_data *ch, char *argument, int cmd)
{
	...
	if (IS_SET(victim->denyclass, DENYBASH)) {
		act("Your attempted bash has no affect on $N.", TRUE, ch, 0,
		    victim, TO_CHAR);
		return;
	}
	check_killer(ch, victim);
...
void do_rescue(struct char_data *ch, char *argument, int cmd)
{
	...
	if (IS_UNDEAD(victim)
		...
		send_to_char
		    ("There's nothing you can do, the gods must decide this battle.",
		     ch);
		return;
	}
	...
	if (!tmp_ch) {
		act("But nobody is fighting $M?", FALSE, ch, 0, victim,
		    TO_CHAR);
		return;
	}
...
/* beef up rescue for warriors */
	...
	if ((GET_CLASS(ch) != CLASS_WARRIOR) && GET_LEVEL(ch) < 33
		...
		act("$n heroically rescues $N.", FALSE, ch, 0, victim,
		    TO_NOTVICT);
...
void do_kick(struct char_data *ch, char *argument, int cmd)
{
	...
	if (IS_SET(victim->denyclass, DENYKICK)) {
		act("Your kick doesn't seem to hurt $N.", TRUE, ch, 0, victim,
		    TO_CHAR);
		return;
	}
	check_killer(ch, victim);
	if (!IS_IN_FRONT(ch)) {
		send_to_char
		    ("You can't get forward enough in your formation to kick.\n\r",
		     ch);
		return;
	}
	if (!IS_IN_FRONT(victim)) {
		act("You can't get close enough to $M to use your feet.", TRUE,
		    ch, 0, victim, TO_CHAR);
		return;
	}
