	...
	case 4:{
			...
			if (!sub_object) {
				for (int i = 0; i < MAX_WEAR; i++) {
					...
					if (GET_ITEM_TYPE(ch->equipment[i]) == ITEM_CONTAINER && is_abbrev(arg2, ch->equipment[i]->name)) {
						sub_object = ch->equipment[i];
						break;
					}
				}
			}
...
void do_drop(struct char_data *ch, char *argument, int cmd)
{
	...
	struct obj_data *next_obj = NULL;
	bool test = FALSE;
	char allbuf[MAX_INPUT_LENGTH] = { 0 };
 ...
 */
	...
	if (*arg) {
		...
		else if (sscanf(arg, "all.%s", allbuf) != 0) {
			do_dropalldot(ch, allbuf, cmd);
			return;
		} else {
...
void do_dropalldot(struct char_data *ch, char *name, int cmd)
{
	struct obj_data *tmp_object = NULL;
	...
	if (*arg1) {
		if (*arg2) {
			...
			if (obj_object) {
				...
				bits = generic_find(arg2, FIND_OBJ_INV | FIND_OBJ_ROOM | FIND_OBJ_EQUIP, ch, &tmp_char, &sub_object);
					...
					if (GET_ITEM_TYPE(sub_object) == ITEM_CONTAINER) {
						if (!IS_SET(sub_object->obj_flags.value[1], CONT_CLOSED)) {
							...
							if (((sub_object->obj_flags.weight) + (obj_object->obj_flags.weight)) < (sub_object->obj_flags.value[0])) {
								if (!stuffitvar)
									act("You put $p in $P.", 0, ch, obj_object, sub_object, TO_CHAR);
	...
	if (is_number(obj_name)) {
		...
		if (vict == ch) {
			...
			if (GET_GOLD(ch) < 0)
				GET_GOLD(ch) = 0;
			if (GET_LEVEL(ch) > 31) {
				sprintf(log_buf, "%s gives %d gold coins to %s.", GET_NAME(ch), amount, GET_NAME(vict));
				log_hd(log_buf);
			}
			return;
		}
		if (GET_GOLD(vict) + amount >
		    ((GET_LEVEL(vict) * GET_LEVEL(vict)) * 5555)) {
