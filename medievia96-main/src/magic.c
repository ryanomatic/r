...
extern bool put_in_formation(struct char_data *leader,
		       ...
		       struct char_data *victim, struct obj_data *obj)
{
	...
	if (victim && ch && level >= 1 && level <= 50) {
		...
		if (!saves_spell(victim, SAVING_SPELL)) {
			...
			act("You feel a painful stiffening in your muscles!",
			    FALSE, ch, 0, victim, TO_VICT);
			act("$N tenses for a moment, and moans in agony.",
			...
			struct char_data *victim, struct obj_data *obj)
{
	...
	if (victim && ch) {
		...
		if (!saves_spell(victim, SAVING_SPELL)) {
			...
			act("The bright burst of colors has blinded you!",
			    FALSE, ch, 0, victim, TO_VICT);
			act("$N turns $S eyes away and screams!", FALSE, ch, 0,
			    victim, TO_CHAR);
			act("$N turns $S eyes away and screams!", FALSE, ch, 0,
			    victim, TO_NOTVICT);
		}
			...
			struct char_data *victim, struct obj_data *obj)
{
	...
	if (victim && ch) {
		...
		if (!IS_EVIL(ch)) {
			send_to_char
			    ("Your inner spirit lacks the strength and conviction needed.\n\r",
			     ch);
			return;
		}
		if (!saves_spell(victim, SAVING_SPELL)) {
		    ...
		    struct char_data *victim, struct obj_data *obj)
{
	...
	if (victim && ch) {
		...
		if (world[ch->in_room]->sector_type == SECT_UNDERWATER) {
			send_to_char
			    ("The fireball fizzles away in the water..\n\r",
			     ch);
			return;
		}
		...
		if (saves_spell(victim, SAVING_SPELL))
			dam *= .69;
		act("You conjure a ball of magical flame and throw it at $N.",
		    FALSE, ch, 0, victim, TO_CHAR);
		act("You are enveloped in flames from $n's fireball.", TRUE, ch,
		    0, victim, TO_VICT);
		act("$n conjures a ball of magical flame and hurls it at $N.",
		     ...
		     struct char_data *victim, struct obj_data *obj)
{
	...
	if (ch != victim)
		act("$N is quickened.", FALSE, ch, 0, victim, TO_CHAR);

	act("$N is quickened.", FALSE, ch, 0, victim, TO_NOTVICT);
	...
	send_to_char("You feel quickened.\n\r", victim);
}
		 ...
		 struct char_data *victim, struct obj_data *obj)
{
	...
	if (ch != victim)
		act("$N is protected by your deity.", FALSE, ch, 0, victim,
		    TO_CHAR);
	...
	if (ch) {
		...
		for (v = world[ch->in_room]->people; v; v = next_v) {
			...
			if (IS_NPC(v) && IS_NPC(v->master) && (!IS_DEAD(v))) {
				if (IS_NPC(v) && v->specials.stpMount)
					continue;
		       ...
		       struct char_data *victim, struct obj_data *obj)
{
	...
	if (victim && ch) {
		...
		if (IS_GOOD(victim)) {
			act("The Gods protect $N.", FALSE, ch, 0, victim,
			    TO_ROOM);
		...
		if (IS_NEUTRAL(victim)) {
			act("$N does not seem to be affected.", FALSE, ch, 0,
			    victim, TO_CHAR);
			return;
		}
...
void spell_teleport(byte level, struct char_data *ch,
		    struct char_data *victim, struct obj_data *obj)
{
	int sects[] = { SECT_FIELD, SECT_FOREST, SECT_HILLS, SECT_MOUNTAIN, SECT_DESERT, SECT_SWAMP, SECT_JUNGLE, SECT_ARCTIC };
	int x = 0;
	int y = 0;
	int room = 0;
	int found = 0;
	...
	for(int tries = 100; tries > 0; tries--) {
		int distX = number(1, 30);
		int distY = number(1, 30);

		if (rand() % 2 == 0)
			x = HOLOX(victim) + distX;
		else
			x = HOLOX(victim) - distX;

		if (rand() % 2 == 0)
			y = HOLOY(victim) + distY;
		else
			y = HOLOY(victim) - distY;
		...
		for (int i = 0; i < 8; i++) {
			if (world[room]->sector_type == sects[i]) {
				found = 1;
				break;
			}
		}
	...
	if (found == 0) {
		send_to_char("Your magic could not center itself.\n", ch);
		return;
	}
	...
	if (ch && (victim || obj)) {
		...
		if (obj) {
			if ((5 * GET_LEVEL(ch) > GET_OBJ_WEIGHT(obj)) &&
				...
				act("$p briefly glows.", FALSE, ch, obj, 0,
				    TO_CHAR);
			}
		} else {
			if (!victim)
		     ...
		     struct char_data *victim, struct obj_data *obj)
{
	...
	if (victim && ch) {
		...
		if (saves_spell(victim, SAVING_SPELL)) {
			act("$N seems to be unaffected!", FALSE, ch, NULL,
			    victim, TO_CHAR);
		...
		if (!affected_by_spell(victim, SPELL_BLINDNESS)) {
			act("$n seems to be blinded!", TRUE, victim, 0, 0,
			    TO_ROOM);
		       ...
		       struct char_data *victim, struct obj_data *obj)
{
	...
	if (ch) {
		...
		tmp_obj->description =
		    str_dup
		    ("A really delicious looking magic mushroom lies here.");
		...
		act("$n creates $p.", TRUE, ch, tmp_obj, 0, TO_ROOM);
		act("You create $p.", TRUE, ch, tmp_obj, 0, TO_CHAR);
	}
}
		...
		if (GET_ITEM_TYPE(obj) == ITEM_DRINKCON) {
			if ((obj->obj_flags.value[2] != LIQ_WATER)
				...
				if (water > 0) {
					...
					act("$p is filled.", FALSE, ch, obj, 0,
					    TO_CHAR);
				}
			}
		}
	...
	if (obj) {
		...
		if (obj->obj_flags.type_flag == ITEM_WEAPON) {
			if (obj->obj_flags.value[2] > 1) {
				...
				act("$p glows red.", FALSE, ch, obj, 0,
				    TO_CHAR);
			} else {
		...
		act("$n briefly reveals a red aura!", FALSE, victim, 0, 0,
		    TO_ROOM);
		act("You feel very uncomfortable.", FALSE, victim, 0, 0,
		    TO_CHAR);
		one_hit(victim, ch, TYPE_UNDEFINED);
	}
}
			 ...
			 struct char_data *victim, struct obj_data *obj)
{
	...
	if (victim) {
		if (victim == ch)
			...
			act("You sense that $E is poisoned.", FALSE, ch, 0,
			    victim, TO_CHAR);
		} else {
			act("You sense that $E is healthy.", FALSE, ch, 0,
			    victim, TO_CHAR);
		}
	} else {		/* It's an object */
		if (!obj)
		...
		if ((obj->obj_flags.type_flag == ITEM_DRINKCON) ||
			...
			if (obj->obj_flags.value[3])
				act("Poisonous fumes are revealed.", FALSE, ch,
				    0, 0, TO_CHAR);
			else
		...
		if ((GET_ITEM_TYPE(obj) == ITEM_WEAPON) &&
			...
			if (IS_GOOD(ch)) {
				SET_BIT(obj->obj_flags.extra_flags,
					ITEM_ANTI_EVIL);
				act("$p glows blue.", FALSE, ch, obj, 0,
				    TO_CHAR);
			} else if (IS_EVIL(ch)) {
				SET_BIT(obj->obj_flags.extra_flags,
					ITEM_ANTI_GOOD);
				...
				act("$p glows yellow.", FALSE, ch, obj, 0,
				    TO_CHAR);
			}
		}
	}
	...
	if (obj && ch) {
		if (CAN_WEAR(obj, ITEM_TAKE)) {
			if (!IS_SET(obj->obj_flags.extra_flags, ITEM_INVISIBLE)) {
				act("$p turns invisible.", FALSE, ch, obj, 0,
				    TO_CHAR);
				act("$p turns invisible.", TRUE, ch, obj, 0,
				    TO_ROOM);
				SET_BIT(obj->obj_flags.extra_flags,
					ITEM_INVISIBLE);
			}
		} else
			act("You fail to make $p invisible.", FALSE, ch, obj, 0,
			    TO_ROOM);
		...
		if (!affected_by_spell(victim, SPELL_INVISIBLE)) {
			act("$n slowly fades out of existence.", TRUE, victim,
			    0, 0, TO_ROOM);
	...
	for (i = object_list; i && (j > 0); i = i->next) {
		...
		if (GET_LEVEL(ch) < 35)
			if ((isname(name, i->name))
			    &&
			    (!IS_SET(i->obj_flags.extra_flags, ITEM_INVISIBLE)))
			{
				if (i->carried_by) {
		  ...
		  struct char_data *victim, struct obj_data *obj)
{
	...
	if (victim) {
		...
		if (!saves_spell(victim, SAVING_PARA)) {
			...
			act("$N winces as the poison enters $S body.", FALSE,
			    ch, 0, victim, TO_CHAR);
			act("$N winces as the poison enters $S body.", FALSE,
			    ch, 0, victim, TO_NOTVICT);
		} else
			act("$N resists the magic.", FALSE, ch, NULL, victim,
			    TO_CHAR);
				...
				struct char_data *victim, struct obj_data *obj)
{
	...
	if (GET_ALIGNMENT(ch) < 350) {
		send_to_char
		    ("Your soul is impure and the Gods fail to answer your prays.\n\r",
		     ch);
		return;
	}
				...
				struct char_data *victim, struct obj_data *obj)
{
	...
	if (!affected_by_spell(victim, SPELL_PROTECT_FROM_GOOD))
		send_to_char
		    ("You feel the power of vile darkness surge through your body!\n\r",
		     victim);
			...
			struct char_data *victim, struct obj_data *obj)
{
	...
	if (obj) {
		...
		if (affected_by_spell(victim, SPELL_CURSE)) {
			act("$n briefly glows red, then blue.", FALSE, victim,
			    0, 0, TO_ROOM);
			 ...
			 struct char_data *victim, struct obj_data *obj)
{
	...
	if (victim) {
		if (affected_by_spell(victim, SPELL_POISON)) {
			...
			act("A warm feeling runs through your body.", FALSE,
			    victim, 0, 0, TO_CHAR);
		...
		if ((obj->obj_flags.type_flag == ITEM_DRINKCON) ||
			...
			act("The $p steams briefly.", FALSE, ch, obj, 0,
			    TO_CHAR);
		}
	}
}
		     ...
		     struct char_data *victim, struct obj_data *obj)
{
	...
	if (!affected_by_spell(victim, SPELL_SANCTUARY)) {
		act("$n is surrounded by a white aura.", TRUE, victim, 0, 0,
		    TO_ROOM);
		act("You start glowing.", TRUE, victim, 0, 0, TO_CHAR);
	}

	af.type = SPELL_SANCTUARY;
	af.duration = (level < 32) ? 10 : level;
	af.modifier = 0;
	af.location = APPLY_NONE;
		      ...
		      struct char_data *victim, struct obj_data *obj)
{
	...
	if (!affected_by_spell(victim, SPELL_FIRESHIELD)) {
		act("$n is surrounded by a deep red shield of fire.", TRUE,
		    victim, 0, 0, TO_ROOM);
		act("A deep red shield of fire surrounds you.", TRUE, victim, 0,
		    0, TO_CHAR);
	}
		 ...
		 struct char_data *victim, struct obj_data *obj)
{
	...
	if (GET_LEVEL(victim) >= 20) {
		act("$N laughs in your face at your feeble attempt.", FALSE, ch,
		    NULL, victim, TO_CHAR);
		act("$N tries to sleep you, but fails horribly.", FALSE, ch,
		    NULL, victim, TO_VICT);
		one_hit(victim, ch, TYPE_UNDEFINED);
		return;
	}
	...
	if ((!saves_spell(victim, SAVING_SPELL)) || (GET_LEVEL(ch) > 31)) {
		...
		if (GET_POS(victim) > POSITION_SLEEPING) {
			act("You feel very sleepy ..... zzzzzz", FALSE, victim,
			    0, 0, TO_CHAR);
			act("$n suddenly curls up and falls asleep.", TRUE,
			    victim, 0, 0, TO_ROOM);
		...
		act("$N does not look sleepy!", FALSE, ch, NULL, victim,
		    TO_CHAR);
			  ...
			  struct char_data *victim, struct obj_data *obj)
{
	...
	iDist = (int)dHoloDistance(HOLOX(victim),
				   HOLOY(victim),
				   HOLORX(loc_nr), HOLORY(loc_nr));

	if (iDist > 400) {
		send_to_char
		    ("You try to focus your power over such a great distance and fail.\r\n",
		     victim);
		return;
	}
...
*/
	...
	MOUNTMOVE = FALSE;
	act("$n appears in the room with a bright flash of white light.", TRUE,
	    victim, 0, 0, TO_ROOM);
	do_look(victim, "", 15);
...
{
	char* objs[] = { "bag", "lantern", "paper", "quill", "raft", "trap", "waterskin" };
	int obj_nums[] = { 3032, 3031, 1205, 1206, 3050, 9822, 305 };
	int num_objs = sizeof(objs) / sizeof(objs[0]);
	struct obj_data* obj;
	int r_num = 0;
	int number = 0;

	for (int i = 0; i < num_objs; i++) {
		if (is_abbrev(arg, objs[i])) {
			number = obj_nums[i];
			break;
		}
	}
	...
	if (number == 0) {
		send_to_char("You are not skilled enough to create that!\n", ch);
		return;
	}
	...
	act("$n creates $p!", FALSE, ch, obj, 0, TO_NOTVICT);
}
		 ...
		 struct char_data *victim, struct obj_data *obj)
{
	...
	if (iHoloChDistance(ch, victim) > (GET_LEVEL(ch) * 5) && GET_LEVEL(ch) <= 31) {
		send_to_char("You try to focus your powers over to vast a distance and fail.\r\n", ch);
		return;
	}
	if (IS_SET(world[victim->in_room]->room_flags, NEUTRAL) || IS_SET(world[victim->in_room]->room_flags, CHAOTIC)) {
		global_color = 33;
		send_to_char("The area seems to be protected from phasing!\n\r", ch);
		global_color = 0;
		return;
	}
	...
	for (i = descriptor_list; i; i = i->next)
		if (i->character != ch && !i->connected && GET_LEVEL(i->character) > 30)
	...
	act("$n has phased you!", FALSE, ch, 0, victim, TO_VICT);
	global_color = 0;
	do_look(ch, "", 15);
}
		  ...
		  struct char_data *victim, struct obj_data *obj)
{
	...
	if (iHoloChDistance(ch, victim) > (GET_LEVEL(ch) * 5)
	    && GET_LEVEL(ch) <= 31) {
		send_to_char
		    ("You try to focus your powers over to vast a distance and fail.\r\n",
		     ch);
		return;
	}
	...
	if (IS_SET(world[ch->in_room]->room_flags, NEUTRAL) ||
		...
		global_color = 33;
		send_to_char
		    ("The area seems to be protected from summoning!\n\r", ch);
		global_color = 0;
		return;
	}
	if (IS_SET(world[victim->in_room]->room_flags, NO_SUMMON)) {
		send_to_char
		    ("A magical force greater than your own prevents your summons.\n\r",
		     ch);
		return;
	}
	target = ch->in_room;
	...
	for (i = descriptor_list; i; i = i->next)
		if (i->character != ch && !i->connected &&
			...
			if (IS_SET
			    (i->character->specials.god_display, GOD_SUMMONS))
				send_to_char(log_buf, i->character);
...
/*
	if (GET_LEVEL(ch) <= 31 && IS_SET(victim->specials.act, ACT_AGGRESSIVE)) {
...
*/
	...
	if (GET_LEVEL(ch) > 31 || (!IS_AFFECTED(victim, AFF_CHARM) && !IS_AFFECTED(ch, AFF_CHARM) && (level >= GET_LEVEL(victim)))) {
		if (is_formed(victim)) {
			send_to_char("Didn't seem to work.\n\r", ch);
			return;
		}
		if (GET_LEVEL(ch) <= 31 && saves_spell(victim, SAVING_SPELL)) {
			act("$N doesnt seem to like you!", FALSE, ch, NULL, victim, TO_CHAR);
			one_hit(victim, ch, TYPE_UNDEFINED);
			return;
		}
		...
		act("$N becomes groggy and begins following $n.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$N becomes groggy and begins following you.", FALSE, ch, 0, victim, TO_CHAR);
	}
}
		    ...
		    struct char_data *victim, struct obj_data *obj)
{
	...
	if (obj) {
		...
		if (obj->obj_flags.bitvector) {
			send_to_char
			    ("Item will give you following abilities:  ", ch);
		...
		fPercent =
		    (float)time(0) / (float)(obj->iDetLife + obj->iBornDate);
		if (obj->iDetLife == -1)
			Det = 0;
		else if (fPercent >= 1)
			Det = 9;
			...
			struct char_data *victim, struct obj_data *obj)
{
	...
	if (number(0, 50) < GET_LEVEL(ch)) {
		if (!saves_spell(victim, SAVING_BREATH)) {
			...
			if (frozen) {
				act("$o breaks.", 0, victim, frozen, 0,
				    TO_CHAR);
				extract_obj(frozen);
			}
		}
		...
		if (!saves_spell(victim, SAVING_BREATH)) {
			for (damaged = 0; damaged < MAX_WEAR &&
			     !((victim->equipment[damaged]) &&
			       (victim->equipment[damaged]->obj_flags.
				type_flag != ITEM_ARMOR)
			       && (victim->equipment[damaged]->obj_flags.
				   value[0] > 0) && (number(0, 2) == 0));
			     damaged++) ;
			if (damaged < MAX_WEAR) {
				act("$o is damaged.", 0, victim,
				    victim->equipment[damaged], 0, TO_CHAR);
	...
	if (!affected_by_spell(victim, SPELL_FLY)) {
		...
		if (victim != ch) {
			if (CAN_SEE(ch, victim))
				act("$N's feet rise off the ground.", TRUE, ch,
				    0, victim, TO_CHAR);
		} else
			send_to_char("Your feet rise up off the ground.\n\r",
				     ch);
		if (CAN_SEE(ch, victim))
			act("$N's feet rise off the ground.", TRUE, ch, 0,
			    victim, TO_NOTVICT);
	}
	af.type = SPELL_FLY;
			...
			struct char_data *victim, struct obj_data *obj)
{
	...
	act("You feel your lungs harden and heat up!", TRUE, ch, 0, victim,
	    TO_VICT);
	if (victim != ch) {
		act("$N gasps and breathes deeper.", TRUE, ch, 0, victim,
		    TO_CHAR);
	} else {
		send_to_char("You gasp and breathe in deeply.\n\r", ch);
	}
	act("$N gasps and breathes in deeply .", TRUE, ch, 0, victim,
	    TO_NOTVICT);
...
*/
	...
	tmp_obj->description =
	    str_dup("There is a bright ball of light on the ground here.");
	...
	act("$n twiddles $s thumbs and $p suddenly appears.", TRUE, ch, tmp_obj,
	    0, TO_ROOM);
...
*/
	...
	act("With mystic power, $n animates $p.", TRUE, ch, corpse, 0, TO_ROOM);
			  ...
			  struct char_data *victim, struct obj_data *obj)
{
	...
	if (ap > 700)
		...
		sprintf(buf,
			"%s doesn't seem to have a firm moral commitment\n\r",
			name);
	else if (ap > -100)
		sprintf(buf, "%s isn't the worst you've come across\n\r", name);
	else if (ap > -350)
		sprintf(buf,
			"%s could be a little nicer, but who couldn't?\n\r",
			name);
	else if (ap > -700)
		sprintf(buf, "%s probably just had a bad childhood\n\r", name);
	else
		sprintf(buf,
			"I'd rather just not say anything at all about %s\n\r",
			name);
			...
			struct char_data *victim, struct obj_data *obj)
{
	if (obj) {
		if (IS_SET(obj->obj_flags.extra_flags, ITEM_INVISIBLE)) {
			...
			send_to_char("The invisibility spell is broken.\n", ch);
		}
		else {
	...
	if (GET_POS(victim) == POSITION_DEAD) {
		if (ch)
			send_to_char
			    ("The magic of the necromancer is too powerful, and you shirk away in terror.\n\r",
			     ch);
		return;
	}
	if (level != -1)
	...
	if (GET_LEVEL(victim) - GET_LEVEL(ch) < 10) {
		...
		if (affected_by_spell(victim, SPELL_SANCTUARY)) {
			...
			if (yes || !saves_spell(victim, SAVING_SPELL)) {
				...
				send_to_char
				    ("You don't feel so invulnerable any more.\n\r",
				     victim);
				act("The white glow around $n's body fades.",
		...
		if (affected_by_spell(victim, SPELL_SHIELD))
			if (yes || !saves_spell(victim, SAVING_SPELL)) {
				...
				send_to_char
				    ("Your magical shield has been dispelled.\n\r",
				     victim);
			}
		if (affected_by_spell(victim, SPELL_STONE_SKIN))
			if (yes || !saves_spell(victim, SAVING_SPELL)) {
				...
				send_to_char
				    ("Your skin feels like stone no longer.\n\r",
				     victim);
			}
		if (affected_by_spell(victim, SPELL_FLY))
			if (yes || !saves_spell(victim, SAVING_SPELL)) {
				...
				send_to_char
				    ("You don't feel lighter than air any more.\n\r",
				     victim);
			}
		if (affected_by_spell(victim, SPELL_FIRESHIELD))
			if (yes || !saves_spell(victim, SAVING_SPELL)) {
		...
		if (affected_by_spell(victim, SPELL_BREATH_WATER))
			if (yes || !saves_spell(victim, SAVING_SPELL)) {
				...
				send_to_char
				    ("Your lungs heave and you cough water.\n\r",
				     victim);
				act("$n's lungs heave and $e coughs water.",
				    FALSE, victim, 0, 0, TO_ROOM);
			}
		if (affected_by_spell(victim, SPELL_MAP_CATACOMBS))
			if (yes || !saves_spell(victim, SAVING_SPELL)) {
				...
				send_to_char
				    ("You get woozy as your purple mapping aura fades.\n\r",
				     victim);
			     ...
			     struct char_data *victim, struct obj_data *obj)
{
	...
	if (!ch)
		return;
	send_to_char
	    ("You focus in on yourself, drawing in upon your heartbeat.\n\r",
	     ch);
	mob_num = 0;
	/*
	 ** objects:
	...
	if (obj_object) {
		if (obj_object->item_number == 5243
		    || obj_object->item_number == 5233
		    || obj_object->item_number == 5239
		    || obj_object->item_number == 5230) {
			if (obj_object->item_number == 5243)
				mob_num = 3;
			if (obj_object->item_number == 5233)
				mob_num = 4;
			if (obj_object->item_number == 5239)
				mob_num = 5;
			if (obj_object->item_number == 5230)
				mob_num = 6;
	...
	if (!mob_num) {
		send_to_char
		    ("You cast the spell and fail miserably, perhaps the wrong sacrificial object.\n\r",
		     ch);
		return;
	}
	...
	act("$n gestures, and a cloud of smoke appears", TRUE, ch, 0, 0,
	    TO_ROOM);
	act("$n gestures, and a cloud of smoke appears", TRUE, ch, 0, 0,
	    TO_CHAR);
	...
	act("Out of the smoke, $N emerges!!", TRUE, ch, 0, elemental,
	    TO_NOTVICT);
		      ...
		      struct char_data *victim, struct obj_data *obj)
{
	...
	if (!affected_by_spell(ch, SPELL_STONE_SKIN)) {
		act("$n's skin turns grey and granite-like.", TRUE, ch, 0, 0,
		    TO_ROOM);
		act("Your skin turns to a stone-like substance.", TRUE, ch, 0,
		    0, TO_CHAR);
	}
		  ...
		  struct char_data *victim, struct obj_data *obj)
{
	...
	if (!affected_by_spell(victim, SPELL_SHIELD)) {
		send_to_char
		    ("You are surrounded by a strong magical force shield.\r\n",
		     victim);
		act("$N glows for a moment as $e is protected by a magical force shield.", FALSE, ch, 0, victim, TO_NOTVICT);
	}
		  ...
		  struct char_data *victim, struct obj_data *obj)
{
	...
	if (!affected_by_spell(victim, SPELL_WEAKEN)) {
		if (!saves_spell(victim, SAVING_SPELL)) {
			...
			af.modifier =
			    (int)0 - (victim->abilities.str * modifier);
			af.location = APPLY_STR;
			af.bitvector = 0;
		       ...
		       struct char_data *victim, struct obj_data *obj)
{
	...
	if (!ch || !victim || !if_allowed_to_attack(ch, victim))
		return;
		     ...
		     struct char_data *victim, struct obj_data *obj)
{
	...
	if (!victim || (victim->in_room == NOWHERE))
		return;

	chance = (level * 5) - 10;
	if ((chance > number(1, 100))
	    && world[ch->in_room]->dir_option[dir = number(0, 5)]) {
		act("Your sandstorm hurls $N out of the room!", FALSE, ch, 0,
		    victim, TO_CHAR);
		act("$N is hurled out of the room!", FALSE, ch, 0, victim,
		    TO_NOTVICT);
		act("You are hurled out of the room!", FALSE, ch, 0, victim,
		    TO_VICT);
		...
		act("$n is hurled into the room!", FALSE, victim, 0, 0,
		    TO_ROOM);
	}
}
			 ...
			 struct char_data *victim, struct obj_data *obj)
{
	...
	act("You sense $S aura drawing closer...", FALSE, ch, 0, victim,
	    TO_CHAR);

	act("$n hurls a stream of violent wind at $N!", FALSE, ch, 0, victim,
	    TO_ROOM);
	...
	GET_MOVE(ch) /= 2;
	send_to_char
	    ("Creating such a wind causes you to lose your balance.\n\r", ch);
		  ...
		  struct char_data *victim, struct obj_data *obj)
{
	...
	if (GET_LEVEL(victim) >= GET_LEVEL(ch))
		if (saves_spell(victim, SAVING_SPELL) || (!number(0, 5))) {
			act("$N appears to block the magic.", FALSE, ch, 0,
			    victim, TO_CHAR);
			one_hit(victim, ch, TYPE_UNDEFINED);
			return;
		}
	...
	if (IS_AFFECTED(victim, AFF_PLAGUE)) {
		act("$N, sick as $E may be, attacks!", FALSE, ch, 0, victim,
		    TO_CHAR);
		act("$N, sick as $E may be, attacks!", FALSE, ch, 0, victim,
		    TO_ROOM);
		one_hit(victim, ch, TYPE_UNDEFINED);
	...
	act("$n chokes and utters some muffled noises.", FALSE, victim, 0, 0,
	    TO_ROOM);
	act("You choke and utter some muffled noises.", FALSE, victim, 0, 0,
	    TO_CHAR);
	GET_HIT(victim) -= GET_LEVEL(ch);
	GET_HIT(victim) -=
	    number(1, (int)(GET_HIT(victim) * (.01 * GET_LEVEL(ch))));
	if (GET_HIT(victim) < 1)
		GET_HIT(victim) = 1;
}
			   ...
			   struct char_data *victim, struct obj_data *obj)
{
	...
	if ((GET_ALIGNMENT(ch) > 350 && GET_ALIGNMENT(victim) > 350) ||
		...
		dam = 0;
		send_to_char
		    ("The shimmering, ghostly image of a warhammer appears in your hands.\n\r",
		     ch);
			 ...
			 struct char_data *victim, struct obj_data *tar_obj)
{
	...
	case SPELL_TYPE_SPELL:
		if (OUTSIDE(ch) && (weather_info.sky >= SKY_RAINING)) {
			...
			send_to_char
			    ("You fail to call the lightning down from the sky!\n\r",
			     ch);
		}
		break;
...
void cast_sense_death(byte level, struct char_data *ch, char *arg, int type,
		      struct char_data *tar_ch, struct obj_data *tar_obj)
{
	...
	case SPELL_TYPE_SPELL:
		...
		if (!world[room])
			return;
		if (!world[room]->room_afs)
			return;
		send_to_char
		    ("You focus....you call upon death......you sense...\n\r",
		     ch);
...
void cast_shield_room(byte level, struct char_data *ch, char *arg, int type,
		      struct char_data *tar_ch, struct obj_data *tar_obj)
{
	...
	case SPELL_TYPE_SPELL:
		...
		while (ra) {
			if (ra->type == RA_SHIELD) {
				send_to_char
				    ("Your concentration breaks down because the room is already shielded.\n\r",
				     ch);
				return;
			}
			ra = ra->next;
		...
		send_to_char
		    ("You concentrate deep...feeling the area..anchoring yourself...\n\r",
		     ch);
		send_to_room
		      ...
		      struct char_data *tar_ch, struct obj_data *tar_obj)
{
	...
	switch (type) {
	case SPELL_TYPE_SPELL:
		act("$n magically creates a mushroom.", FALSE, ch, 0, 0,
		    TO_ROOM);
		 ...
		 struct char_data *tar_ch, struct obj_data *tar_obj)
{
	...
	case SPELL_TYPE_SPELL:
		...
		if (!buf || !buf[0]) {
			send_to_char
			    ("Scribe a scroll...but make what scroll?\n\r", ch);
			return;
		}
		...
		if (!obj || obj->obj_flags.type_flag != ITEM_NOTE) {
			send_to_char
			    ("You remember you must be Holding some sort of paper or note.\n\r",
			     ch);
			return;
		}
		if (amount_blood_in_room(ch->in_room) < 4) {
			send_to_char
			    ("There doesn't seem to be enough blood here..\n\r",
			     ch);
			return;
		}
		act("$n throws $s hands up and $s hands start bleeding terribly. ", TRUE, ch, 0, 0, TO_ROOM);
		send_to_char
		    ("You throw your hands up in the air and they start bleeding terribly..\n\r",
		     ch);
		   ...
		   struct char_data *tar_ch, struct obj_data *tar_obj)
{
	...
	case SPELL_TYPE_SCROLL:
		...
		if (!arg || !arg[0]) {
			send_to_char
			    ("You try and focus....but focusin what direction?\n\r",
			     ch);
			return;
		}
		switch (arg[0]) {
		...
		default:
			send_to_char
			    ("You realize you dont understand that direction.\n\r",
			     ch);
			return;
		}
		global_color = 33;
		sprintf(log_buf,
			"$n inhales...looks %s and folds $s hands across $s chest.",
			direction);
...
{
	if (GET_LEVEL(ch) <= 31) {
		send_to_char("Only gods may cast this command.\n", ch);
		return;
	}
	...
	case SPELL_TYPE_SPELL:
		...
		if (!arg || !arg[0]) {
			send_to_char
			    ("You try and focus....but focus in what direction?\n\r",
			     ch);
			return;
		}
		switch (arg[0]) {
		...
		if (world[ch->in_room]->dir_option[dir]) {
			if (!is_allowed_here
			    (ch,
			     world[ch->in_room]->dir_option[dir]->to_room)) {
				act("$n looks visibly shaken...", TRUE, ch, 0,
				    0, TO_ROOM);
				return;
			}
			sprintf(log_buf,
				"The image of $n fades some then becomes transparent..$e walks %s.",
				direction);
			act(log_buf, TRUE, ch, 0, 0, TO_ROOM);
			sprintf(log_buf,
				"You feel disoriented..then amazingly light..you walk %s....\n\r",
				direction);
			send_to_char(log_buf, ch);
			...
			send_to_char
			    ("That direction seems to push you away.\n\r", ch);
			return;
		}
		break;
	default:
		     ...
		     struct char_data *tar_ch, struct obj_data *tar_obj)
{
	...
	if (!world[ch->in_room]->holox || !world[ch->in_room]->holoy) {
		send_to_char
		    ("You feel unable to concentrate your inner eye here.\r\n",
		     ch);
		return;
	}
		    ...
		    struct char_data *tar_ch, struct obj_data *tar_obj)
{
	...
	if (STATE(vict->desc) != CON_HOVERING) {
		send_to_char
		    ("You can only resurrect dead corpses before necromancer takes them away.\n\r",
		     ch);
		return;
	}
	...
	for (x = 0; x < 3; x++) {
		for (y = 0; y < 3; y++) {
			if (ch->master->formation[x][y])
				if (ch->in_room ==
				    ch->master->formation[x][y]->in_room)
	...
	if (h < hits) {
		send_to_char
		    ("You could not gather enough strength from your comrads.\n\r",
		     ch);
		return;
	}
	f = hits / h;
						    ...
						    ((int)
						     (f *
						      (float)GET_HIT(ch->
								     ...
								     [x][y])));
						send_to_char
						    ("You feel your strength drain into the corpse...\n\r",
						     ch->master->
						     formation[x][y]);
					}
		}
	...
	act("$n bends over the corpse of $N and utters some words.", TRUE, ch,
	    0, vict, TO_NOTVICT);
	act("$n bends over your limp body and utters some words.", TRUE, ch, 0,
	    vict, TO_VICT);
	act("You bend over $N's limp body and utter some words.", TRUE, ch, 0,
	    vict, TO_CHAR);
	global_color = 33;
		    ...
		    struct char_data *tar_ch, struct obj_data *tar_obj)
{
	...
	case SPELL_TYPE_SPELL:
		if (amount_blood_in_room(ch->in_room) < 10) {
			send_to_char
			    ("As you start concentrating you realize there is not enough blood here\n\r",
			     ch);
			return;
		}
		global_color = 32;
		act("$n Throws $s hands high in the air...Thunder rolls close by....  You GASP as $e starts bleeding all over....  ", TRUE, ch, 0, 0, TO_ROOM);
		      ...
		      struct char_data *tar_ch, struct obj_data *tar_obj)
{
	...
	case SPELL_TYPE_SPELL:
		if (GET_LEVEL(ch) == 32)
			sprintf(log_buf,
				"## %s(32) casts cure critic on %s(%d)",
				GET_NAME(ch), GET_NAME(tar_ch),
				GET_LEVEL(tar_ch));
	       ...
	       struct char_data *tar_ch, struct obj_data *tar_obj)
{
	...
	case SPELL_TYPE_SCROLL:
		if (!tar_ch)
			tar_ch = ch;
		act("Magical energy surrounds $N's body, healing $m.", FALSE,
		       ...
		       struct char_data *tar_ch, struct obj_data *tar_obj)
{
	...
	case SPELL_TYPE_SPELL:
		...
		if (tar_obj) {
			if (IS_SET
			    (tar_obj->obj_flags.extra_flags, ITEM_INVISIBLE))
				send_to_char("Nothing new seems to happen.\n\r",
					     ch);
			else
		...
		struct char_data *tar_ch, struct obj_data *tar_obj)
{
	switch (type) {
	case SPELL_TYPE_SPELL:
		spell_phase(level, ch, tar_ch, 0);
		break;
	default:
		log_hd("Serious screw-up in phase!");
		break;
	}
}
		       ...
		       struct char_data *tar_ch, struct obj_data *tar_obj)
{
	...
	case SPELL_TYPE_SPELL:
		if (GET_LEVEL(ch) == 32)
			sprintf(log_buf,
				"## %s(32) casts cure serious on %s(%d)",
				GET_NAME(ch), GET_NAME(tar_ch),
				GET_LEVEL(tar_ch));
	       ...
	       int type, struct char_data *tar_ch, struct obj_data *tar_obj)
{
	...
	dir = 0;
	act("$n crosses $s arms on $s chest and fills $s lungs with air.....",
	    TRUE, ch, 0, 0, TO_ROOM);
	send_to_room
	    ("You cross your arms on your chest and breath in deeply...\n\r",
	     ch->in_room);
	if (!world[ch->in_room]->dir_option[NORTH])
		dir = 'n';
	if (!world[ch->in_room]->dir_option[SOUTH])
		dir = 's';
	if (!world[ch->in_room]->dir_option[WEST])
		dir = 'w';
	if (!world[ch->in_room]->dir_option[EAST])
		dir = 'e';
	if (!dir || world[ch->in_room]->zone != 198) {
		send_to_char
		    ("You focus till you feel you will burst...but..couldn't seem to find the right direction...\n\r",
		     ch);
		return;
	}
	if (gate_room) {
		...
		global_color = 31;
		send_to_room
		    ("*****WWWOOOOOoooooSHSHHH!*** the portal collapses.\n\r",
		     gate_room);
		global_color = 0;
		gate_room = 0;
		gate_dir = 0;
	...
	global_color = 33;
	send_to_room
	    ("The temperature in the area drops below zero.......then....\r <<<<* CRACK *>>>>\r A sudden wind almost knock you over..you notice a strange portal...\r  ",
	     ch->in_room);
	global_color = 0;
}
		      ...
		      struct obj_data *tar_obj)
{
	...
	case SPELL_TYPE_STAFF:
		for (tar_ch = world[ch->in_room]->people;
		     tar_ch; tar_ch = tar_ch->next_in_room)
			if (IS_NPC(tar_ch)
			    && IS_SET(tar_ch->specials.act, ACT_UNDEAD))
				spell_turn_undead(level, ch, tar_ch, 0);
		break;
	default:
...
void cast_quickness(byte level, struct char_data *ch, char *arg, int type,
		    struct char_data *tar_ch, struct obj_data *tar_obj)
{
	spell_quickness(level, ch, tar_ch, 0);
}
