		...
		for (col = 0; col < 3; col++)
			if (ch->master->formation[row][col] == ch) {
				if (ch->master->formation[0][col]) {
					if (ch->master->formation[0][col] == ch)
						return (TRUE);
					else {
						if (ch->master->
						    formation[0][col]->
				...
				if ((ch->master->formation[0][0]
				     && ch->master->formation[0][0]->in_room ==
				     room) || (ch->master->formation[0][1]
					       && ch->master->formation[0][1]->
					       in_room == room)
				    || (ch->master->formation[0][2]
					&& ch->master->formation[0][2]->
					in_room == room))
					return (FALSE);
				if (ch->master->formation[1][col]) {
					if (ch->master->formation[1][col] == ch)
						return (TRUE);
					else {
						if (ch->master->
						    formation[1][col]->
				...
				if ((ch->master->formation[1][0]
				     && ch->master->formation[1][0]->in_room ==
				     room) || (ch->master->formation[1][1]
					       && ch->master->formation[1][1]->
					       in_room == room)
				    || (ch->master->formation[1][2]
					&& ch->master->formation[1][2]->
					in_room == room))
					return (FALSE);
				return (TRUE);
		...
		for (col = 0; col < 3; col++)
			if (ch->master->formation[row][col] == ch) {
				if (ch->master->formation[2][col]) {
					if (ch->master->formation[2][col] == ch)
						return (TRUE);
					else {
						if (ch->master->
						    formation[2][col]->
				...
				if ((ch->master->formation[2][0]
				     && ch->master->formation[2][0]->in_room ==
				     room) || (ch->master->formation[2][1]
					       && ch->master->formation[2][1]->
					       in_room == room)
				    || (ch->master->formation[2][2]
					&& ch->master->formation[2][2]->
					in_room == room))
					return (FALSE);
				if (ch->master->formation[1][col]) {
...
void do_formtest(struct char_data *ch, char *argument, int cmd)
{
	...
	sprintf(log_buf,
		" Breakdown of firing hits and %%'s\r~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\r");
	send_to_char(log_buf, ch);
	if (!array[0][0])
		sprintf(log_buf, "---- --%%     ");
	else
		sprintf(log_buf, "%4d %2d%%     ", array[0][0],
			return_perc(array[0][0], n));
	send_to_char(log_buf, ch);
	if (!array[0][1])
		sprintf(log_buf, "---- --%%     ");
	else
		sprintf(log_buf, "%4d %2d%%     ", array[0][1],
			return_perc(array[0][1], n));
	send_to_char(log_buf, ch);
	if (!array[0][2])
		sprintf(log_buf, "---- --%%");
	else
		sprintf(log_buf, "%4d %2d%%     ", array[0][2],
			return_perc(array[0][2], n));
	send_to_char(log_buf, ch);
	send_to_char("\n\r", ch);
	if (!array[1][0])
		sprintf(log_buf, "---- --%%     ");
	else
		sprintf(log_buf, "%4d %2d%%     ", array[1][0],
			return_perc(array[1][0], n));
	send_to_char(log_buf, ch);
	if (!array[1][1])
		sprintf(log_buf, "---- --%%     ");
	else
		sprintf(log_buf, "%4d %2d%%     ", array[1][1],
			return_perc(array[1][1], n));
	send_to_char(log_buf, ch);
	if (!array[1][2])
		sprintf(log_buf, "---- --%%");
	else
		sprintf(log_buf, "%4d %2d%%     ", array[1][2],
			return_perc(array[1][2], n));
	send_to_char(log_buf, ch);
	send_to_char("\n\r", ch);
	if (!array[2][0])
		sprintf(log_buf, "---- --%%     ");
	else
		sprintf(log_buf, "%4d %2d%%     ", array[2][0],
			return_perc(array[2][0], n));
	send_to_char(log_buf, ch);
	if (!array[2][1])
		sprintf(log_buf, "---- --%%     ");
	else
		sprintf(log_buf, "%4d %2d%%     ", array[2][1],
			return_perc(array[2][1], n));
	send_to_char(log_buf, ch);
	if (!array[2][2])
		sprintf(log_buf, "---- --%%");
	else
		sprintf(log_buf, "%4d %2d%%     ", array[2][2],
			return_perc(array[2][2], n));
	send_to_char(log_buf, ch);
	send_to_char("\n\r", ch);
}
...
struct char_data *pick_victim(struct char_data *ch)
{
	...
	while (1) {
		...
		if (ch->formation[row][col]) {
			if (mobflag && IS_NPC(ch->formation[row][col]))
				num = 30;
			else
				num = 70;
			if (number(0, 100) < num)
	...
	for (x = 0; x < 3; x++) {
		for (y = 0; y < 3; y++) {
			if (ch->master->formation[x][y] == ch) {
				sprintf(log_buf, "%s is removed from %s's formation.\n\r", (IS_NPC(ch) ? ch->player.short_descr : GET_NAME(ch)), (IS_NPC(ch->master) ? ch->master->player.short_descr : GET_NAME(ch->master)));
				...
				if (IS_NPC(ch) && ch->nr < 5821 && ch->nr > 5825)
				...
				if (IS_NPC(ch) && ch->nr >= 5821 && ch->nr <= 5825) {
					...
					global_color = 30;
					act("$N vanishes suddenly, leaving a cloud of black smoke.", FALSE, ch, 0, NULL, TO_ROOM);
					global_color = 0;

					extract_char(ch, TRUE);

					struct obj_data* obj = get_obj_in_list_num(5757, world[room]->contents);
					if (obj) {
						extract_obj(obj);
					}
				}
...
/*
        for (int x = 0; x < 3; x++) {
                for (int y = 0; y < 3; y++) {
                        if (ch->formation[x][y]) {
                                ...
                                else if (IS_NPC(ch->formation[x][y])) {
                                        extract_char(ch->formation[x][y], TRUE);
                                }
                        }
                }
        }
        ...
        for (int i = 0; i < 2; i++) {
                struct obj_data* obj = get_obj_in_list_vis(ch, "frostreaver", world[ch->in_room]->contents);
                if (obj) {
                        extract_obj(obj);
                }
        }
...
void do_reform(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!name[0] || !pos[0]) {
		sprintf(log_buf,
			"   SYNTAX: reform name position(row,column)\rPositions: fl fc fr\r           cl cc cr\r           bl bc br\rRows    f=front c=center b=back\rColumns l=left  c=center r=right\r\rExample: reform vryce bl    (put Vryce in back left position)\n\r");
		send_to_char(log_buf, ch);
		return;
	}
	...
	sprintf(log_buf, "%s is moved from %s/%s to %s/%s.\n\r",
		(IS_NPC(player) ? player->player.
		 short_descr : GET_NAME(player)), or, oc, nr, nc);
	for (x = 0; x < 3; x++)
		for (y = 0; y < 3; y++)
			if (ch->formation[x][y]
			    && ch->formation[x][y]->in_room != ch->in_room)
...
void do_unform(struct char_data *ch, char *argument, int cmd)
{
	...
	if (*name) {
		...
		send_to_char
		    ("SYNTAX: unform name (remove him/her from your formation)\n\r        unform YOURNAME unform your whole group.\n\r",
		     ch);
		return;
	}
	if (ch->master != ch) {
...
void do_follow(struct char_data *ch, char *argument, int cmd)
{
	...
	if (cmd == 1000) {
		if (ch->ask_for_follow) {
			if (!ch->ask_for_follow->desc) {
				send_to_char
				    ("That person seems to have lost link.\r\n",
				     ch);
			...
			if (name[0] == 'y' || name[0] == 'Y') {
				if (ch->ask_for_follow->master !=
				    ch->ask_for_follow) {
					send_to_char
					    ("Too late, that person has joined another.\n\r",
					     ch);
					ch->ask_for_follow = NULL;
					ch->p->querycommand = 0;
					return;
				...
				if (!put_in_formation(ch, ch->ask_for_follow)) {
					...
					send_to_char("Formation is FULL!",
						     ch->ask_for_follow);
				} else {
					act("OK! $n allows you, you start following.", TRUE, ch, 0, ch->ask_for_follow, TO_VICT);
					act("Ok, $N starts following you.",
					    TRUE, ch, 0, ch->ask_for_follow,
					    TO_CHAR);
					act("$N starts following $n.", TRUE, ch,
					    0, ch->ask_for_follow, TO_NOTVICT);
				}
			} else {
				sprintf(log_buf,
					"Sorry, %s won't allow you to follow!\n\r",
					GET_NAME(ch));
				send_to_char(log_buf, ch->ask_for_follow);
				act("Ok, $N can't follow now.", TRUE, ch, 0,
				    ch->ask_for_follow, TO_CHAR);
			}
		}
		ch->ask_for_follow = NULL;
		ch->p->querycommand = 0;
	...
	if (leader == ch) {
		...
		if (ch->master == ch && is_formed(ch)) {
			send_to_char
			    ("You are following yourself. If you want to remove everyone from \n\ryour formation, type unform YOURNAME\n\r",
			     ch);
			return;
		}
		remove_from_formation(ch);
	...
	if (ch->master != ch) {
		send_to_char
		    ("You must follow yourself before following someone else.\n\r",
		     ch);
		return;
	}
	...
	if (leader->master != leader) {
		send_to_char
		    ("That person is not a leader(following someone else).\n\r",
		     ch);
		return;
	}
	if (is_formed(ch)) {
	...
	sprintf(leader->p->queryprompt, "Can %s follow you? (y/n)> ",
		GET_NAME(ch));
	...
	for (x = 0; x < 3; x++) {
		for (y = 0; y < 3; y++) {
			if (ch->master->formation[x][y]) {
				...
				sprintf(buf,
					"[%-15s][%s][lv %2d| %4d/%4dhp %4d/%4dm %4d/%4dmv]\r\n",
					GET_NAME(ch->master->formation[x][y]),
					class,
					...
					GET_MAX_MANA(ch->master->
						     formation[x][y]),
					GET_MOVE(ch->master->formation[x][y]),
					GET_MAX_MOVE(ch->master->
						     formation[x][y]));
				send_to_char(buf, ch);
