...
extern put_obj_in_store(struct obj_data *obj, struct char_data *ch,
			FILE * fpsave);
...
void do_push(struct char_data *ch, char *argument, int cmd)
{
	...
	if (cmd == 1000) {
		...
		if (ch->specials.eggs < 1000) {
			send_to_char
			    ("You must help fight the WAR!  Vryce will make you reborn into a new player only if you have collected 1000 eggs from the catacombs.\n\r",
			     ch);
			ch->p->querycommand = 0;
			return;
		...
		case 'm':
			if (GET_CLASS(ch) == CLASS_MAGIC_USER ||
			    IS_SET(ch->player.multi_class,
				   MULTI_CLASS_MAGIC_USER)) {
		...
		send_to_room
		    ("The room is filled with a blinding light for 3 full breaths...you feel yourself flung as if in a hurricane...your sense of balance fails you..  you're falling...THWOOP!  You blink to clear your eyes.  ",
		     ch->in_room);
		if (GET_CLASS(ch) == CLASS_THIEF) {
	...
	if (get_total_level(ch) >= 124)
		return;
	send_to_room
	    ("A powerful voice roars across the mountain...  You ... are ... *NOT* ... <Ready> ... for godhood!  You are not wise enough, not OLD enough!  Not complete!  Go back.. Go back...... Go back as a newborn!  Learn what there is to be learned from the mortal realm...learn all things great and small...learn all skills..learn all classes..be everything..  Come back to me, Mortal when you are Ready!  ",
	     6151);
	strcpy(ch->p->queryprompt,
	       "Pick a class you have not mastered yet.\n([M]age [C]leric [W]arrior [T]hief) or ENTER to abort> ");
...
int horneg_sarcophogus_room(struct char_data *ch, int cmd, char *arg)
{
	...
	if (cmd == 222 && (isname("sarcophagus", arg)
			   || isname("large", arg)
			   || isname("stone", arg)
	    )
	    ) {
		sarcophagus =
		    get_obj_in_list("sarcophagus",
				    world[ch->in_room]->contents);
		if (!sarcophagus)
			return (FALSE);
		else
			corpse =
			    get_obj_in_list("corpse", sarcophagus->contains);
		if (!corpse)
			return (FALSE);
		...
		if (!mob) {
			...
			send_to_char
			    ("A large veil of mist swirls up from the sarcophogus and shifts into the form of a beautiful young lady!\n\r",
			     ch);
			...
			do_say(mob,
			       "You have defiled my sanctuary, now You Must Die!",
			       0);
			do_close(mob, "wall", 0);
			hit(mob, ch, 0);
			return (FALSE);
...
int convention(struct char_data *ch, int cmd, char *arg)
{
	...
	send_to_char
	    ("You wake up feeling groggy, remembering what must have been a dream...\n\r",
	     ch);
	act("$n steps into the portal and vanishes.\n\r", TRUE, ch, 0, 0,
	    TO_ROOM);
	if (world[ch->in_room]->number == 25)
		room = 204;
	else
		room = 25;
	char_from_room(ch);
...
void load_dragon_crystal(void)
{
	...
	while (tries < 1500) {
		...
		if (world[random] && world[random]->zone < 196)
			for (mob = world[random]->people; mob;
...
int mythago_wood(struct char_data *ch, int cmd, char *arg)
{
	...
	send_to_char("You pass out.\n\r", ch);
	send_to_char
	    ("As you wake up, feeling groggy, you remember what must have been a dream..\n\rAbout being in the spooky wood...NNNaaaa.\n\r",
	     ch);
...
int swamp_bog(struct char_data *ch, int cmd, char *arg)
{
	...
	if (IS_AFFECTED(ch, AFF_FLYING)) {
		send_to_char
		    ("You FLY just above the bog so you don't get stuck!\n\r",
		     ch);
		return (FALSE);
	}
	if (number(0, n) < 100) {
		send_to_char
		    ("You try to free your feet from the bog mud but fail.\n\r",
		     ch);
		send_to_char("Perhaps you are TOO HEAVY! (hint)\n\r", ch);
		act("$n tries to move but is stuck in the bog mud.", TRUE, ch,
		    0, 0, TO_ROOM);
		return (TRUE);
	}
	return (FALSE);
...
int river_tree(struct char_data *ch, int cmd, char *arg)
{
	...
	send_to_char
	    ("The Old Man Willow tree GRABS YOU, in a steel-like grip!\n\r",
	     ch);
	send_to_char
	    ("As you struggle to break free, it pulls you down its gaping throat.\n\r",
	     ch);
	send_to_char
	    ("You are mysteriously transported into the middle of a spooky wood!\n\r\n\r",
	     ch);

	act("The Old Man Willow tree GRABS $n, in a steel like grip!", TRUE, ch,
	    0, 0, TO_ROOM);
...
int tear_fountain(struct char_data *ch, int cmd, char *arg)
{
	...
	if (ch->specials.practices > 9999) {
		...
		a = number(20, 75);
		sprintf(log_buf, "As a Reward I give you %d Practice Points!",
			a);
		act(log_buf, TRUE, ch, 0, 0, TO_ROOM);
		act(log_buf, TRUE, ch, 0, 0, TO_CHAR);
		do_gossip(ch,
			  "I FOUND THE DRAGON CRYSTAL AND HAVE RECEIVED MY REWARD!",
			  0);
...
*/
{
	...
	if ((GET_LEVEL(ch) >= 31))
		return (FALSE);
	send_to_char
	    ("Your limp body hangs impaled on the stalagmite, bleeding and  weezing.\n\r",
	     ch);
...
int river_current(struct char_data *ch, int cmd, char *arg)
{
	...
	for (iWear = 0; iWear < MAX_WEAR; iWear++)
		if (ch->equipment[iWear])
			if (ch->equipment[iWear]->obj_flags.type_flag ==
			    ITEM_FLY)
				return (0);
	if (number(0, 10) < 10)
		return (0);
	...
	if (room == 734)
		...
		toroom = 749;
	else if ((room >= 749) && (room < 758))
		toroom = room + 1;
	else if (room == 758)
		toroom = 764;
	else if ((room >= 764) && (room <= 773))
		toroom = room + 1;
...
int guild(struct char_data *ch, int cmd, char *arg)
{
	...
	case CLASS_MAGIC_USER:{
			if (!*arg) {
				...
				sprintf(prac_buf,
					"%sGuild practicing now costs from 1 to 3 (die roll) practice points.\n\r",
					prac_buf);
				sprintf(prac_buf,
					"%sYou can practice any of these spells:\n\r",
					prac_buf);
			...
			number =
			    old_search_block(arg, 0, strlen(arg), spells,
					     FALSE);
			if (number == -1) {
				send_to_char
				    ("You do not know of this spell...\n\r",
				     ch);
				return (TRUE);
			}
			...
			percent =
			    ch->skills[number].learned + MAX(25,
							     int_app[GET_INT
								     (ch)].
							     learn);
	...
	case CLASS_THIEF:{
			if (!*arg) {
				sprintf(prac_buf,
					...
					ch->specials.practices);
				sprintf(prac_buf,
					"%sYou can practice any of these skills:\n\r",
					prac_buf);
				for (i = 0; *t_skills[i] != '\n'; i++) {
					sprintf(prac_buf,
						"%s%s[%s%20s%s][%s%14s%s]\n\r",
						prac_buf, BLU(ch), NRM(ch),
						t_skills[i], BLU(ch), NRM(ch),
						how_good(ch->skills[i + 45].
							 learned), BLU(ch));
				}
				for (i = 0; *nt_skills[i] != '\n'; i++) {
					sprintf(prac_buf,
			...
			if (nnumber == -1 && mednumber == -1) {
				if (ch->skills[number + SKILL_SNEAK].learned >=
				    90) {
					send_to_char
					    ("You are already learned in this area.\n\r",
					     ch);
				...
				send_to_char("You Practice for a while...\n\r",
					     ch);
				percent =
				    ch->skills[number + SKILL_SNEAK].learned +
				...
				if (ch->skills[nnumber + SKILL_TRIP].learned >=
				    90) {
					send_to_char
					    ("You are already learned in this area.\n\r",
					     ch);
				...
				send_to_char("You Practice for a while...\n\r",
					     ch);
				percent =
				    ch->skills[nnumber + SKILL_TRIP].learned +
				    MIN(int_app[GET_INT(ch)].learn, 12);
				ch->skills[nnumber + SKILL_TRIP].learned =
				    MIN(90, percent);
				if (ch->skills[nnumber + SKILL_TRIP].learned >=
				...
				if (ch->skills[mednumber + SKILL_THROW].
				    learned >= 90) {
					send_to_char
					    ("You are already learned in this area.\n\r",
					     ch);
				...
				percent =
				    ch->skills[mednumber +
					       SKILL_THROW].learned +
				    MIN(int_app[GET_INT(ch)].learn, 12);
				ch->skills[mednumber + SKILL_THROW].learned =
				    MIN(90, percent);
				if (ch->skills[mednumber + SKILL_THROW].
				    learned >= 90) {
					send_to_char
	...
	case CLASS_WARRIOR:{
			...
			if ((number == -1)
			    && (nnumber == -1 && (mednumber == -1))) {
				send_to_char
				    ("You do not have ability to practice this skill!\n\r",
				     ch);
				return (TRUE);
			}
			if (ch->specials.practices <= 0) {
			...
			if (nnumber == -1 && mednumber == -1) {
				if (ch->skills[number + SKILL_KICK].learned >=
				    85) {
					send_to_char
					    ("You are already learned in this area.\n\r",
					     ch);
				...
				percent =
				    ch->skills[number + SKILL_KICK].learned +
				...
				if (ch->skills[nnumber + SKILL_SECOND_ATTACK].
				    learned >= 85) {
					send_to_char
					    ("You are already learned in this area.\n\r",
					     ch);
				...
				if (ch->skills[mednumber + SKILL_CHARGE].
				    learned >= 85) {
					send_to_char
					    ("You are already learned in this area.\n\r",
					     ch);
				...
				percent =
				    ch->skills[mednumber +
					       SKILL_CHARGE].learned + MIN(12,
									   int_app
									   [GET_INT
									    (ch)].
									   learn);
				ch->skills[mednumber + SKILL_CHARGE].learned =
				    MIN(85, percent);
				if (ch->skills[mednumber + SKILL_CHARGE].
				    learned >= 85) {
					send_to_char
...
int train(struct char_data *ch, int cmd, char *arg)
{
	...
	if (*arg == '\0') {
		...
		send_to_char(buf, ch);
		send_to_char
		    ("Training costs you 35 Practice points except for..\n\r",
		     ch);
		send_to_char
		    ("The following costs 20 practice points for classes.\n\r",
		     ch);
		send_to_char
		    ("WARRIOR str sta, MAGE int, CLERIC wis, THIEF dex sta.\n\r",
		     ch);
		arg = "foo";
	}
		 ...
		 int *cur_line, void **thing)
{
	...
	case 'o':		/* Find object in room */
		*thing =
		    get_obj_in_list_vis(npc, cmd + 1,
					world[npc->in_room]->contents);
		ok = (*thing != 0);
		break;
...
int snake(struct char_data *ch, int cmd, char *arg)
{
	...
	if (ch->specials.fighting &&
		...
		act("$n bites $N!", 1, ch, 0, ch->specials.fighting,
		    TO_NOTVICT);
...
int thief(struct char_data *ch, int cmd, char *arg)
{
	...
	for (cons = world[ch->in_room]->people; cons; cons = cons->next_in_room)
		if ((!IS_NPC(cons)) && (GET_LEVEL(cons) < 32)
		    && (number(1, 5) == 1))
			npc_steal(ch, cons);
...
int red_dragon(struct char_data *ch, int cmd, char *arg)
{
	...
	for (vict = world[ch->in_room]->people; vict; vict = vict->next_in_room)
		if (vict->specials.fighting == ch) {
			act("$n breathes a cone of fire, enveloping you.", 1,
			    ch, 0, 0, TO_VICT);
...
int block_way_suites(struct char_data *ch, int cmd, char *arg)
{
	...
	if (cmd != 1 && cmd != 2 && cmd != 3 && cmd != 4 && cmd != 5
		...
		return FALSE;
	room = ch->in_room;
	if (!world[room]->dir_option[cmd - 1])
		return FALSE;
	...
	if (world[to_room]->level_restriction == 1) {
		...
		act("That suite is taken, sorry.", FALSE, ch, 0, k, TO_CHAR);
		return TRUE;
	}
	return FALSE;
...
int guild_guard(struct char_data *ch, int cmd, char *arg)
{
	...
	if ((GET_CLASS(ch) != CLASS_MAGIC_USER
	    ...
	    || (GET_CLASS(ch) != CLASS_THIEF && ch->in_room == 3027 && cmd == 2)
...
int puff(struct char_data *ch, int cmd, char *arg)
{
	...
	case 8:
		do_say(ch,
		       "Any of you guys seen any cute female pups around here?",
		       0);
		return (1);
	case 9:
...
int prisoner(struct char_data *ch, int cmd, char *arg)
{
	...
	switch (number(0, 60)) {
	case 5:
		do_say(ch, "Please take my life and free me from this hell!",
		       0);
		return (1);
	case 2:
		do_emote(ch,
			 "struggles to get free but only causes herself more pain.",
			 0);
		return (1);
	case 8:
	...
	case 9:
		do_emote(ch,
			 "tries to pick the lock on the shackles and gets zapped",
			 0);
		return (1);
	case 10:
	...
	case 11:
		...
		return (1);
	case 12:
		do_say(ch, "If I could only kill Gith for what he has done!",
		       0);
		return (1);
	case 13:
...
int towncrier(struct char_data *ch, int cmd, char *arg)
{
	...
	if (GET_POS(ch) == POSITION_FIGHTING) {
		if (number(0, 10))
			do_say(ch, "I declare that I die entirely too often!!",
			       0);
		return (1);
	}
	...
	case 1:
		sprintf(buf,
			"I declare that Medievia now has %d rooms and %d zones!",
			number_of_rooms + 4000000, 1 + number_of_zones);
		do_say(ch, buf, 0);
		return (1);
	case 2:
		do_say(ch,
		       "Hear Ye! Hear Ye!  Be it known that on the first day of\n\rthe fourth month in the year of the Dragon, nineteen hundred and ninety three \n\r Medievia declared itself a sovereign land!",
		       0);
		return (1);
	case 3:
		sprintf(buf,
			"Let it be proclaimed throughout the land that there\n\rare now %d fair citizens in Medievia",
			number_of_players());
		do_say(ch, buf, 0);
		return (1);
	case 4:
		sprintf(buf,
			"Hear Ye!  Hear Ye!  Be it known that there have been %ld passersby in Medievia since the 11th month of the year of the Dragon, nineteen hundred and ninety three",
			connect_count);
		do_say(ch, buf, 0);
		return (1);
	case 5:
	...
	for (i = world[ch->in_room]->contents; i; i = i->next_content) {
		if (IS_SET(i->obj_flags.wear_flags, ITEM_TAKE) &&
		    ((i->obj_flags.type_flag == ITEM_DRINKCON) ||
		     (i->obj_flags.cost <= 10))) {
			act("$n picks up some trash.", FALSE, ch, 0, 0,
			    TO_ROOM);
...
int pet_shops(struct char_data *ch, int cmd, char *arg)
{
	...
	if (cmd == 59) {	/* List */
		...
		for (pet = world[pet_room]->people; pet;
		...
		if (*pet_name) {
			...
			pet->player.description =
			    my_free(pet->player.description);
...
int atm_machines_helper(struct char_data *ch, int cmd, char *arg,
			char *filename)
{
	...
	if (cmd == 102) {	/* balance */
		...
		sprintf(log_buf,
			"[%d]-Withdrawn, leaving a balance of [%d].\n\r",
			withdrawal, balance);
		send_to_char(log_buf, ch);
		sprintf(log_buf, "%s withdrawals %d, for a balance of %d.",
			GET_NAME(ch), withdrawal, balance);
		log_hd(log_buf);
		...
		if (GET_LEVEL(ch) < 2) {
			send_to_char
			    ("You must be level 2+ to use our services.\n\r",
			     ch);
			return (TRUE);
		}
		...
		if (deposit > GET_GOLD(ch)) {
			send_to_char
			    ("You realize you don't have that many coins.\n\r",
			     ch);
			return (TRUE);
		}
		if ((balance + deposit) > 1000000000) {
			send_to_char
			    ("SORRY 1,000,000,000 GOLD TOTAL is the limit\n\r",
			     ch);
			return (TRUE);
		}
		GET_GOLD(ch) -= deposit;
		...
		sprintf(log_buf,
			"Transaction completed for a charge of [%d] coins.\n\r",
			(int)(deposit * .036));
		send_to_char(log_buf, ch);
		sprintf(log_buf,
			"[%d]-Deposited, leaving a balance of [%d].\n\r",
			deposit, balance);
		send_to_char(log_buf, ch);
		sprintf(log_buf, "%s deposits %d, for a balance of %d.",
			GET_NAME(ch), deposit, balance);
		log_hd(log_buf);
...
int thief_guild(struct char_data *ch, int cmd, char *arg)
{
	...
	if ((GET_CLASS(ch) != CLASS_THIEF)
		...
		send_to_char
		    ("You watch the lesson but can't comprehend a thing.\n\r",
		     ch);
		return (TRUE);
	}
	...
	if (!*arg) {
		...
		for (i = 0; *nt_skills[i] != '\n'; i++) {
			...
			send_to_char(how_good
				     (ch->skills[i + SKILL_TRIP].learned), ch);
			send_to_char("\n\r", ch);
		}
		for (i = 0; *medt_skills[i] != '\n'; i++) {
			send_to_char(medt_skills[i], ch);
			send_to_char(how_good
				     (ch->skills[i + SKILL_THROW].learned), ch);
			send_to_char("\n\r", ch);
		}
		return (TRUE);
	}
...
int mage_guild(struct char_data *ch, int cmd, char *arg)
{
	...
	if ((GET_CLASS(ch) != CLASS_MAGIC_USER)
		...
		send_to_char
		    ("You could watch the lesson but wouldn't comprehend a thing.\n\r",
		     ch);
		return (TRUE);
	}
	...
	if (!*arg) {
		...
		send_to_char(buf, ch);
		send_to_char
		    ("Guild Practicing between 1 and 3(Die Roll) practice points,\n\r",
		     ch);
...
int warrior_guild(struct char_data *ch, int cmd, char *arg)
{
	...
	if ((GET_CLASS(ch) != CLASS_WARRIOR)
		...
		send_to_char
		    ("You watch the lesson but don't comprehend a thing.\n\r",
		     ch);
		return (TRUE);
	}
	...
	if (!*arg) {
		...
		for (i = 0; *w_skills[i] != '\n'; i++) {
			...
			send_to_char(how_good
				     (ch->skills[i + SKILL_KICK].learned), ch);
			send_to_char("\n\r", ch);
		}
		for (i = 0; *nw_skills[i] != '\n'; i++) {
			send_to_char(nw_skills[i], ch);
			send_to_char(how_good
				     (ch->skills[i + SKILL_SECOND_ATTACK].
				      learned), ch);
			send_to_char("\n\r", ch);
		}
		for (i = 0; *medw_skills[i] != '\n'; i++) {
			...
			send_to_char(how_good
				     (ch->skills[i + SKILL_CHARGE].learned),
				     ch);
			send_to_char("\n\r", ch);
		}
		return (TRUE);
	...
	if (nnumber == -1 && mednumber == -1) {
		...
		percent = ch->skills[nnumber + SKILL_SECOND_ATTACK].learned +
		    MIN(12, int_app[GET_INT(ch)].learn);
		ch->skills[nnumber + SKILL_SECOND_ATTACK].learned =
		    MIN(85, percent);
		if (ch->skills[nnumber + SKILL_SECOND_ATTACK].learned >= 85) {
			send_to_char("You are now learned in this area.\n\r",
				     ch);
 ...
 */
	...
	global_color = 32;
	send_to_char
	    ("You lose your balance and slide down a steep ravine towards the river.\n\r",
	     ch);
	global_color = 0;

	if (!IS_NPC(ch) && (GET_LEVEL(ch) > 31)) {
		act("$n dives gracefully into the river from the slope above.",
		    FALSE, ch, NULL, NULL, TO_ROOM);
		global_color = 34;
		send_to_char
		    ("You slide down the slope but regain control, diving gracefully into the river.\n\r",
		     ch);
		global_color = 0;
		return;
	}
	...
	while (GET_HIT(ch) > 0) {
		if (number(1, 100) < ((GET_DEX(ch) - 12) * 4)) {
			...
			global_color = 34;
			sprintf(log_buf,
				"Nimbly regaining control, you flip %d times in the air, and dive into the river.\n\r",
				spin);
			send_to_char(log_buf, ch);
			sprintf(log_buf,
				"$n flips %d times in the air, diving gracefully into the river.\n\r",
				spin);
	...
	global_color = 31;
	send_to_char
	    ("You limply bounce off a rock and splash into the river.\n\r", ch);
	act("$n's limp body bounces off a rock and splashes into the river.",
	    FALSE, ch, NULL, NULL, TO_ROOM);
	global_color = 0;
	sprintf(log_buf, "<%dhp %dm %dmv>\n\r",
		GET_HIT(ch), GET_MANA(ch), GET_MOVE(ch));
...
int moongate(struct char_data *ch, int cmd, char *arg)
{
	...
	send_to_char
	    ("Your vision blackens for a moment. When it returns, you find yourself\n\r",
	     ch);
	send_to_char
	    ("drifting through an endless gray mist.  A bright ball of light in the\n\r",
	     ch);
	send_to_char
	    ("distance rushes towards you, engulfing your body, blinding you.\n\r",
	     ch);
	send_to_char
	    ("Your vision returns, and you find yourself back on the material plane.\n\r",
	     ch);
...
int illusionary_room(struct char_data *ch, int cmd, char *arg)
{
	if ((world[ch->in_room]->number == 6324) && (cmd == 2)) {
		...
		do_look(ch, "", 9);
		send_to_char
		    ("Just as your foot is about to touch the floor, you notice\n\r",
		     ch);
		send_to_char
		    ("everything in the room is slightly transparent. In utter shock,\n\r",
		     ch);
		send_to_char
		    ("you pass straight through the floor and fall into darkness...\n\r",
		     ch);
		send_to_char
		    ("With a loud *SMACK*, you land in frigid, black waters.\n\r",
		     ch);
		...
		if (GET_HIT(ch) < 1) {
			send_to_char
			    ("You land in a pool of frigid water. A stalagmite hidden just beneath the surface\n\r",
			     ch);
			send_to_char
			    ("of the lake pierces your eye, impaling your brain and shatters your skull.\n\r",
			     ch);
			raw_kill(ch, 0);
			return (TRUE);
		} else {
...
int pit_traps(struct char_data *ch, int cmd, char *arg)
{
	if (world[ch->in_room]->number == 15067 && !IS_NPC(ch)) {
		if (number(1, 20) > GET_DEX(ch)) {
			...
			char_from_room(ch);
			send_to_char
			    ("You slip on the oil, loosing your footing...\n\r",
			     ch);
			send_to_char
			    ("Darkness engulfs you as you fall downward through a rough hewn tunnel.\n\r",
			     ch);
			char_to_room(ch, 1159);
			GET_HIT(ch) -= dice(3, 10);
			if (GET_HIT(ch) < 1) {
				send_to_char
				    ("You land hard on the cold stone floor, snapping your neck on a stalagmite.\n\r",
				     ch);
				raw_kill(ch, 0);
				...
				send_to_char
				    ("You land painfully on the stone floor, bruised and cut.\n\r",
				     ch);
				act("$n falls from a dark hole in the ceiling, landing hard on the cavern floor.\n\r", TRUE, ch, 0, 0, TO_ROOM);
				do_look(ch, "", 9);
				return (TRUE);
		...
		if (number(1, 20) > GET_DEX(ch)) {
			...
			if (GET_HIT(ch) < 1) {
				send_to_char
				    ("You feel your legs exit the shoot and you begin falling.  A jagged, protruding\n\r",
				     ch);
				send_to_char
				    ("rock shatters your face and skull just before your head exits the tunnel.\n\r",
				     ch);
				raw_kill(ch, 0);
				...
				send_to_char
				    ("You slam into the walls of the shoot painfully before falling out the other end,\n\r",
				     ch);
				send_to_char
				    ("landing in a pool of icy waters.\n\r",
				     ch);
				act("$n slides out of a dark hole in the cavern wall, landing in the cold water.\n\r", TRUE, ch, 0, 0, TO_ROOM);
				do_look(ch, "", 9);
				return (TRUE);
		...
		if (number(1, 500) < IS_CARRYING_W(ch)) {
			...
			char_from_room(ch);
			send_to_char
			    ("The floor under your feet crumbles and falls away...\n\r",
			     ch);
			send_to_char
			    ("Deserately you grab for something to hold onto, but it's too late.\n\r",
			     ch);
			send_to_char
			    ("You plummet into the blackness of the pit, splattering powerfully on the\n\r",
			     ch);
...
int silver_shrine(struct char_data *ch, int cmd, char *arg)
{
	...
	send_to_char
	    ("You float through a silvery, misty void for a few moments.\n\r",
	     ch);
	send_to_char
	    ("The mist clears and you find yourself in a forest clearing.\n\r",
	     ch);
	act("$n steps into the portal and vanishes.\n\r", TRUE, ch, 0, 0,
	    TO_ROOM);
	if (world[ch->in_room]->number == 813)
		room = 3596;
	else
		room = 813;
	char_from_room(ch);
...
int waterfall(struct char_data *ch, int cmd, char *arg)
{
	if ((world[ch->in_room]->number == 774) && (cmd == 6)) {
		...
		act("helpless screams of terror as $e nears the bottom.\n\r",
		    TRUE, ch, 0, 0, TO_ROOM);
		send_to_char
		    ("You are thrown powerfully over the waterfall's edge and fall helplessly.\n\r",
		     ch);
		char_from_room(ch);
		char_to_room(ch, 775);
		if (GET_LEVEL(ch) > 31) {
			send_to_char
			    ("You float harmlessly to the ground below.\n\r",
			     ch);
			do_look(ch, "", 9);
			...
			send_to_char
			    ("You land on an unforgiving rock below, your body shatters from the impact.\n\r",
			     ch);
			...
			if (GET_HIT(ch) < 0) {
				...
				send_to_char
				    ("You land painfully in the water and are forced under by the powerful current.\n\r",
				     ch);
				send_to_char
				    ("You slam against the river floor and quickly resurface, gasping for air.\n\r",
				     ch);
				do_look(ch, "", 9);
				return (TRUE);
			}
