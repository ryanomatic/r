...
void do_pigeon(struct char_data *stpCh, char *szpArg, int iCmd)
{
 ...
 one_argument(szpArg, Buf2);
	...
	act("\rA billowing cloud of purple smoke seeps from the ground and a small imp\rappears. It snatches a message from $n's hand and disappears with the\rfading smoke.",FALSE,stpCh,0,0,TO_ROOM);
	global_color=0;
	return;
}
...
void deliver_message(struct pigeon_message_data *stpMes, struct char_data *stpCh)
{
	...
	global_color=36;
	act("\rA billowing cloud of purple smoke seeps up through the ground.  A small\rimp appears and give a rolled piece of parchment to $n.  It then vanishes\rwith the fading cloud of smoke.",FALSE,stpCh,0,0,TO_ROOM);
