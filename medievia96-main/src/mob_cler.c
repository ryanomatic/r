...
extern void cast_fireshield(byte level, struct char_data *ch, char *arg, int si,
			    struct char_data *tar_ch, struct obj_data *tar_obj);
extern void cast_cause_serious(byte level, struct char_data *ch, char *arg,
			       int si, struct char_data *tar_ch,
			       struct obj_data *tar_obj);
extern void cast_fly(byte level, struct char_data *ch, char *arg, int si,
		     struct char_data *tar_ch, struct obj_data *tar_obj);
extern void cast_detect_invisibility(byte level, struct char_data *ch,
				     char *arg, int si,
				     struct char_data *tar_ch,
				     struct obj_data *tar_obj);
extern void cast_invisibility(byte level, struct char_data *ch, char *arg,
			      int si, struct char_data *tar_ch,
			      struct obj_data *tar_obj);
...
extern void cast_armor(byte level, struct char_data *ch, char *arg, int si,
		       struct char_data *tar_ch, struct obj_data *tar_obj);
extern void cast_demonfire(byte level, struct char_data *ch, char *arg, int si,
			   struct char_data *tar_ch, struct obj_data *tar_obj);
extern void cast_hammer_of_faith(byte level, struct char_data *ch, char *arg,
				 int si, struct char_data *tar_ch,
				 struct obj_data *tar_obj);
extern void cast_mass_levitation(byte level, struct char_data *ch, char *arg,
				 int si, struct char_data *tar_ch,
				 struct obj_data *tar_obj);
extern void cast_mass_invis(byte level, struct char_data *ch, char *arg, int si,
			    struct char_data *tar_ch, struct obj_data *tar_obj);
extern void cast_flamestrike(byte level, struct char_data *ch, char *arg,
			     int si, struct char_data *tar_ch,
			     struct obj_data *tar_obj);
extern void cast_dispel_evil(byte level, struct char_data *ch, char *arg,
			     int si, struct char_data *tar_ch,
			     struct obj_data *tar_obj);
extern void cast_call_lightning(byte level, struct char_data *ch, char *arg,
				int si, struct char_data *tar_ch,
				struct obj_data *tar_obj);
extern void cast_cure_critic(byte level, struct char_data *ch, char *arg,
			     int si, struct char_data *tar_ch,
			     struct obj_data *tar_obj);
extern void cast_magic_missile(byte level, struct char_data *ch, char *arg,
			       int si, struct char_data *tar_ch,
			       struct obj_data *tar_obj);
extern void cast_cont_light(byte level, struct char_data *ch, char *arg, int si,
			    struct char_data *tar_ch, struct obj_data *tar_obj);
extern struct char_data *pick_victim(struct char_data *ch);
extern void cast_dispel_magic(byte level, struct char_data *ch, char *arg,
			      int si, struct char_data *tar_ch,
			      struct obj_data *tar_obj);
extern void cast_heal(byte level, struct char_data *ch, char *arg, int si,
		      struct char_data *tar_ch, struct obj_data *tar_obj);
extern void cast_bless(byte, struct char_data *, char *, int,
...
void cleric_combat(struct char_data *ch)
{
	...
	if (GET_LEVEL(ch) >= 16 && GET_MANA(ch) >= 15) {
		...
		if (!m)
			return;
		if (GET_LEVEL(ch) >= GET_LEVEL(m) && (IS_AFFECTED(m, AFF_SANCTUARY) || IS_AFFECTED(m, AFF_FIRESHIELD))) {
	...
	if (GET_LEVEL(ch) >= 14 && GET_MANA(ch) > 50 && ((GET_HIT(ch) < (.3 * GET_MAX_HIT(ch))))) {
	...
	if (GET_LEVEL(ch) >= 25 && GET_MANA(ch) >= 31) {
		...
		if (!m)
			return;
		if ((GET_ALIGNMENT(ch) > 349 && GET_ALIGNMENT(m) < 349) || (GET_ALIGNMENT(ch) < (-349) && GET_ALIGNMENT(m) > 349) || ((GET_ALIGNMENT(ch) > (-350) && GET_ALIGNMENT(ch) < 350) && (GET_ALIGNMENT(m) > 349 || GET_ALIGNMENT(m) < (-349)))) {
			act("$n chants the magical phrase, 'In Vas Grav'", TRUE, ch, 0, 0, TO_ROOM);
			GET_MANA(ch) -= 31;
			if (number(1, 100) > (80 + GET_WIS(ch)))
				return;
	...
	if (GET_LEVEL(ch) >= 20 && GET_MANA(ch) >= 20 && OUTSIDE(ch) && (weather_info.sky >= SKY_RAINING)) {
		m = pick_victim(ch->specials.fighting);
		if (!m)
			return;
	...
	if (GET_LEVEL(ch) >= 12 && GET_MANA(ch) >= 20 && OUTSIDE(ch) && (weather_info.sky >= SKY_RAINING)) {
		m = pick_victim(ch->specials.fighting);
		if (!m)
			return;
		act("$n chants the magical phrase, 'Kal Jux Grav'", TRUE, ch, 0, 0, TO_ROOM);
