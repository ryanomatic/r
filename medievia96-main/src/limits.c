...
{
	FREE(GET_TITLE(ch));
	GET_TITLE(ch) = str_dup(title_table[GET_CLASS(ch) - 1]
				[(int)GET_LEVEL(ch)][GET_SEX(ch) ==
						     SEX_FEMALE]);
	if (!ch->player.title) {
		FREE(GET_TITLE(ch));
		GET_TITLE(ch) = str_dup("NOTITLE");
	}
	return;
}
...
void gain_exp(CHAR_DATA * ch, int gain, CHAR_DATA * victim)
{
	...
	if (!IS_NPC(ch)
	    && victim && IS_SET(world[ch->in_room]->room_flags, NEUTRAL)
	    && (gain > 0) && !IS_NPC(victim)) {
		global_color = 31;
	...
	if (!IS_NPC(ch)
	    && victim && !IS_SET(world[ch->in_room]->room_flags, CHAOTIC)
	    && (gain > 0) && !IS_NPC(victim)) {
		global_color = 31;
		send_to_char("Experience gain canceled (LAWFUL AREA)n\r", ch);
		sprintf(my_buf, "##[%s] killed [%s] IN A LAWFUL ZONE!",
	...
	for (d_room = daroom_list; d_room; d_room = d_room->next) {
		...
		case FIRE:
			if (!IS_SET(world[d_room->room_num]->room_flags, FIRE))
				break;
			for (i = world[d_room->room_num]->people; i;
			     i = i->next_in_room) {
				if (!IS_NPC(i) && GET_LEVEL(i) < 32
				    && !IS_HOVERING(i)) {
					GET_HIT(i) -=
					    number(1, d_room->damage_amt);
					global_color = 31;
					send_to_char
					...
					if (GET_HIT(i) < 0) {
						global_color = 31;
						send_to_char
						    ("Your body shrivels up in the heat of the fire, You are DEAD!\r\n",
						     i);
			...
			for (i = world[d_room->room_num]->people; i;
				...
				if (!IS_NPC(i) && GET_LEVEL(i) < 32
					...
					if (GET_HIT(i) < 0) {
						global_color = 31;
						send_to_char
						    ("The cold finnaly gets to you, You are DEAD!\r\n",
						     i);
			...
			for (i = world[d_room->room_num]->people; i;
				...
				if (!IS_NPC(i) && GET_LEVEL(i) < 32
					...
					if (GET_HIT(i) < 0) {
						global_color = 31;
						send_to_char
						    ("Your lungs finnaly give out from breathing the fumes, You are DEAD!\r\n",
						     i);
...
*/
		...
		if (!IS_NPC(i) && i->in_room > 0 && (GET_LEVEL(i) < 32)
		    && world[i->in_room]->sector_type == SECT_UNDERWATER
		    && !IS_HOVERING(i) && !IS_AFFECTED(i, AFF_BREATH_WATER)) {
...
void decay_corpses(void)
{
	...
	extern bool in_container(struct obj_data *obj,
	...
	while (obj) {
		...
		if ((GET_ITEM_TYPE(obj) == ITEM_REGEN)
		    && (time_info.hours == 0)) {
			...
			if (!obj->obj_flags.timer) {
				...
				if (corpse->carried_by)
					act("$p decays in your hands.",
					    FALSE, corpse->carried_by, corpse,
					    0, TO_CHAR);
				else if ((corpse->in_room != NOWHERE)
					 && (world[corpse->in_room]->people))
				{
					act("A quivering horde of maggots consumes $p.", TRUE, world[corpse->in_room]->people, corpse, 0, TO_ROOM);
					act("A quivering horde of maggots consumes $p.", TRUE, world[corpse->in_room]->people, corpse, 0, TO_CHAR);
				}
