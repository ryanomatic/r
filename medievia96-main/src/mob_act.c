		...
		if (EXIT(ch, door)) {
			if (EXIT(ch, door)->to_room != NOWHERE
				...
				if (EXIT(ch, door)->to_room ==
				    ch->specials.was_in_room_pk)
					last = door;
				for (rap =
				     world[EXIT(ch, door)->to_room]->room_afs;
				     rap; rap = rap->next)
...
void auto_group_mob(struct char_data *ch)
{
	...
	for (m = world[ch->in_room]->people; m; m = next_m) {
		...
		if (m == ch)
			continue;
		if (m->specials.fighting)
			continue;
		if (m->master != m)
			continue;
		if (GET_CLASS(m) == CLASS_OTHER && m->nr != ch->nr)
			continue;
		if (GET_CLASS(ch) == CLASS_OTHER && m->nr != ch->nr)
			continue;
		if (!AWAKE(m))
			continue;
		if (GET_CLASS(ch) != CLASS_OTHER) {
			if (GET_ALIGNMENT(ch) > (GET_ALIGNMENT(m) + 250))
				continue;
			if (GET_ALIGNMENT(ch) < (GET_ALIGNMENT(m) - 250))
				continue;
		}
		...
		if (m->nr == 9801)
			continue;
		if (IS_SET(m->specials.act, ACT_SPEC))
			continue;
		if (IS_SET(m->specials.act, ACT_SENTINEL)
		...
		if (put_in_formation(m, ch)) {
			...
			act("$N starts following you.", TRUE, m, 0, ch,
			    TO_CHAR);
			act("$N starts following $n.", TRUE, m, 0, ch,
			    TO_NOTVICT);
			return;
		}
	}
...
void mob_non_combat_inteligence(void)
{
	...
	for (m = character_list; m; m = next_m) {
		...
		if (m->nr == 9800)
			continue;
		if (is_trapped(m)) {
			act("$n struggles to get out of the trap.", TRUE, m, 0,
			    0, TO_ROOM);
			continue;
		}
		if (GET_POS(m) != POSITION_STANDING)
			continue;
...
void mobile_activity(void)
{
	...
	for (ch = character_list; ch; ch = next_ch) {
		...
		if (ch->equipment[HOLD]
			...
			|| IS_SET(ch->equipment[HOLD]->obj_flags.wear_flags,
				  ITEM_THROW))
		    && !number(0, 10)
		    ) {
			my_obj = unequip_char(ch, HOLD);
			equip_char(ch, my_obj, WIELD);
			act("$n wields $p.", TRUE, ch, ch->equipment[WIELD], 0,
			    TO_ROOM);
		}
		...
		if (IS_SET(ch->specials.act, ACT_SPEC)) {
			if (!mob_index[ch->nr].func) {
				sprintf(log_buf,
					"Mobile_activity: MOB[%d]%s no func",
					ch->nr, GET_NAME(ch));
				//if (port != 1220)
				//	log_hd(log_buf);
		...
		if (IS_SET(ch->specials.act, ACT_SCAVENGER)
			...
			if (best_obj) {
				...
				act("$n gets $p.", FALSE, ch, best_obj, 0,
				    TO_ROOM);
			}
		}
		/* Wander */
		if (ch->specials.hunting) {
			// slow down mob chasing
			if (pulse % (PULSE_MOBILE * 3) == 0) {
		...
		if ((IS_SET(ch->specials.act, ACT_AGGRESSIVE) && (!ch->desc)) ||
			...
			case 1:
				do_emote(ch,
					 "smiles at you and adorns your weapon with a wreath of daisies.",
					 0);
				break;
			case 2:
			...
			for (tmp_ch = world[ch->in_room]->people; tmp_ch;
				...
				if (GET_LEVEL(tmp_ch) > 31
				    && tmp_ch->specials.holyLite)
					continue;
				...
				if (IS_AFFECTED(tmp_ch, AFF_SNEAK)
				    && tmp_ch->p->sneaked_room == 0
				...
				if (IS_SET(ch->specials.act, ACT_AGGRESSIVE) &&
				    tmp_ch != ch->specials.hunting)
					if ((IS_SET
					     (ch->specials.act, ACT_AGGR_EVIL)
					     && IS_EVIL(tmp_ch))
					    ||
					    (IS_SET
					     (ch->specials.act, ACT_AGGR_NEUT)
					     && IS_NEUTRAL(tmp_ch))
					    ||
					    (IS_SET
					     (ch->specials.act, ACT_AGGR_GOOD)
					     && IS_GOOD(tmp_ch))
					    || (ch->specials.
						act & ACT_AGGR_ALL) == 0) {
						hit(ch, tmp_ch, 0);
				...
				if ((pulse % (PULSE_MOBILE * 4)) == 0) {
					if (ch->specials.hunting) {
						if ((ch->specials.hunting == tmp_ch)
						    && !IS_HOVERING(tmp_ch)) {
