...
extern void cast_stone_skin(byte level, struct char_data *ch, char *arg, int si,
			    struct char_data *tar_ch, struct obj_data *tar_obj);
...
extern void cast_invisibility(byte level, struct char_data *ch, char *arg,
			      ...
			      struct obj_data *tar_obj);
extern void cast_shield(byte level, struct char_data *ch, char *arg, int si,
			struct char_data *tar_ch, struct obj_data *tar_obj);
...
extern void cast_fireball(byte level, struct char_data *ch, char *arg, int si,
			  struct char_data *tar_ch, struct obj_data *tar_obj);
extern void cast_frost_shards(byte level, struct char_data *ch, char *arg,
			      int si, struct char_data *tar_ch,
			      struct obj_data *tar_obj);
extern void cast_colour_spray(byte level, struct char_data *ch, char *arg,
			      int si, struct char_data *tar_ch,
			      struct obj_data *tar_obj);
extern void cast_curse(byte level, struct char_data *ch, char *arg, int si,
		       struct char_data *tar_ch, struct obj_data *tar_obj);
extern void cast_weaken(byte level, struct char_data *ch, char *arg, int si,
			struct char_data *tar_ch, struct obj_data *tar_obj);
extern void cast_lightning_bolt(byte level, struct char_data *ch, char *arg,
				int si, struct char_data *tar_ch,
				struct obj_data *tar_obj);
extern void cast_shockwave(byte level, struct char_data *ch, char *arg, int si,
			   struct char_data *tar_ch, struct obj_data *tar_obj);
extern void cast_plague(byte level, struct char_data *ch, char *arg, int si,
			struct char_data *tar_ch, struct obj_data *tar_obj);
...
void mage_combat(struct char_data *ch)
{
	...
	if (GET_LEVEL(ch) >= 25 && GET_LEVEL(ch) < 38 && GET_MANA(ch) >= 25) {
		...
		if (!m)
			return;
		which = number(1, 6);
		if ((GET_LEVEL(ch) > (GET_LEVEL(m) + 1)) && !IS_AFFECTED(m, AFF_PLAGUE) && which == 1) {
			act("$n chants the magical phrase, 'Vas Nox'", TRUE, ch, 0, 0, TO_ROOM);
			GET_MANA(ch) -= 25;
			if (number(1, 100) > (80 + GET_INT(ch)))
				return;
