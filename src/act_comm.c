...
void do_auction(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*(argument + x))
		...
		for (to = descriptor_list; to; to = to->next) {
			...
			if ((ch != to->character)
			    && !to->connected
			    && !IS_SET(ORIGINAL(to->character)->specials.
				       new_comm_flags, PLR_NOAUCTION)
			    && !IS_SET(ORIGINAL(to->character)->specials.
				       plr_flags, PLR_WRITING)
			    ) {
				global_color = 35;
		...
		if (flag) {
			global_color = 35;
			sprintf(buf,
				"%s[%d] people heard you auction, '%s'.\n\r",
				MAG(ch), flag, argument + x);
			send_to_char(buf, ch);
			global_color = 0;
		} else {
			send_to_char
			    ("There are no people in Medievia with Auction turned on.",
			     ch);
		}
	}
}
...
void do_discuss(struct char_data *ch, char *argument, int cmd)
{
	...
	for (to = descriptor_list; to; to = to->next) {
		if (to->character && GET_LEVEL(to->character) < 33)
			break;
	...
	if (!gflag) {
		send_to_char
		    ("No GODS are present with DISCUSSION channel turned on.\n\r",
		     ch);
		return;
	}
	for (x = 0; *(argument + x) == ' '; x++) ;
	if (!*(argument + x))
		...
		if (flag) {
			if (ch->specials.ansi_color == 69)
				sprintf(buf,
					"\033[1m\033[35m[%d] people heard you Discuss, '%s'.\n\r\033[0m",
					flag, argument + x);
			else
				sprintf(buf,
					"[%d] people heard you Discuss, '%s'.\n\r",
					flag, argument + x);
			send_to_char(buf, ch);
		} else {
			send_to_char
			    ("There are no people in Medievia with DISCUSSION CHANNEL turned on.",
			     ch);
		}
	}
}
...
void do_quest(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!ch->desc
	    || IS_SET(ORIGINAL(ch)->specials.new_comm_flags, PLR_NOQUEST)) {
	...
	if (!*(argument + x))
		send_to_char
		    ("Yes, but WHAT do you want to quest channel say?\n\r", ch);
	else {
		for (to = descriptor_list; to; to = to->next) {
		...
		if (flag) {
			if (ch->specials.ansi_color == 69)
				sprintf(buf,
					"\033[1m\033[35m[%d] people heard you quest, '%s'.\n\r\033[0m",
					flag, argument + x);
			else
				sprintf(buf,
					"[%d] people heard you quest, '%s'.\n\r",
					flag, argument + x);
			send_to_char(buf, ch);
		} else {
			send_to_char
			    ("There are no people in Medievia with Quest turned on.",
			     ch);
		}
	}
}
...
void do_gossip(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!IS_NPC(ch))
		if (!ch->desc
		    || IS_SET(ORIGINAL(ch)->specials.new_comm_flags,
			      PLR_NOGOSSIP)) {
	...
	if ((GET_LEVEL(ch) < 32)
		...
		s = t % 60;
		sprintf(log_buf,
			"Gossiping is ONCE per 5 minutes, you have to wait another %d min %d sec.\n\r",
			m, s);
	...
	if (!*(argument + x))
		...
		if (flag) {
			...
			if (ch->specials.ansi_color == 69)
				sprintf(buf,
					"\033[1m\033[35m[%d] people heard you gossip, '%s'.\n\r\033[0m",
					flag, argument + x);
			else
				sprintf(buf,
					"[%d] people heard you gossip, '%s'.\n\r",
					flag, argument + x);
			send_to_char(buf, ch);
		} else {
			send_to_char
			    ("There are no people in Medievia with Gossip turned on.",
			     ch);
		}
	}
...
void do_pray(struct char_data *ch, char *argument, int cmd)
{
	...
	if ((IS_SET(world[ch->in_room]->room_flags, CHURCH) || world[ORIGINAL(ch)->in_room]->number == 2) && IS_UNDEAD(ch)) {
		if (ORIGINAL(ch)->specials.death_timer < 1) {
			reanimate(ch->desc, 0);
			guy_deleted = TRUE;
		}
		else {
			char b[64];
			sprintf(b, "You must wait %d minutes before praying at an altar.", ORIGINAL(ch)->specials.death_timer);
			send_to_char(b, ch);
		}
		return;
	}
	...
	if (!*(argument + x))
		...
		if (flag) {
			...
			send_to_char
			    ("There are no GODS in Medievia at the moment.",
			     ch);
		}
	}
}
	...
	if (!*(argument + x))
		...
		for (to = descriptor_list; to; to = to->next) {
			if (!to->character)
				continue;
			if ((!to->connected)
			    && (GET_POS(to->character) == POSITION_SLEEPING)
			    && (ch != to->character)) {
				if (iHoloChDistance(ch, to->character) < 250) {
					sprintf(buf, "%s dreams '%s'.\n\r",
						GET_NAME(ch), argument + x);
					send_to_char(buf, to->character);
					flag++;
				}
			}
		...
		if (flag) {
			...
			send_to_char
			    ("There are no sleeping people in Medievia right now\n\r",
			     ch);
		}
	}
}
	...
	if (!*(argument + x))
		...
		for (; to; to = to->next_in_room) {
			...
			if ((!strcmp(ch_alignment, toalignment))
			    && (ch != to)
			    && !IS_NPC(to)
			    && !IS_SET(ORIGINAL(to)->specials.plr_flags,
				       PLR_WRITING)) {
				sprintf(buf,
					"%s says to all %s people, '%s'.\n\r",
					(IS_NPC(ch) ? ch->player.
					 short_descr : GET_NAME(ch)),
					ch_alignment, argument + x);
				send_to_char(buf, to);
				flag++;
			}
		}
...
void do_talk(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*name || !*message)
		...
		act("$E is editing right now, try again later.", FALSE, ch, 0,
		    vict, TO_CHAR);
	else {
		global_color = 36;
...
void do_tell(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*name || !*message)
	...
	} else if (IS_SET(vict->specials.plr_flags, PLR_WRITING)
		   && GET_LEVEL(ch) < 35)
		act("$E is editing right now, try again later.", FALSE, ch, 0,
		    vict, TO_CHAR);
	else {

		if ((vict->desc && vict->desc->connected != CON_SOCIAL_ZONE)
		    && (GET_LEVEL(vict) < 32) && (GET_LEVEL(ch) < 32)) {
			send_to_char("They are not in MedLink!\r\n", ch);
			return;
		}

		if (ch->desc && ch->desc->connected != CON_SOCIAL_ZONE
		    && (GET_LEVEL(ch) < 32) && (GET_LEVEL(vict) < 32)) {
			send_to_char("You are not in MedLink!\r\n", ch);
			return;
		}

		global_color = 31;
		sprintf(buf, "%s telepaths you, '%s'.\n\r",
			(IS_NPC(ch) ? ch->player.short_descr : GET_NAME(ch)),
			message);
		send_to_char(buf, vict);
		sprintf(buf, "You telepath %s, '%s'.\n\r",
			(IS_NPC(vict) ? vict->player.
			 short_descr : GET_NAME(vict)), message);
		send_to_char(buf, ch);
		if (strcmp("Vryce", GET_NAME(ch))
		    && strcmp("Vryce", GET_NAME(vict)))
		...
		if (vict->specials.afk) {
			act("$N has AFK on, $E may not see your tell.", TRUE,
			    ch, 0, vict, TO_CHAR);
...
void do_whisper(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*name || !*message)
		...
		act(buf, FALSE, ch, 0, vict, TO_CHAR);
		act("$n whispers something to $N.", FALSE, ch, 0, vict,
		    TO_NOTVICT);
		global_color = 0;
	}
}
...
void do_ask(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*name || !*message)
		...
		act("$n quietly asks $mself a question.", FALSE, ch, 0, 0,
		    TO_ROOM);
...
void do_write(struct char_data *ch, char *argument, int cmd)
{
	...
	if (pen->obj_flags.type_flag != ITEM_PEN) {
		act("$p is no good for writing with.", FALSE, ch, pen, 0,
		    TO_CHAR);
...
*/
		...
		if (!paper->action_description) {
			...
			if (objs[paper->item_number]->action_description !=
			    paper->action_description)
				paper->action_description =
				    my_free(paper->action_description);
			paper->action_description = str_dup("BlAnK");
		}
		if (objs[paper->item_number]->action_description ==
		    paper->action_description) {
	...
	if (world[ch->in_room]->holox || world[ch->in_room]->holoy) {
		...
		for (p = descriptor_list; p; p = p->next) {
			...
			if (iDist < 300) {
				...
				CorruptString(corrupt_name_buf, GET_NAME(ch),
					      iPercent);
				sprintf(shout_buf,
					"You hear %s shout, '%s'.\r\n",
	...
	for (door = 0; door <= 5; door++) {
		if (EXIT(ch, door)) {
			if ((EXIT(ch, door)->to_room != NOWHERE)
				...
				shout_stuff_one(ch, shout_buf, distance,
	...
	for (door = 0; door <= 5; door++) {
		if (EXIT(ch, door)) {
			if ((EXIT(ch, door)->to_room != NOWHERE)
				...
				shout_stuff_one(ch, arg, distance,
						EXIT(ch, door)->to_room);
			}
		}
	}
