 ...
 */
	...
	if (IS_NPC(victim)
	    && ((victim->nr != 9800) || (!victim->specials.death_timer))) {
...
/* extract the corpse mob and put pc back into character_list */
		...
		for (iWear = 0; iWear < MAX_WEAR; iWear++)
			if (corpse->equipment[iWear])
				obj_to_char(unequip_char(corpse, iWear),
					    corpse);
...
void do_quit(struct char_data *ch, char *argument, int cmd)
{
	...
	struct room_affect *stpR = 1;        // pretend we camped to bypass this shit code
	...
	if (IN_HOLO(ch)) {
		for (stpR = world[ch->in_room]->room_afs; stpR;
		     stpR = stpR->next) {
	...
	if (cmd != 1111 && ch->specials.cost_to_rent != -2) {
		...
		if (!IS_NPC(ch)) {
			...
			} else if ((ch->specials.home_number)
				   && (world[ch->in_room]->number ==
				       ch->specials.home_number)
				   && (ch->specials.home_number))
				send_to_char
				    ("OK you are in your HOME, QUITTING IS SAFE!\n\r",
				     ch);
			else if (GET_LEVEL(ch) >= 32)
				send_to_char
				    ("OK GOD, come back soon MEDIEVIA NEEDS YOU!\n\r",
				     ch);
			else {
				global_color = 31;
				send_to_char
				    ("WE HAVE RENT HERE, YOU WILL LOSE ALL YOUR STUFF!\n\r",
				     ch);
				global_color = 0;
			}
	...
	if (ch->specials.cost_to_rent != -2) {
		if ((GET_LEVEL(ch) < 32)
		    && (world[ch->in_room]->number != ch->specials.home_number)
		    && (!stpR))
		...
		if (ch->specials.stpMount
		    && ch->in_room == ch->specials.stpMount->in_room) {
	...
	global_color = 34;
	send_to_char
	    ("                \\/\\ ^ /`0\r\n          /_^_\\ |          medievia.com 4000            | /_^_\\ \r\n          || || |                                       | || ||\r\n__________d|_|b_T_______________________________________T_d|_|b__________\r\n",
	     ch);
	global_color = 0;
...
void do_save(struct char_data *ch, char *argument, int cmd)
{
	...
	send_to_char
	    ("The [savedisplay command] toggles displaying of autosaves.\n\r",
	     ch);
}
...
void do_track(struct char_data *stpCh, char *szpArgument, int iCmd)
{
	...
	if (!szName || !*szName) {
		send_to_char
		    ("You must enter a name to track, use track off to stop tracking.\r\n",
		     stpCh);
		return;
	}
...
void do_sneak(struct char_data *ch, char *argument, int cmd)
{
	...
	if (IS_UNDEAD(ch)) {
		send_to_char("How can you sneak rattling all those chains?",
			     ch);
		return;
	}
	...
	if (GET_CLASS(ch) != CLASS_THIEF)
		if (!IS_SET(ch->player.multi_class, MULTI_CLASS_THIEF)) {
			send_to_char
			    ("Silly mudder, sneaking is for thieves!\n\r", ch);
			return;
		}
...
void do_steal(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!(victim = get_char_room_vis(ch, victim_name))) {
		...
		send_to_char
		    ("You practice stealing something from yourself!\n\r", ch);
		return;
	}
 ...
 */
	...
	if (!IS_NPC(victim) && GET_POS(victim) != POSITION_DEAD) {
		send_to_char
		    ("Due to misuse, you can't steal from other living players.\n\r",
		     ch);
		return;
	}
	...
	if (str_cmp(obj_name, "coins") && str_cmp(obj_name, "gold")) {
		...
		if (!
		    (obj =
		     get_obj_in_list_vis(victim, obj_name, victim->carrying))) {

			for (eq_pos = 0; (eq_pos < MAX_WEAR); eq_pos++)
				if (victim->equipment[eq_pos] &&
				    (isname
				     (obj_name,
				      victim->equipment[eq_pos]->name))
				    && CAN_SEE_OBJ(ch,
						   victim->equipment[eq_pos])) {
					obj = victim->equipment[eq_pos];
					break;
				}

			if (!obj) {
				act("$E has not got that item.", FALSE, ch, 0,
				    victim, TO_CHAR);
				return;
				...
				if (obj->item_number == 10
				    || obj->item_number == 16) {
					send_to_char
					    ("You would steal an object given to this soul by GOD???\n\r",
					     ch);
					return;
				}
				...
				if (IS_NPC(victim)
				    && IS_SET(obj->obj_flags.extra_flags,
					      ITEM_INVISIBLE)) {
					send_to_char
					    ("You try to steal the object but fail.\r\n",
					     ch);
					return;
				}
				...
				if ((GET_POS(victim) > POSITION_DEAD)) {
					...
					act("$n steals $p from $N.", FALSE, ch,
					    obj, victim, TO_NOTVICT);
					act("$n steals $p from you.", FALSE, ch,
					    obj, victim, TO_VICT);
				...
				if ((IS_CARRYING_N(ch) + 1 < CAN_CARRY_N(ch))) {
					if ((IS_CARRYING_W(ch) +
						...
						if (obj->item_number == 211) {
							send_to_char
							    ("You would break the egg in the attempt\n\r",
							     ch);
							return;
						}
						if (IS_NPC(victim)
						    && IS_SET(obj->obj_flags.
							      extra_flags,
							      ITEM_INVISIBLE)) {
							send_to_char
							    ("You try to steal the object but fail.\r\n",
						...
						if (IS_HOVERING(victim)) {
							sprintf(log_buf,
								"$n takes the %s from you.",
								obj->
								short_description);
							act(log_buf, FALSE, ch,
							    0, victim, TO_VICT);
							sprintf(log_buf,
								"$n takes the %s from $N.",
								obj->
								short_description);
							act(log_buf, TRUE, ch,
							    0, victim,
	...
	if (ohoh && IS_NPC(victim) && AWAKE(victim) && GET_LEVEL(ch) < 32) {
		if (IS_SET(victim->specials.act, ACT_NICE_THIEF)) {
			sprintf(log_buf, "%s Don't you *EVER* do that again!",
				GET_NAME(ch));
...
void do_note(struct char_data *ch, char *argument, int cmd)
{
	...
	act("$n jots down a note and the game saves it to $s file.", FALSE, ch,
	    0, 0, TO_ROOM);
}
...
void do_quaff(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!(temp = get_obj_in_list_vis(ch, buf, ch->carrying))) {
		...
		if ((temp == 0) || !isname(buf, temp->name)) {
			act("You do not have that item.", FALSE, ch, 0, 0,
			    TO_CHAR);
			return;
		}
	}
	...
	for (i = 1; i < 4; i++)
		if (temp->obj_flags.value[i] >= 1)
			((*spell_info[temp->obj_flags.value[i]].spell_pointer)
			 ((byte) temp->obj_flags.value[0], ch, "",
...
void do_recite(struct char_data *ch, char *argument, int cmd)
{
	...
	if (scroll->obj_flags.type_flag != ITEM_SCROLL) {
		act("Recite is normally used for scrolls.", FALSE, ch, 0, 0,
		    TO_CHAR);
		return;
	}
...
void do_use(struct char_data *ch, char *argument, int cmd)
{
	...
	for (x = 0; x < MAX_WEAR; x++)
		if (ch->equipment[x] != 0
		    && isname(buf, ch->equipment[x]->name)) {
			break;
		}
	if (x >= MAX_WEAR) {
		act("You are not holding or wearing that item!", FALSE, ch, 0,
		    0, TO_CHAR);
		return;
	}
	if (in_a_shop(ch) || IS_SET(world[ch->in_room]->room_flags, NO_MAGIC)) {
	...
	if (stick->obj_flags.type_flag == ITEM_STAFF) {
		...
		act("$n uses $p.", TRUE, ch, stick, 0, TO_ROOM);
		act("You use $p.", FALSE, ch, stick, 0, TO_CHAR);
		...
		if (bits) {
			if (bits == FIND_CHAR_ROOM) {
				act("$n uses $p on $N.", TRUE, ch, stick,
				    tmp_char, TO_ROOM);
				act("You use $p on $N.", FALSE, ch, stick,
				    tmp_char, TO_CHAR);
				act("You see $p glow with magic and slowly quiet down.", TRUE, ch, stick, tmp_char, TO_ROOM);
				act("You see $p glow with magic and slowly quiet down.", FALSE, ch, stick, tmp_char, TO_CHAR);
			} else {
				act("$n use $p on $P.", TRUE, ch, stick,
				    tmp_object, TO_ROOM);
				act("You use $p on $P.", FALSE, ch, stick,
				    tmp_object, TO_CHAR);
				act("You see $p glow with magic and slowly quiet down.", TRUE, ch, stick, tmp_object, TO_ROOM);
				act("You see $p glow with magic and slowly quiet down.", FALSE, ch, stick, tmp_object, TO_CHAR);
			}

			if (stick->obj_flags.value[2] > 0) {
				/* Is there any charges left? */
				stick->obj_flags.value[2]--;
				((*spell_info[stick->obj_flags.value[3]].
				  spell_pointer)
				 ((byte) stick->obj_flags.value[0], ch, "",
