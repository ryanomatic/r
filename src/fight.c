	...
	while (1) {
		for (ch = combat_list; ch; ch = combat_next_dude) {
			...
			if (ch->specials.fighting->in_room == -1) {
				sprintf(log_buf,
					"## %s->specials.fighting->in_room==-1 in perform_violence(%d)#1",
...
void hit(struct char_data *ch, struct char_data *victim, int type)
{
	...
	if (IS_DEAD(ch) && (victim->specials.fighting == ch)) {
		for (tmp = world[ch->in_room]->people; tmp;
			...
			if ((tmp != ch) && (tmp != victim)
			    && (tmp->specials.fighting == victim)
			    && (!IS_NPC(tmp)) && (victim->specials.fighting)
			    && (ch->specials.fighting)
			    && !IS_DEAD(tmp)) {
				stop_fighting(ch);
				stop_fighting(victim);
				sprintf(log_buf,
					"%s So!  You thought you could trick me into fighting a corpse!",
					GET_NAME(tmp));
				do_talk(victim, log_buf, 0);
				sprintf(log_buf,
					"stops fighting for a moment and then attacks %s with a vengeance.",
					GET_NAME(tmp));
...
/********************MOB COMBAT INTELIGENCE HERE*****************************/
	...
	if (ch && !IS_NPC(ch)) {
		if (ch->equipment[WIELD] && (ch->equipment[WIELD]->item_number == 6995 || ch->equipment[WIELD]->item_number == 6998)) {
			if (number(1, 12) == 5) {
				global_color = 31;
                                act("Fiery rain appears and falls on $N, searing $M.", FALSE, ch, NULL, victim, TO_CHAR);
                                act("Fiery rain appears and falls on $N, searing $M.", FALSE, ch, NULL, victim, TO_NOTVICT);
                                act("Fiery rain appears and falls on you, searing your skin.", FALSE, ch, NULL, victim, TO_VICT);
				global_color = 0;
				int fiery_rain = 100;
				damage(ch, victim, fiery_rain, TYPE_UNDEFINED);
			}
		}
	}

	if (ch && !IS_NPC(ch)) {
		if (ch->equipment[WIELD] && (ch->equipment[WIELD]->item_number == 7001 || ch->equipment[WIELD]->item_number == 7002)) {
			if (number(1, 12) == 3) {
				global_color = 30;
                                act("Darkness seeps out of your blade and surrounds $N.", FALSE, ch, NULL, victim, TO_CHAR);
                                act("Darkness seeps out of $n's blade and surrounds $N.", FALSE, ch, NULL, victim, TO_NOTVICT);
                                act("Darkness seeps out of $n's blade and surrounds you.", FALSE, ch, NULL, victim, TO_VICT);
				global_color = 0;
				int darkness = 100;
				damage(ch, victim, darkness, TYPE_UNDEFINED);
			}
		}
	}
	...
	if (percent >= 100)
		...
		strcat(buffer, " has many grievous wounds.\n\r");
	else if (percent >= 15)
		strcat(buffer,
		       " is covered with blood from oozing wounds.\n\r");
	else if (percent >= 8)
...
*/
	...
	if (diceroll < 120 && AWAKE(victim) && (diceroll <= 10 || diceroll < calc_thaco - victim_ac)) {
		//fprintf(stderr,"we missed? %d\n", diceroll);
		/* Miss. */
	...
	if (GET_CLASS(ch) == CLASS_WARRIOR)
		...
		dam +=
		    number(((int)((float)GET_DAMROLL(ch) / 2.0)),
			   GET_DAMROLL(ch));
	else
...
void position_change(struct char_data *ch)
{
	...
	case POSITION_SLEEPING:
		...
		act("$n wakes and stand up!", FALSE, ch, NULL, NULL, TO_ROOM);
	...
	case POSITION_BACK:
		send_to_char("You get off your back and get on your knees.\n\r",
			     ch);
		act("$n gets off $s back and starts kneeling.", FALSE, ch, NULL,
		    NULL, TO_ROOM);
		GET_MOB_POS(ch) = POSITION_KNEELING;
		break;
	case POSITION_BELLY:
		send_to_char
		    ("You push off your belly and get up on your knees.\n\r",
		     ch);
		act("$n pushes up off $s belly and gets on $s knees.", FALSE,
	    ...
	    int dam, int attacktype)
{
	...
	if (dam > 0) {
		if (IS_AFFECTED(victim, AFF_FIRESHIELD)) {
			if ((IS_UNDEAD(ch)) && (victim->nr == 9801) && (victim->specials.death_timer != 0)) {
				act("An invisible aura protects $N from your fireshield.", TRUE, victim, 0, ch, TO_CHAR);
				act("An invisible aura protects $N from $n's fireshield.", TRUE, victim, 0, ch, TO_NOTVICT);
				act("An invisible aura protects you from $n's fireshield.", TRUE, victim, 0, ch, TO_VICT);
			} else {
				act("Energy rebounds from your fireshield hitting $N.", TRUE, victim, 0, ch, TO_CHAR);
				act("Energy rebounds from $n's fireshield hitting $N.", TRUE, victim, 0, ch, TO_NOTVICT);
				act("Energy rebounds from $n's fireshield hitting you.", TRUE, victim, 0, ch, TO_VICT);
				if ((GET_LEVEL(ch) < 32) || IS_NPC(ch)) {
					if (IS_NPC(ch))
						GET_HIT(ch) -= dam / 2;
					else
						GET_HIT(ch) -= dam * 0.5 * mob_damage_multiplier;
				}
				if (GET_HIT(ch) < 1)
					GET_HIT(ch) = 1;
			}
	...
	if (IS_SET(ch->player.siMoreFlags, DRAGON)
	    && victim == ch->specials.hunting && ((GET_HIT(victim) - dam) < 15))
		dam = 0;
	/*
...
*/
		...
		for (i = 0; i < MAX_MESSAGES; i++) {
			...
			if (!IS_NPC(victim) && GET_LEVEL(victim) > 31) {
				...
				if (attacktype == SKILL_BACKSTAB && GET_CLASS(ch) == CLASS_THIEF) {
					if (number(1, 15) == 5) {
						dam *= 2;

						struct obj_data* wielded = ch->equipment[WIELD];
						char* weapon = wielded->short_description;
						char buf[128] = { 0 };

						global_color = 31;
						sprintf(buf, "You twist %s as you remove it from %s's back.", weapon, GET_NAME(victim));
						act(buf, FALSE, ch, NULL, victim, TO_CHAR);
						sprintf(buf, "%s twists %s as %s removes it from your back.", GET_NAME(ch), weapon, HSSH(ch));
						act(buf, FALSE, ch, NULL, victim, TO_VICT);
						sprintf(buf, "%s twists %s as %s removes it from %s's back.", GET_NAME(ch), weapon, HSSH(ch), GET_NAME(victim));
	...
	default:
		...
		if (GET_HIT(victim) < max_hit / 5) {
			global_color = 31;
			send_to_char
			    ("You wish you would stop BLEEDING so much!\n\r",
			     victim);
			global_color = 0;
		}
		break;
...
/*	if ( IS_NPC(victim) || victim->desc != NULL )*/
		...
		if (GET_CLASS(ch) == CLASS_MAGIC_USER) {
			mana_gain =
			    number(1, GET_LEVEL(victim) * 3) * number(1,
								      1 +
								      GET_LEVEL
								      (ch) /
								      10);
			...
			if (ch_al < -333 && vi_al <= -333)
				al = 1;
			else if (ch_al < -333 && vi_al >= -333 && vi_al <= 333)
				al = 2;
			else if (ch_al < -333 && vi_al >= 333)
				al = 3;
			else if (ch_al > -333 && ch_al <= 333 && vi_al <= -333)
				al = 2;
			else if (ch_al > -333 && ch_al <= 333 && vi_al >= -333
				 && vi_al <= 333)
				al = 1;
				...
				al = 1;
			mana_gain *= al;
			if (mana_gain + GET_MANA(ch) > GET_MAX_MANA(ch))
				GET_MANA(ch) = GET_MAX_MANA(ch);
			else
				GET_MANA(ch) += mana_gain;
			global_color = 34;
			if (al == 1)
				send_to_char
				    ("Your spirits were similiar, your power gain was minimal.\n\r",
				     ch);
			if (al == 2)
				send_to_char
				    ("Thy spirits were of a modest difference, You feel the power seep into your body.\n\r",
				     ch);
			if (al == 3)
				send_to_char
				    ("Thy spirits were diametrically opposed, You are flooded with power.\n\r",
				     ch);
			global_color = 0;
		}
		if (IS_NPC(victim)) {
			if (!victim->in_room) {
				sprintf(log_buf,
					"##%s is nowhere! killed by %s in room %d",
					GET_NAME(victim), GET_NAME(ch),
					ch->in_room);
				log_hd(log_buf);
			} else if (!world[victim->in_room]) {
				sprintf(log_buf,
					"##%s is out of this world in room %d, fixing....",
					GET_NAME(victim), victim->in_room);
				log_hd(log_buf);
			} else
				zone_table_array[world[victim->in_room]->zone].
				    numkills++;
		}
		if (!IS_NPC(victim)) {
			...
			for (ii = descriptor_list; ii; ii = ii->next)
				if (ii->character != ch
				    && !ii->connected
				    && GET_LEVEL(ii->character) > 32)
					if (IS_SET
					    (ii->character->specials.
					     god_display, GOD_DEATHS)) {
						sprintf(log_buf,
							"(death) %s[%d] killed by %s[%d]\n\r",
							GET_NAME(victim),
							GET_LEVEL(victim),
							(IS_NPC(ch) ? ch->
							 player.
						...
						send_to_char(log_buf,
							     ii->character);
					}
			global_color = 0;
			sprintf(log_buf, "%s %s[%d] killed by %s[%d] at %d",
				((GET_LEVEL(ch) == 32)
				 && !IS_NPC(ch)) ? "##" : "", GET_NAME(victim),
				GET_LEVEL(victim),
				(IS_NPC(ch) ? ch->player.
				 short_descr : GET_NAME(ch)), GET_LEVEL(ch),
				world[victim->in_room]->number);
			log_hd(log_buf);
			if (!IS_NPC(victim) && !IS_NPC(ch)) {
				if (IS_NPC(ch)
				    || IS_SET(world[victim->in_room]->
					      room_flags, CHAOTIC))
					if (GET_EXP(victim) >
					    exp_table[(int)GET_LEVEL(victim)]) {
						GET_EXP(victim) =
						    exp_table[(int)
							      GET_LEVEL(victim)]
						    +
...
bool check_parry(struct char_data * ch, struct char_data * victim)
{
	...
	if (victim->equipment[WIELD] == NULL)
		return FALSE;
	if (GET_MOB_POS(ch) == POSITION_SLEEPING)
		return FALSE;
	if (GET_MOB_POS(ch) == POSITION_SITTING)
		return FALSE;
	if (GET_MOB_POS(ch) == POSITION_BACK)
		return FALSE;
	if (GET_MOB_POS(ch) == POSITION_BELLY)
		return FALSE;
	if (GET_MOB_POS(ch) == POSITION_KNEELING)
		return FALSE;
...
void do_assist(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*name)
		...
		if (IS_NPC(vict)) {
			send_to_char
			    ("You must go to ARENA to fight a player!\n\r", ch);
			return;
		}
...
void make_corpse(struct char_data *ch, char keep_stuff)
{
	...
	if (ch->in_room == -1) {
		sprintf(log_buf, "## %s tried to make_corpse in room -1",
			GET_NAME(ch));
		log_hd(log_buf);
	...
	if (!IS_NPC(ch)) {
		...
		if (objs[9825]->short_description != head->short_description)
			head->short_description =
			    my_free(head->short_description);
		...
		if ((GET_LEVEL(ch) > 22) && (world[ch->in_room]->zone != 15)) {
			head->obj_flags.cost =
			    MIN(10000,
				MAX(300,
				    (GET_LEVEL(ch) * ch->specials.numpkills)));
		}
...
void death_cry(struct char_data *ch)
{
	...
	global_color = 33;
	act("You shiver in disgust as $n lets out a final, agonizing moan before death.", FALSE, ch, 0, 0, TO_ROOM);

	if (IS_NPC(ch))
		strcpy(message,
		       "You hear the pitiful wail of some dying creature.");
	else
		strcpy(message, "You hear a blood curdling death cry.");
	global_color = 0;
	was_in = ch->in_room;
	if (was_in < 0 || !world[was_in])
		return;
...
/*if(!IS_NPC(ch) && !IS_NPC(victim) &&
	...
	if (!is_formed(ch)) {	/* killer is soloing */
		...
		max_exp = (int)(pow((double)GET_LEVEL(ch), 3.79)
				* 50.0 / (double)GET_LEVEL(ch)
		    );
		tmp_share = MIN(max_exp, share);
		tmp_share *= exp_multiplier;
		...
		no_players = 0;
		for (x = 0; x < 3; x++)
			for (y = 0; y < 3; y++) {	/* count players in group */
				if (k->formation[x][y])
		...
		if ((GET_LEVEL(ch) > 31) && (min_level < 31)) {
			sprintf(buf, "## %s killed %s while grouped with a mortal.", GET_NAME(ch), GET_NAME(victim));
			log_hd(buf);
			global_color = 1;
			send_to_char("Let mortals kill for themselves!\n\r", ch);
			global_color = 0;
		}
		if (IS_NPC(victim) || IS_SET(world[victim->in_room]->room_flags, CHAOTIC)) {
			for (x = 0; x < 3; x++)
				for (y = 0; y < 3; y++) {	/* award exp to everyone in group */
					tmp_ch = k->formation[x][y];
					if (tmp_ch)
						if ((tmp_ch->in_room == ch->in_room) && ((tmp_ch->master == ch->master) || (tmp_ch->master == ch) || (tmp_ch == ch) || (tmp_ch == k)) && (!IS_NPC(tmp_ch))) {
							max_exp = (int)(pow((double)GET_LEVEL(tmp_ch), 3.79) * 50.0 / (double)GET_LEVEL(tmp_ch));
							if (level_diff == 0) {
							...
							if (tmp_share < 0)
								tmp_share = 0;
							sprintf(buf, "You receive %d experience points out of %d total.\n\r", tmp_share, share);
...
void dam_message(int dam_inflicted,
		 struct char_data *ch, struct char_data *victim, int w_type)
{
	...
	// handle % ones
	if (dam > 0) {
		float pct = (float)dam / (float)victim->points.hit * 100.0f;
		if (pct >= 99) {
			vs = "decapitate";
			vp = "decapitates";
		}
...
void disarm(struct char_data *ch, struct char_data *victim)
{
	...
	if (IS_OBJ_STAT(obj, ITEM_INVISIBLE)) {
		if (!(IS_AFFECTED(ch, AFF_DETECT_INVISIBLE))) {
			act("$n tries to disarm your invisible weapon and misses!", FALSE, ch, NULL, victim, TO_VICT);
			act("You try and disarm $N's invisible weapon and miss!", FALSE, ch, NULL, victim, TO_CHAR);
			return;
		}
	}
...
void trip(struct char_data *ch, struct char_data *victim)
{
	if (GET_MOB_POS(ch) != POSITION_STANDING)
		return;
...
*/
	...
	act("$n trips you and you go down!", FALSE, ch, NULL, victim, TO_VICT);
	act("You trip $N and $N goes down!", FALSE, ch, NULL, victim, TO_CHAR);
...
void fall(struct char_data *ch)
{
	...
	if (IS_SET(world[to_room]->room_flags, NEUTRAL) && !IS_SET(world[ch->in_room]->room_flags, NEUTRAL))
		return;

	if (IS_SET(world[to_room]->room_flags, CHAOTIC) && !IS_SET(world[ch->in_room]->room_flags, CHAOTIC))
		return;
...
/* if room below has any restrictions at all return */
	...
	act("You hear a blood curdling scream!", FALSE, ch, NULL, NULL,
	    TO_ROOM);
	act("$n falls from above! You scramble, to avoid being squished.",
	    FALSE, ch, NULL, NULL, TO_ROOM);
	if ((world[to_room]->sector_type == SECT_UNDERWATER)
		...
		send_to_char
		    ("You make a huge splash falling into the water!\n\r", ch);
		act("$n makes a huge splash falling in the water!", FALSE, ch,
		    NULL, NULL, TO_ROOM);
...
/* if exit down, make everyone fall */
	if ((EXIT(ch, 5)) && (EXIT(ch, 5)->to_room != ch->in_room)) {
		...
		if (ch && (GET_LEVEL(ch) < 32)
		    && (ch->in_room > 1)
		    && (ch->in_room < MAX_ROOM)
		    && (world[ch->in_room])) {
			act("You see the drop below and struggle to avoid falling further.", TRUE, ch, 0, 0, TO_CHAR);
			fall(ch);
		}
	}
