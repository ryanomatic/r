...
void do_socialwarp(struct char_data *ch, char *argument, int cmd)
{
	...
	if (GET_LEVEL(ch) > 32) {
		send_to_char
		    ("For many reasons gods may not use this command.  It is impossible to keep track of where you are and where you were as a god.  \n\rThe MEDLINK zone begins at room 1652 just go there.\n\r",
		     ch);
		return;
	}
	if (ch->desc->connected == CON_SOCIAL_ZONE) {
		if (ch->p->iLastSocialX || ch->p->iLastSocialY) {
			if (Holo[ch->p->iLastSocialX][ch->p->iLastSocialY] >
			    255)
				room =
				    Holo[ch->p->iLastSocialX][ch->p->
							      iLastSocialY];
			else
				room =
				    iMakeHoloRoom(ch->p->iLastSocialX,
						  ch->p->iLastSocialY);
			if (!world[room]) {
				sprintf(log_buf,
					"## No where to send home %s from SocialZone\r\n",
					GET_NAME(ch));
				log_hd(log_buf);
				send_to_char
				    ("Error, Please tell vryce what you did.\r\n",
				     ch);
				return;
			}
			act("$n flashes out of MedLink.", TRUE, ch, 0, 0,
			    TO_ROOM);
			MOUNTMOVE = TRUE;
			char_from_room(ch);
			...
			act("$n flashes back into the realms from MedLink.",
			    TRUE, ch, 0, 0, TO_ROOM);
			do_look(ch, "", 15);
			return;
		} else {
	...
	if (ch->desc->connected == CON_PLAYING) {
		if ((world[ch->in_room]->zone == 198 ||
		     ...
		     || GET_POS(ch) == POSITION_FIGHTING) && (!IN_HOLO(ch))) {
		...
		if (IN_HOLO(ch)) {
			...
			act("$n flashes out of existance.", TRUE, ch, 0, 0,
			    TO_ROOM);
			MOUNTMOVE = TRUE;
			char_from_room(ch);
			...
			act("$n flashes into MedLink.", TRUE, ch, 0, 0,
			    TO_ROOM);
...
int is_allowed_here(struct char_data *ch, int to_room)
{
	...
	if (world[to_room]->level_restriction) {
		if (world[to_room]->level_restriction > 1
		    && GET_LEVEL(ch) < world[to_room]->level_restriction) {
			send_to_char
			    ("Your low level keeps you from entering.\n\r", ch);
			return (FALSE);
		}
		if (world[to_room]->level_restriction < 0
		    && (GET_LEVEL(ch) >
			(world[to_room]->level_restriction * -1))) {
			send_to_char
			    ("Your high level keeps you from entering.\n\r",
			     ch);
			return (FALSE);
		}
	}
...
/*
	...
	if (IS_NEUTRAL(ch)
	    && IS_SET(world[to_room]->align_restriction, NO_NEUTRAL)) {
		global_color = 31;
...
*/
{
	...
	if (IS_UNDEAD(ch) && number(1, 100) < 3) {
	...
	if (ch->desc && ch->desc->connected != CON_SOCIAL_ZONE)
		if (GET_BREATH(stpMoveCh) <= 0) {
			...
			global_color = 32;
			send_to_char
			    ("You STOP and PANT loud, sucking in air!\n\r",
			     stpMoveCh);
			...
			global_color = 32;
			send_to_char
			    ("You pant loudly, almost out of breath.\n\r",
			     stpMoveCh);
			act("$n pants loudly, almost out of breath.", TRUE,
			    stpMoveCh, 0, 0, TO_ROOM);
			global_color = 0;
		}
	if (world[ch->in_room]->zone == 198) {
		if (number(0, 38) == 25) {
			if (world[ch->in_room]->sector_type == SECT_INSIDE) {
				if (IS_AFFECTED(ch, AFF_FLYING)) {
					act("$n hits $s head on a stalactite.",
					    TRUE, ch, 0, 0, TO_ROOM);
					send_to_char
					    ("You hit your head on a stalactite.\n\r",
					     ch);
					return (FALSE);
				} else {
					act("$n trips over a small stalagmite and stumbles.", TRUE, ch, 0, 0, TO_ROOM);
					send_to_char
					    ("You trip over a small stalagmite and stumble.\n\r",
					     ch);
					return (FALSE);
				}
			}
	...
	need_movement = (movement_loss[world[ch->in_room]->sector_type]
			 ...
			 +
			 movement_loss[world[EXIT(ch, cmd)->to_room]->
				       sector_type]);
	...
	if (IS_SET(world[EXIT(ch, cmd)->to_room]->room_flags, TUNNEL))
		if (is_tunnel_full(ch, EXIT(ch, cmd)->to_room)) {
			send_to_char
			    ("You try to enter the room, but it is too cramped to proceed.\r\n",
			     ch);
			return (FALSE);
		}
...
*/
	...
	if (world[ch->in_room]->zone != 180) {	/*Social Zone */
		...
		if ((iMovement < need_movement)
		    && (IS_UNDEAD(ch) || !IS_NPC(ch))) {
			if (!following) {
				...
				if (!IS_NPC(ch) && ch->specials.stpMount)
					send_to_char
					    ("Your mount is too exhausted to follow.\n\r",
					     ch);
				else
					send_to_char
...
*/
			if (IS_SET(world[to_room]->room_flags, CHAOTIC) &&	/*law to cha */
				...
				strcpy(ch->p->queryprompt,
				       "ENTER CHAOTIC PlayerKill Area? (y/n)>");
				ch->p->querycommand = 1111;
				ch->specials.direction = cmd;
				return (FALSE);
			}
...
*/
	...
	if (!IS_AFFECTED(ch, AFF_SNEAK)) {
		...
		if (!world[ch->in_room]->dir_option[cmd]->exit
		    || (world[ch->in_room]->dir_option[cmd]->exit
			&& !strstr(world[ch->in_room]->dir_option[cmd]->exit,
				   "$n"))) {
			if (IS_AFFECTED(ch, AFF_FLYING)) {
				sprintf(tmp, "$n flies %s.", dirs[cmd]);
			} else {
				if (!ch->specials.stpMount) {
					sprintf(tmp, "$n leaves %s.",
						dirs[cmd]);
				} else {
					sprintf(tmp, "$n riding %s leaves %s.",
						ch->specials.stpMount->player.
						short_descr, dirs[cmd]);
				}
				act(tmp, TRUE, ch, 0, 0, TO_ROOM);
			}
		} else {
			act(world[ch->in_room]->dir_option[cmd]->exit, TRUE, ch,
			    0, 0, TO_ROOM);
		}
		global_color = 0;
	}
	...
	if (!IS_AFFECTED(ch, AFF_SNEAK)) {
		if (!world[was_in]->dir_option[cmd]->entrance
		    || (world[was_in]->dir_option[cmd]->entrance
			&& !strstr(world[was_in]->dir_option[cmd]->entrance,
				   "$n"))) {
			if (!ch->specials.stpMount) {
				act("$n has arrived.", TRUE, ch, 0, 0, TO_ROOM);
			} else {
				sprintf(tmp, "$n riding %s arrived.",
					ch->specials.stpMount->player.
					short_descr);
				act(tmp, TRUE, ch, 0, 0, TO_ROOM);
			}
		} else {
			act(world[was_in]->dir_option[cmd]->entrance, TRUE, ch,
			    0, 0, TO_ROOM);
		}
	}
	global_color = 0;
	...
	if (IS_SET(world[ch->in_room]->room_flags, FIRE)) {
		if (GET_LEVEL(ch) < 32 && !IS_NPC(ch)) {
			...
			if (GET_HIT(ch) < 0) {
				global_color = 31;
				send_to_char
				    ("Your body BURNS brightly and turns to ASH. You are dead!\r\n",
				     ch);
				act("$n's body turns to ash.", FALSE, ch, 0, 0,
				    TO_ROOM);
				global_color = 0;
				raw_kill(ch, NULL);
				return (1);
	...
	if (!IS_NPC(ch) && !IS_UNDEAD(ch) &&
	    (world[ch->in_room]->sector_type <= 5
	     || world[ch->in_room]->sector_type == 8))
					...
					if ((GET_LEVEL(k) < GET_LEVEL(ch))
						...
						if (!IS_UNDEAD(k))
							if ((number(0, 100) -
							    ...
							    50) {
								sprintf(log_buf,
									"AH HA! I have been looking for you %s!",
									GET_NAME
									(ch));
								do_say(k,
								       log_buf,
...
void do_move(struct char_data *ch, char *argument, int cmd)
{
	...
	if (stpEarthquake && GET_LEVEL(ch) < 32) {
		...
		GET_HIT(ch) /= 3;
		send_to_char
		    ("You try and move during the QUAKE and fall.. . ..\n\r",
		     ch);
		global_color = 0;
		return;
	}
	if (IS_SET(EXIT(ch, cmd)->exit_info, EX_CLOSED)
	    && IS_UNDEAD(ch)
	    && !IS_SET(world[ch->in_room]->dir_option[cmd]->exit_info,
		       EX_PICKPROOF)
	    ) {
		act("You turn into a ghostly shimmer and try to pass through solid mass.", FALSE, ch, 0, ch, TO_CHAR);
...
/* Direction is possible */
	...
	if (!IS_NPC(ch) && ch->specials.stpMount &&
	    IS_SET(world[world[ch->in_room]->dir_option[cmd]->to_room]->
		...
		if (IS_AFFECTED(ch, AFF_CHARM) && (ch->master != ch)
			...
			send_to_char
			    ("The thought of leaving your master makes you weep.\n\r",
			     ch);
			act("$n wants to leave you!.", FALSE, ch, 0, 0,
			    TO_ROOM);
		} else {
			was_in = ch->in_room;
			MOUNTMOVE = TRUE;
					...
					for (x = 0; x < 3; x++)
						for (y = 0; y < 3; y++) {
							if (ch->formation[x][y] && ch->formation[x][y] != ch)
								if ((was_in == ch->formation[x][y]->in_room) && (GET_POS(ch->formation[x][y]) >= POSITION_STANDING)) {
									global_color = 1;
									act("You follow $N.", FALSE, ch->formation[x][y], 0, ch, TO_CHAR);
									cmd++;
									send_to_char("\n\r", ch->formation[x][y]);
									global_color = 0;
...
void do_open(struct char_data *ch, char *argument, int cmd)
{
	...
	else if (!isdoor && generic_find(argument, FIND_OBJ_INV | FIND_OBJ_ROOM,
			      ch, &victim, &obj))
...
void do_close(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*type)
		...
		if (!IS_SET(EXIT(ch, door)->exit_info, EX_ISDOOR))
			...
			if (EXIT(ch, door)->keyword)
				act("$n closes the $F.", 0, ch, 0,
				    EXIT(ch, door)->keyword, TO_ROOM);
			else
				act("$n closes the door.", FALSE, ch, 0, 0,
				    TO_ROOM);
			send_to_char("Ok.\n\r", ch);
...
void do_lock(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*type)
		...
		if (obj->obj_flags.type_flag != ITEM_CONTAINER)
			...
			send_to_char("*Click*\n\r", ch);
			act("You hear a soft click as $n locks $p.", FALSE, ch,
			    obj, 0, TO_ROOM);
	} else if ((door = find_door(ch, type, dir)) >= 0)
		...
		if (!IS_SET(EXIT(ch, door)->exit_info, EX_ISDOOR))
			...
			send_to_char
			    ("You have to close it first, I'm afraid.\n\r", ch);
		else if (EXIT(ch, door)->key < 0)
			send_to_char
			    ("There do not seem to be any keyholes.\n\r", ch);
			...
			if (EXIT(ch, door)->keyword)
				act("$n locks the $F.", 0, ch, 0,
				    EXIT(ch, door)->keyword, TO_ROOM);
			else
				act("$n locks the door.", FALSE, ch, 0, 0,
				    TO_ROOM);
			send_to_char("*Click*\n\r", ch);
...
void do_unlock(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*type)
		...
		if (obj->obj_flags.type_flag != ITEM_CONTAINER)
			...
			send_to_char
			    ("Odd - you can't seem to find a keyhole.\n\r", ch);
		else if (!has_key(ch, obj->obj_flags.value[2]))
			send_to_char
			    ("You don't seem to have the proper key.\n\r", ch);
		else if (!IS_SET(obj->obj_flags.value[1], CONT_LOCKED))
			...
			for (mykey = ch->carrying; mykey; mykey = next_key) {
				next_key = mykey->next;
				if (mykey->item_number ==
				    obj->obj_flags.value[2]) {
					if (mykey->obj_flags.value[1] == 1) {
						act("$p crumbles to dust in $n's hand.", TRUE, ch, mykey, 0, TO_ROOM);
						act("$p crumbles to dust in your hand.", FALSE, ch, mykey, 0, TO_CHAR);
						extract_obj(mykey);
					}
					if (mykey->obj_flags.value[1] > 1)
			...
			for (iWear = 0; iWear < MAX_WEAR; iWear++)
				if (ch->equipment[iWear])
					if (ch->equipment[iWear]->item_number ==
					    obj->obj_flags.value[2]) {
						if (ch->equipment[iWear]->
						    obj_flags.value[1] == 1) {
							act("$p crumbles to dust in $n's hand.", TRUE, ch, ch->equipment[iWear], 0, TO_ROOM);
							act("$p crumbles to dust in your hand.", FALSE, ch, ch->equipment[iWear], 0, TO_CHAR);
							mykey =
							    ch->
							    equipment[iWear];
		...
		if (!IS_SET(EXIT(ch, door)->exit_info, EX_ISDOOR))
			...
			if (EXIT(ch, door)->keyword)
				act("$n unlocks the $F.", 0, ch, 0,
				    EXIT(ch, door)->keyword, TO_ROOM);
			else
				act("$n unlocks the door.", FALSE, ch, 0, 0,
				    TO_ROOM);
			send_to_char("*click*\n\r", ch);
			...
			if ((other_room = EXIT(ch, door)->to_room) != NOWHERE)
				if ((back =
					...
					if (back->to_room == ch->in_room)
						REMOVE_BIT(back->exit_info,
							   EX_LOCKED);
			for (mykey = ch->carrying; mykey;
			     mykey = mykey->next_content)
...
void do_pick(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*type)
		...
		if (!IS_SET(EXIT(ch, door)->exit_info, EX_ISDOOR))
			...
			REMOVE_BIT(EXIT(ch, door)->exit_info, EX_LOCKED);
			if (EXIT(ch, door)->keyword)
				act("$n skillfully picks the lock of the $F.",
				    0, ch, 0, EXIT(ch, door)->keyword, TO_ROOM);
			else
				act("$n picks the lock of the.", TRUE, ch, 0, 0,
				    TO_ROOM);
			send_to_char
...
void do_enter(struct char_data *ch, char *argument, int cmd)
{
	...
	int from_room = 1724;
	int vecna2_room = 3704;
	if (ch->in_room == from_room && !strcmp(buf, "portal")) {
		act("$n steps into the portal and disappears.", TRUE, ch, 0, 0, TO_NOTVICT);
		act("You step into the portal.", TRUE, ch, 0, 0, TO_CHAR);

		char_from_room(ch);
		char_to_room(ch, vecna2_room);

		act("$n steps out the portal.", TRUE, ch, 0, 0, TO_NOTVICT);
	...
	for (door = 0; door <= 5; door++) {
		if (EXIT(ch, door))
			if (EXIT(ch, door)->to_room != NOWHERE && IS_SET(EXIT(ch, door)->exit_info, EX_CLOSED)) {
				if (!IS_SET(EXIT(ch, door)->exit_info, EX_HIDDEN) && (!IS_SET(EXIT(ch, door)->exit_info, EX_SECRET) || !OK))
					continue;
				...
				if (EXIT(ch, door)->keyword)
					sprintf(buf, "$n finds a secret %s in the %s and opens it.", EXIT(ch, door)->keyword, exits[door]);
				else
...
void do_stand(struct char_data *ch, char *argument, int cmd)
{
	...
	case POSITION_STANDING:{
			act("You are already standing.", FALSE, ch, 0, 0,
			    TO_CHAR);
		}
		break;
	...
	case POSITION_RESTING:{
			act("You stop resting, and stand up.", FALSE, ch, 0, 0,
			    TO_CHAR);
			act("$n stops resting, and clambers to $s feet.", TRUE,
			    ch, 0, 0, TO_ROOM);
			GET_POS(ch) = POSITION_STANDING;
		}
		break;
	case POSITION_SLEEPING:{
			act("You have to wake up first!", FALSE, ch, 0, 0,
			    TO_CHAR);
		}
		break;
...
void do_sit(struct char_data *ch, char *argument, int cmd)
{
	...
	case POSITION_SITTING:{
		...
		break;
	case POSITION_RESTING:{
			act("You stop resting, and sit up.", FALSE, ch, 0, 0,
			    TO_CHAR);
...
void do_rest(struct char_data *ch, char *argument, int cmd)
{
	...
	case POSITION_STANDING:{
		...
		break;
	case POSITION_SITTING:{
			act("You rest your tired bones.", FALSE, ch, 0, 0,
			    TO_CHAR);
		...
		break;
	case POSITION_RESTING:{
			act("You are already resting.", FALSE, ch, 0, 0,
			    TO_CHAR);
		}
		break;
	case POSITION_SLEEPING:{
		...
		break;
	case POSITION_FIGHTING:{
			act("Rest while fighting? are you MAD?", FALSE, ch, 0,
			    0, TO_CHAR);
		}
		break;
	default:{
...
void do_sleep(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!IS_NPC(ch) && ch->specials.stpMount) {
		send_to_char
		    ("You try and fall asleep on a beast... it didn't work...\n\r",
		     ch);
		return;
	}
	switch (GET_POS(ch)) {
	...
	case POSITION_RESTING:{
			...
			act("$n lies down and falls asleep.", TRUE, ch, 0, 0,
			    TO_ROOM);
...
{
        ch->specials.solo = !ch->specials.solo;
        if (ch->specials.solo)
                send_to_char("Ok, you're now in solo mode.\n", ch);
        else
                send_to_char("Ok, you're no longer in solo mode.\n", ch);
}
