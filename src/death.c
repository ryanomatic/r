...
int undead_corpse(struct char_data *ch, int cmd, char *arg)
{
	...
	case 2:
		if (GET_POS(ch) > POSITION_RESTING) {
			do_emote(ch,
				 "trips, falling under the weight of its burden.",
				 0);
	...
	case 3:
		do_emote(ch,
			 "emits a pitiful wail, sending shivers up your spine.",
			 0);
		return (1);
		break;
	case 4:
	...
	case 5:
		do_emote(ch,
			 "has a sudden muscle spasm and its head rotates 360 degrees.",
			 0);
		return (1);
		break;
	case 6:
		do_emote(ch, "pulls out a section of intestine and eats it.",
			 0);
		return (1);
		break;
	case 7:
		do_emote(ch,
			 "pops out an eyeball and shoves it through an ear.",
			 0);
		return (1);
		break;
	case 8:
	...
	case 10:
		do_emote(ch,
			 "drips ooze from its nose, then sniffs it back up.",
			 0);
		return (1);
		break;
	default:
...
/* stay hovering until the next HOVERING pulse */
	for (d = descriptor_list; d; d = d_next) {
		...
		if (!d->character || (STATE(d) != CON_HOVERING))
			...
			send_to_char
			    ("The necromancer returns in a puff of smoke and pokes your body with the\r\nend of his staff as if testing it.  Pleased with the results, he opens\r\na small blue bottle and proceeds with a chant that makes your hair stand\r\non end.  The corpse twists and turns and rises off the ground. Finally\r\nwith a woosh something is sucked from the body into the bottle. The corpse\r\nfalls back down to the ground in a heap.  Without your spirit, you are\r\ndoomed to roam the world undead.\r\n",
			     d->character);
			act("The necromancer returns and performs a ritual over the corpse.  It levitates\r\nand writhes violently for a few moments until you hear a whoosh and the clink\r\nof the old mage sealing $n's spirit into a small blue bottle.", FALSE, d->character, 0, NULL, TO_ROOM);
		} else {
			send_to_char
			    ("A figure in a black robe arrives from a puff of smoke nodding his head\r\napprovingly at the fresh corpse. Getting straight to work, he traces a\r\ncircle around the body with his staff. Removing a handful of dried scarabs\r\nfrom a fold in his cloak, he places one over each eye and crumbles the rest,\r\nsprinkling them over the body.  Standing outside the circle, he touches the\r\nstaff to your forehead uttering the words \'Drixnil itum Ptlm\'.  The corpse,\r\nthe staff, and everything within the circle are engulfed in a strange green\r\nflame which he douses after a few moments with some water from a blue bottle.\r\nThe water has a strange effect on the corpse.  Suddenly the necromancer is\r\ngone as quickly as he arrived, leaving some time for the unholy water to have\r\nits effect.\r\n",
			     d->character);
			act("A figure in a black robe arrives from a puff of smoke. He prepares the\r\ncorpse, and performs a short ritual, engulfing it in a circle of flame. He\r\ndouses the flame with a vial of unholy water, then leaves as quickly as he\r\n arrived.", FALSE, d->character, 0, NULL, TO_ROOM);
		}
...
/* delete old spirit bottle from the crypt and make new ones later */
	...
	while (obj) {
		while ((obj->next_content)
		       && (obj->next_content->item_number == 9811))
			extract_obj(obj->next_content);
		obj = obj->next_content;
	}
	if ((world[crypt]->contents)
	    && (world[crypt]->contents->item_number == 9811))
	...
	for (d = descriptor_list; d; d = d_next) {
		...
		if (STATE(d) == CON_UNDEAD) {
			if ((ORIGINAL(d->character)->specials.death_timer < -60)
				...
				if (objs[urn->item_number]->description !=
				    urn->description)
					urn->description =
					    my_free(urn->description);
				urn->description = str_dup(log_buf);
				sprintf(log_buf, "%s bottle sapphire",
					GET_NAME(d->character));
				if (objs[urn->item_number]->name != urn->name)
					urn->name = my_free(urn->name);
				urn->name = str_dup(log_buf);
				SET_BIT(urn->obj_flags.value[1],
					CONT_CLOSEABLE);
				...
				sprintf(log_buf,
					...
					GET_NAME(d->character));
				if (objs[2999]->short_description !=
				    spirit->short_description)
					spirit->short_description =
					    my_free(spirit->short_description);
...
struct char_data *loadundead(struct char_data *ch)
{
	...
	sprintf(log_buf, "This tasty morsel used to be %s's finger.",
		GET_NAME(ch));
	obj->ex_description->description =
	    my_free(obj->ex_description->description);
...
*/
	...
	af.next = NULL;
	affect_to_char(ch, &af);
	act("$n's consciousness escapes the painful tomb of its dead body and hovers\r\npeacefully above you.  Empowered with extrordinary perception, $e is able\r\nto see everything that goes on here, a silent witness to anyone who might\r\ntry to desecrate the corpse.", FALSE, ch, 0, NULL, TO_ROOM);
	send_to_char
	    ("You're dead now. You can't move or speak.  The world becomes black and your\r\nhead turns inside out with pain. Unable to scream or move, your senses are\r\noverwhelmed with agony and terror. Finally, an escape!  You feel released,\r\nlighter than air, nothing but pure consiousness hovering over the hellish\r\npain of your corpse.\r\n",
	     ch);
}
...
void undeadify(struct char_data *ch)
{
	...
	act("A chill fills the air and a pair of greyish hands reach up from below, slowly \r\npulling the corpse of $n down into the bowels of the earth until \r\nall that's left is blood and the stench.", FALSE, ch, 0, NULL, TO_ROOM);
...
*/
	...
	else if (!zone_table[world[ch->in_room]->zone].top)
		ch_in_room = ch->in_room;
	else
		...
		while
		    ((obj->next_content) &&
		     ((obj->next_content->obj_flags.type_flag == ITEM_KEY)
		      ...
		      || IS_SET(obj->next_content->obj_flags.extra_flags,
				ITEM_NO_RENT)
		     )
		    )
			extract_obj(obj->next_content);
...
/* delete the PC from character list*/
	...
	send_to_char
	    ("You slowly rise to your feet, condemed as a zombie until you repent.\r\n",
	     ch);
	send_to_char("for your misdoings at a Holy Place.\r\n", ch);

	act("$n's body slowly rises from the ground. Whatever it was before, it is no \r\nlonger human! (temp).", FALSE, corpse, 0, NULL, TO_ROOM);
}

void reanimate(struct descriptor_data *d, int cmd)
{
	struct char_data *corpse = NULL, *player = NULL, *k = NULL, *next_char =
	    NULL;
	extern struct char_data *character_list;
	int iWear;
	int crypt = 1;
...
/* remove spirit bottle from crypt */
	if (world[1729])
		crypt = 1729;
	bottle =
	    get_obj_in_list(GET_NAME(d->character), world[crypt]->contents);
	if (bottle)
		extract_obj(bottle);
	...
	act("$n drags the lifeless body of the necromancer off in the direction of his sanctum.", FALSE, corpse, 0, NULL, TO_ROOM);
...
/* get rid of undead corpse */
	...
	if (cmd) {
		send_to_char
		    ("You drag the lifeless body of the Necromancer back to his sanctum and find\r\nthe bottle containing your spirit.  You're barely strong enough to pull out\r\nthe stopper.  His hand is already cold, but the magical aura still left in\r\nthe twisted fingers is enough to free your spirit from its crystal prison.\r\nYou feel like new!  Your flesh magically grows back, your posessions are\r\nreturned and you are restored to the condition you were in before your\r\ndeath. Then, before you can comprehend what's happened, the corpse\r\nshrivels up and dissolves into a pile of dust.\r\n\r\n",
		     player);
		act("$n limps in dragging the lifeless body of a Necromancer. The magical aura\r\nstill left in the dead mage's fingers frees the spirit from the bottle and\r\nit reunites with $n, magically bringing him back to life. Before you realize\r\nwhat's happened, the corpse of the necromancer dissolves into a pile of dust.", FALSE, player, 0, NULL, TO_ROOM);
	} else {
		send_to_char
		    ("A divine force takes pity on your wretched existance and reunites you\r\nwith your spirit, breathing life into your body.\r\n",
		     player);
	}
}
...
void do_kill(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!IS_NPC(victim)) {
		sprintf(log_buf, "##%s[%d] brutally slays %s[%d].\n",
			GET_NAME(ch), GET_LEVEL(ch), GET_NAME(victim),
			GET_LEVEL(victim));
		log_hd(log_buf);
	}
	global_color = 31;
...
void raw_kill(struct char_data *ch, struct char_data *attacker)
{
	...
	if (ch->in_room == -1) {
		sprintf(log_buf, "## %s vs room -1 in raw_kill", GET_NAME(ch));
		log_hd(log_buf);
		return;
	}
	...
	if (attacker && IS_UNDEAD(attacker)
	    && (ch->nr == 9801)
	...
	if (IS_NPC(ch) && !IS_UNDEAD(ch)) {
		sprintf(log_buf,
			"##A NPC(%s) IS NOT BEING EXTRACTED IN RAW_KILL!",
...
/* do this so people don't spam the city with VfRuYcCkE corpses */
	if (GET_LEVEL(ch) < 5) {
		...
		send_to_char
		    ("The gods take pity on your poor newbie existance by restoring your life and\r\nplacing you safely out of harm's way.",
		     ch);
		act("$n arrives in a puff of smoke, near death.", FALSE, ch, 0,
		    NULL, TO_ROOM);
		return;
	}

	if (IS_DEAD(ch)) {
		sprintf(log_buf, "You're already dead!  Adding 5 minutes.\n\r");
...
*/
		GET_POS(ch) = POSITION_RESTING;
		act("$n digs $mself back up out of $s grave", FALSE, ch, 0,
		    NULL, TO_ROOM);
		return;
	}
...
*/
		if ((attacker)
		    && (	/* suicide in npk */
			       ((attacker == ch)
				&& IS_SET(world[ch->in_room]->room_flags,
					  NEUTRAL)
			       )
			       || ((attacker != ch)
				   && !IS_NPC(attacker)
...
*/
			...
			char_to_room(ch, to_room);
			send_to_char
			    ("\r\nYou wake up with a splitting headache that runs clear down to your\r\ntoenails.  You thought you had died, but you realize you only feel like it. \r\n",
			     ch);
			act("You see a hand sticking out from under some trash.  You look closer and see \r\nthat its attatched to $n whom you nearly mistake $m for dead.", FALSE, ch, 0, NULL, TO_ROOM);
		} else {
	...
	for (cur = character_list; cur; cur = cur_next) {
		...
		if ((cur->nr == 9801) && cur->specials.death_timer) {
			if ((cur->specials.death_timer > 15)
				...
				strcpy(cur->player.description,
				       "THIS NECRO WAS FREED!");
...
*/
			if ((world[room])
			    && (zone_table[world[room]->zone].continent ==
				continent)
			    && (world[room]->people)
...
void load_necromancer(int room, struct char_data *victmob)
{
	...
	for (cur = character_list; cur; cur = cur->next) {
		if (IS_UNDEAD(cur)
		    && (ORIGINAL(cur)->specials.numpkills > max_pkills))
	...
	act("A sudden chill in the air announces the arrival of a Necromancer, come in\rsearch of new victims.", FALSE, mob, 0, NULL, TO_ROOM);
}
...
void undead_gross_out(struct char_data *ch, struct char_data *k)
{
	...
	if ((number(1, 100) > (100 - chance)) && (GET_LEVEL(k) < 32)) {
		sprintf(log_buf,
			"screams in terror at the sight of %s.",
			ch->player.short_descr);
