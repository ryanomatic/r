...
struct command_info cmd_info[] = {
	 ...
	 */
	{"p", do_put, POSITION_RESTING, 0, 223, 1},
	...
	{"telepath", do_tell, POSITION_RESTING, 0, 9, 0},
	...
	{"glance", do_glance, POSITION_RESTING, 0, 9, 0},
	...
	{"stats", do_stats, POSITION_DEAD, 0, 9, 1},
	...
	{"map", do_map, POSITION_DEAD, 0, 9, 1},
	{"affects", do_affects, POSITION_DEAD, 0, 9, 1},
	{"solo", do_solo, POSITION_RESTING, 0, 9, 1},
 ...
 */
	...
	{"playerinfo", do_playerinfo, POSITION_DEAD, 0, 9, 1},
	{"setmobdam", do_setmobdam, POSITION_DEAD, 33, 9, 1},
	{"setmulti", do_setmulti, POSITION_DEAD, 33, 9, 1},
	{"setexp", do_setexp, POSITION_DEAD, 33, 9, 1},
	{"recall", do_recall, POSITION_DEAD, 0, 9, 1},
	{"gang", do_gang, POSITION_RESTING, 0, 9, 1},
...
void command_interpreter(struct char_data *ch, char *pcomm)
{
	...
	int i;
	char c;
	char alias[MAX_STRING_LENGTH], command[MAX_STRING_LENGTH],
	...
	*(tmstr + strlen(tmstr) - 1) = '\0';
	...
	if ((int)pcomm > 45000000) {
		sprintf(log_buf,
			"##PCOMM mem address was %d decimal and 0x%x hex",
			(int)pcomm, (int)pcomm);
		log_hd(log_buf);
		return;
	}
	if ((!pcomm) || (!pcomm[0]))
		return;
#endif
	if ((int)pcomm > high_mem)
		high_mem = (int)pcomm;
	set_to_lower(filename, pcomm);
	if (strstr(filename, "moogooboozoo"))
		return;		/*used for noname mobs */
	c = pcomm[0];
	if (c != 39 && c != ']' && c != ';')
		strcpy(dothis, pcomm);
	else {
	...
	if (!IS_NPC(ch))
		if (ch->p->queryintcommand)
			if ((*ch->p->queryintfunc) (ch, ch->p->queryintcommand,
						    dothispointer))
				return;
	...
	if (!IS_NPC(ch) && stuck_in_water_reaper(ch)) {
		send_to_char
		    ("You are STUCK in a water reaper! Type: help water reaper   when your ride is done.\n\r",
		     ch);
		return;
	}
	...
	for (cmd = 0; cmd < sizeof(cmd_info) / sizeof(cmd_info[0]); cmd++) {
		...
		if (cmd_info[cmd].command_pointer == NULL)
			continue;
		if (memcmp(dothispointer, cmd_info[cmd].command_name, look_at)
		    == 0)
			goto LCmdFound;
	}
 ...
 LCmdFound:
	...
	if (!IS_NPC(ch) && IS_FLYING(ch) && !cmd_info[cmd].fly) {
		send_to_char
		    ("You reconsider and pay attention before you fall off.\n\r",
		     ch);
		return;
	}
	...
	if (!guy_deleted) {
		{
			if (!GET_NAME(ch) || (strlen(GET_NAME(ch)) < 2)) {
				log_hd
				    ("###Another player lost their name! (tail interp).");
				SUICIDE;
			}

			if ((ch->in_room > 0) && (GET_ZONE(ch) == 198)
			    && (cmd_info[cmd].command_number > 8))
	...
	for (j = 0; j <= (MAX_WEAR - 1); j++)
		if (ch->equipment[j] && ch->equipment[j]->item_number >= 0)
			if (obj_index[ch->equipment[j]->item_number].func)
				if ((*obj_index[ch->equipment[j]->item_number].
				     func)
				    (ch, cmd, arg))
					return (1);
	...
	for (i = ch->carrying; i; i = i->next_content)
		if (i->item_number >= 0)
			if (obj_index[i->item_number].func)
				if ((*obj_index[i->item_number].func) (ch, cmd,
								       arg))
					return (1);
...
void do_mudinfo(struct char_data *ch, char *argument, int cmd)
{
	...
	number_mobs = mob1_5 = mob6_10 = mob11_15 = mob16_20 = mob21_25 =
	    mob26_30 = mob31_35 = 0;
	for (i = character_list; i; i = i->next) {
		if (IS_NPC(i)) {
			number_mobs++;
	...
	/* count the commands */
	for (commands = 0; commands < sizeof(cmd_info) / sizeof(cmd_info[0]);
	     commands++) ;
	global_color = 32;
	...
	global_color = 35;
	sprintf(log_buf,
		"1-5[%d],6-10[%d],11-15[%d],16-20[%d],21-25[%d],26-30[%d],31-35[%d]\n\r",
		mob1_5, mob6_10, mob11_15, mob16_20, mob21_25, mob26_30,
		mob31_35);
	send_to_char(log_buf, ch);
	global_color = 33;
	...
	send_to_char(log_buf, ch);
	sprintf(log_buf,
		"Medievia has had [%ld] people connect since Jan 1 1994.\n\r",
		connect_count);
	send_to_char(log_buf, ch);
	global_color = 0;
}
