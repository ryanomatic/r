...
void do_trivia(void);
float mob_damage_multiplier = 1.0f;
float exp_multiplier = 12.0f;
...
void flush_input(struct descriptor_data *d);
...
int port = DFLT_PORT;
int tick_countdown = SECS_PER_MUD_HOUR;
...
void check_gang()
{
	for (struct char_data* i = character_list; i; i = i->next) {
		...
		if (i->nr >= 5821 && i->nr <= 5825) {
			if (!IS_NPC(i->master) && i->in_room != i->master->in_room) {
				die_formation(i->master);
			}
		}
	}
}
...
void autoexec()
{
	// talisman
	for (int i = 0; i < 10; i++) {
		struct obj_data* obj = read_object(10, 0);
		obj_to_room(obj, 1);
	}

	// ancient scroll
	for (int i = 0; i < 200; i++) {
		struct obj_data* obj = read_object(6240, 0);
		obj_to_room(obj, 1);
	}

	// misty
	for (int i = 0; i < 50; i++) {
		struct obj_data* obj = read_object(2015, 0);
		obj_to_room(obj, 1);
	}

	// blue orb
	for (int i = 0; i < 50; i++) {
		struct obj_data* obj = read_object(1411, 0);
		obj_to_room(obj, 1);
	}

	// id scroll
	for (int i = 0; i < 200; i++) {
		struct obj_data* obj = read_object(3040, 0);
		obj_to_room(obj, 1);
	}

	// platinum wand
	for (int i = 0; i < 100; i++) {
		struct obj_data* obj = read_object(5020, 0);
		obj_to_room(obj, 1);
	}

	// oak staff
	for (int i = 0; i < 200; i++) {
		struct obj_data* obj = read_object(4151, 0);
		obj_to_room(obj, 1);
	}

	// grey potion
	for (int i = 0; i < 200; i++) {
		struct obj_data* obj = read_object(5782, 0);
		obj_to_room(obj, 1);
	}
}
	...
	for (pos = 1; pos < argc && argv[pos][0] == '-'; pos++) {
		...
		case 'd':
			if (argv[pos][2] != '\0')
				...
				fprintf(stderr,
					"Directory arg expected after -d.\n\r");
				exit(1);
			}
			break;
	...
	if (port != 1220)
		while (slot_rooms[x] != -1) {
			...
			if ((fh = fopen(log_buf, "r")) != NULL) {
				fread(&world[slot_rooms[x]]->pressure_mod,
				      sizeof(int), 1, fh);
				fclose(fh);
			}
			x++;
...
void prompt(struct descriptor_data *point)
{
	...
	if (ch->desc->connected == CON_SOCIAL_ZONE && ch->p
		...
		for (row = 0; row < 3; row++)
			for (col = 0; col < 3; col++) {
				if (ch->master->formation[row][col] &&
				    ch->master->formation[row][col]->in_room ==
				    ch->in_room) {
					pp = return_perc(GET_HIT
							 (ch->
							  master->formation[row]
							  [col]),
							 GET_MAX_HIT
							 (ch->master->formation
							  [row]
	...
	buf[0] = MED_NULL;
	sprintf(buf, "%s", COL_NRM);

	//sprintf(buf, "T:%d ", tick_countdown);

	if (point->character->specials.solo)
		sprintf(buf + strlen(buf), "%s[S]%s", GRN(ch), NRM(ch));
	...
	if (point->character->p
	    && point->character->p->iNewsVersion < giNewsVersion)
	...
	if (GET_LEVEL(point->character) < 32) {
		...
		if (ch->specials.ansi_color == 69) {
			...
			if (GET_MANA(ch) >
				...
				ansim = COLRED;
			if (GET_MOVE(stpMoveCh) >
			    (GET_MAX_MOVE(stpMoveCh) -
			     (GET_MAX_MOVE(stpMoveCh) / 4)))
				ansiv = COLBLU;
			else if (GET_MOVE(stpMoveCh) >
				 (GET_MAX_MOVE(stpMoveCh) / 2))
				ansiv = COLGRN;
			else if (GET_MOVE(stpMoveCh) >
				 (GET_MAX_MOVE(stpMoveCh) / 4))
				ansiv = COLYEL;
				...
				ansiv = COLRED;
			if (stpMoveCh == ch) {
				if (IN_HOLO(point->character)) {
					sprintf(buf + strlen(buf), "[%d %d %d] ",
						world[point->character->in_room]->number,
						world[point->character->in_room]->holox,
						world[point->character->in_room]->holoy);
				}
				/*
				else {
				...
				*/
				sprintf(buf + strlen(buf),
					"%s<%s%d%shp %s%d%sm %s%d%smv %s%d%sbr>%s ",
					COL_BLU, ansih,
					GET_HIT(point->character), COL_BLU,
					ansim, GET_MANA(point->character),
					COL_BLU, ansiv, GET_MOVE(stpMoveCh),
					COL_BLU, ansib, GET_BREATH(stpMoveCh),
					COL_BLU, COL_NRM);
			}
			else {
				if (IN_HOLO(point->character)) {
				...
				*/
				sprintf(buf + strlen(buf),
					"%s<%s%d%shp %s%d%sm %s%d%sMountMV %s%d%sMountBR>%s ",
					COL_BLU, ansih,
					GET_HIT(point->character), COL_BLU,
					ansim, GET_MANA(point->character),
					COL_BLU, ansiv, GET_MOVE(stpMoveCh),
...
void game_loop(int control)
{
	...
	int next_tick = tv.tv_sec + SECS_PER_MUD_HOUR;
...
#define sigmask(m) (1 << ((m)-1))
	...
	time(&cur_seconds);
	//ttNextQuake = cur_seconds + number(35000, 36000);
	/* Main loop */
	while (!medievia_shutdown) {
		...
		gettimeofday(&tv, NULL);
		tick_countdown = next_tick - tv.tv_sec;
...
*/
		if (select
		    (maxdesc + 1, &input_set, &output_set, &exc_set, &null_time)
		    < 0) {
			perror("Select poll");
			exit(1);
		}
			...
			if (--(point->wait) <= 0
				...
				if (point->character
					...
					char_to_room(point->character,
						     point->character->
						     specials.was_in_room);
					point->character->specials.was_in_room =
					    NOWHERE;
					act("$n returns from reality.", TRUE,
				...
				if (point->character) {
					...
					log_hd
					    ("no character in process output loop");
				}
#endif

				point->wait = 1;
				if ((point->character)
				    && !IS_HOVERING(point->character))
				...
				if (point->str) {
					if (!point->character)
						...
						do_editor(point->character,
							  pcomm,
							  point->
							  character->internal_use2);
...
#ifdef LOGALLINPUT
					sprintf(log_buf,
						"Q-Player:  %s ::  Argument:  %s",
						GET_NAME(point->character),
						pcomm);
					log_hd(log_buf);
#endif
					(*point->character->p->queryfunc)
					    (point->character, pcomm,
					     point->character->p->querycommand);
				} else
					command_interpreter(point->character,
							    pcomm);
				...
				if ((int)pcomm > 49000000) {
					sprintf(log_buf,
						"##PCOMM PREFREE was %d Dec and 0x%x hex",
						(int)pcomm, (int)pcomm);
					log_hd(log_buf);
				} else {
#endif
		...
		for (point = descriptor_list; point; point = next_point) {
			...
			if (FD_ISSET(point->descriptor, &output_set)
			    && point->output.head && !point->ignore_input) {
				...
				if (point->str) {
					if (!point->character)
						...
						if (point->
						    ...
						    1000)
							send_to_char
							    (point->character->p->queryprompt2,
							     point->character);
						else if (point->
							 character->internal_use2
							 == 0)
					 ...
					 || (point->character->
					     p->queryintcommand))
					write_to_q(point->character->
						   p->queryprompt,
						   &point->output);
				else if (point->showstr_point)
					write_to_q("*** Press return ***",
						   &point->output);
				else {
					prompt(point);
				}
		...
		if (pulse % 3 == 0) {
			for (point = descriptor_list; point;
			     point = point->next) {
				if (point->connected != CON_PLAYING
				    && point->connected != CON_UNDEAD)
					continue;
				if (point->character->specials.stpMount
				    && !IS_NPC(point->character))
					stpCh =
					    point->character->specials.stpMount;
				else
					stpCh = point->character;
				if (GET_BREATH(stpCh) < 40) {
					global_color = 32;
					send_to_char
					    ("You pant loud, breathing hard.\n\r",
					     stpCh);
					act("$n pants loud, breathing hard.",
					    TRUE, stpCh, 0, 0, TO_ROOM);
					global_color = 0;
				}
		...
		if (pulse % 16 == 0) {	/*autosave */
			for (i = 0, point = descriptor_list;
			     point && i < autosave; point = point->next, i++) ;
			if (point) {
				autosave++;
				if (point->character)
					if ((point->connected == CON_PLAYING)
...
*/
		...
		if (pulse % 2400 == 0) {
			if (obj_index[1399].number < NUM_DRAGON_CRYSTALS) {
				for (i = obj_index[1399].number;
				     i <= NUM_DRAGON_CRYSTALS; i++) {
					load_dragon_crystal();
				}
			}
		}
		...
		while (rt && rt->time <= cur_seconds) {
			fire_raction(ractions[rt->action]->in_room, 0,
				     rt->action);
...
/*
		...
		if (stpEarthquake
		    && cur_seconds >= stpEarthquake->time_tNextShake)
			Shake();
*/
		...
		if (pulse % 500 == 0) {
			...
			for (point = descriptor_list; point;
			     point = point->next)
				if (point->character
				    && point->character->in_room > 0
				    && world[point->character->in_room]
				    && (world[point->character->in_room]->zone
					== 67
...
/* do pulse_void_people */
		...
		if (pulse % 16 == 0) {
			...
			for (i = character_list; i; i = next_dude) {
				...
				if (i->in_room == -1)
					continue;
				if (IS_NPC(i) && i->specials.stpMount
				    && IS_FLYING(i->specials.stpMount))
					continue;
		...
		if (pulse % ((SECS_PER_MUD_HOUR * 1000000) / OPT_USEC) == 0) {
			gettimeofday(&tv, NULL);
			next_tick = tv.tv_sec + SECS_PER_MUD_HOUR;
			point_update();
			weather_and_time(1);
			affect_update();
			room_affect_update();
		...
		if (fStall) {
			...
			stall_time.tv_usec =
			    last_time.tv_usec - now_time.tv_usec + OPT_USEC;
...
void write_to_q(char *txt, struct txt_q *queue)
{
	...
	if (grep_text[0] && !grep_flag) {	/*GREP CODE */
		...
		while ((p = get_next_line(p, line)) != NULL) {
			...
			if (!strstr(grep, grep_text) && !strstr(grep, "return")
			    && !strstr(grep, "enter"))
				continue;
			else
...
int init_socket(int port)
{
...
#if defined(SO_DONTLINGER)
	{
		...
		if (setsockopt(s, SOL_SOCKET, SO_DONTLINGER, &ld, sizeof(ld)) <
		    0) {
	...
	*newd->last_input = '\0';
	...
	for (tmp = ban_list; tmp; tmp = tmp->next) {
		if (!str_cmp(tmp->name, newd->host)) {
			sprintf(log_buf,
				...
				tmp->god, tmp->date, tmp->reason);
			write_to_descriptor(desc, log_buf);
			sprintf(log_buf, "## %s (banned) tried to connect",
				newd->host);
			log_hd(log_buf);
			close(desc);
			open_descs--;
	...
	write_to_q
	    ("\n\rWelcome to MEDIEVIA!  Leading the way in M.U.D. development.\n\r",
	     &newd->output);
	write_to_q
	    ("Be sure to type HELP MEDIEVIA to see why there is no turning back.\n\r\n\r",
	     &newd->output);
	write_to_q(" By what name are you known on Medievia? ", &newd->output);
...
void close_socket(struct descriptor_data *d)
{
	...
	if (d->character) {
		if ((d->connected == CON_PLAYING)
			...
			act("$n has lost $s link.", TRUE, d->character, 0, 0,
			    TO_ROOM);
			sprintf(log_buf, "LOSTLINK:Closing link to: %s.",
			...
			sprintf(log_buf, "Losing player: %s.",
				GET_NAME(d->character));
			log_hd(log_buf);
...
void sort_descriptor(struct descriptor_data *d)
{
	...
	if (!sort_d) {
		...
		d->next = sort_d;
		sort_d = d;
	} else if (get_total_level(d->character) >=
		   get_total_level(sort_d->character)) {
		d->next = sort_d;
		sort_d = d;
	} else {
		for (tmp = sort_d; tmp; tmp = tmp->next) {
			...
			if (get_total_level(d->character) >=
			    get_total_level(tmp->next->character)) {
				d->next = tmp->next;
				tmp->next = d;
				return;
			}
...
void send_to_char(char *messg, struct char_data *ch)
{
	...
	if (ch->desc && messg) {
		...
		if (global_color && ch->specials.ansi_color == 69) {
			sprintf(buf, "\033[1m\033[%dm%s\033[0m", global_color,
				messg);
...
void send_to_all(char *messg)
{
	...
	if (messg)
		for (i = descriptor_list; i; i = i->next)
			if (!i->connected)
				if (global_color
				    && i->character->specials.ansi_color ==
				    69) {
					sprintf(buf, "\033[1m\033[%dm%s\033[0m",
						global_color, messg);
...
void send_to_outdoor(char *messg)
{
	...
	if (messg)
		for (i = descriptor_list; i; i = i->next)
			if ((!i->connected)
			    && !IS_SET((i->character)->specials.plr_flags,
				       PLR_WRITING))
				if (OUTSIDE(i->character))
					if (global_color
					    && i->character->
					    specials.ansi_color == 69) {
						sprintf(buf,
			...
			*(++point) = '\0';
			...
			if (ACTTO) {
				if (ACTTO == to)
					return;
				if (global_color
				    && ACTTO->specials.ansi_color == 69) {
					sprintf(bufc,
						"\033[1m\033[%dm%s\033[0m",
						global_color, CAP(buf));
			...
			if (global_color && to->specials.ansi_color == 69) {
				sprintf(bufc, "\033[1m\033[%dm%s\033[0m",
					global_color, CAP(buf));
...
void night_watchman(void)
{
	...
	if ((t_info->tm_hour == 8) && (t_info->tm_wday > 0) &&
		...
		if (t_info->tm_min > 50) {
			...
			send_to_all
			    ("Closing down. Thank you for flying Medievia.\n\r");
			medievia_shutdown = 1;
			...
			send_to_all
			    ("Warning: The game will close in 20 minutes.\n\r");
	}
}
