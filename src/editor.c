...
void show_menu(struct char_data *ch)
{
	strcpy(ch->p->queryprompt2,
	       "Medievia Editor: E,S,J,L,I,W,R,T,P,C,D,A,H or M=Menu> ");
...
void do_editor(struct char_data *ch, char *argument, int cmd)
{
	...
	char buf[MAX_INPUT_LENGTH], word[MAX_INPUT_LENGTH], *rest, *buf2,
	    buffer[MAX_INPUT_LENGTH * 2];
	char pre[MAX_INPUT_LENGTH], suf[MAX_INPUT_LENGTH], *wordp,
	    wordb[MAX_INPUT_LENGTH];
	...
	if (cmd == 9 || cmd == 0) {
		...
		if (!strcmp(*ch->desc->str, "BlAnK")) {
			send_to_char
			    ("You start editing a fresh piece of paper...\n\r",
			     ch);
			strcpy(*ch->desc->str, "");
		}
		...
		if (strlen(ch->desc->editing)) {
			sprintf(log_buf, "$n starts editing [%s].",
				ch->desc->editing);
	...
	if (cmd == 1000) {
		...
		case 's':
			...
			if (!strlen(ch->desc->data[0])
			    && !strlen(ch->desc->data[1])) {
				strcpy(*ch->desc->str, "");
			} else {
				x += 5;
				if (!
				    (*ch->desc->str =
				     (char *)realloc(*ch->desc->str, x))) {
					perror("SAVING IN EDITOR1");
					exit(1);
				}
				x = 0;
				if (ch->desc->oneline == 2) {
					ch->specials.tmpstring =
					    my_free(ch->specials.tmpstring);
					sprintf(notefile, "../notes/%s",
						GET_NAME(ch));
					...
					open_files--;
					send_to_char
					    ("Message is saved in your note file.\n\r",
					     ch);
			...
			if (ch->desc->max_str == 2123)
				board(ch, 1000, "");
			if (ch->desc->editing)
				sprintf(log_buf, "$n finishes editing a %s.",
					ch->desc->editing);
			else
		...
		case 'j':
			...
			while (rest && rest[0]) {
				...
				prep++;
				if ((strlen(word) + strlen(ch->desc->data[y]) +
				     1) < 80) {
		...
		case 'r':
			strcpy(ch->p->queryprompt2,
			       "\n\rType in the word to search and replace,\rExample: willow Willow Tree.  willow will be replaced with Willow Tree.\rSearch is sensitive, use exact capitalization> ");
	...
	if (cmd == 1003 || cmd == 1004) {
		...
		if (cmd == 1003)
			sprintf(ch->p->queryprompt2,
				"-=<(OK enter in the text for the line)>=-\n\r\n\rOLD TEXT:\n\r> %s\n\r> ",
				ch->desc->data[x]);
		else
			sprintf(ch->p->queryprompt2,
				"-=<(OK enter in the text for the line)>=-\n\r\n\r> ");
