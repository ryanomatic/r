	...
	for (x = 0; x < MAX_CLANS; x++) {
		...
		if ((fp = med_open(filename, "rb")) != NULL) {
			...
			if (fread(&clan_info, sizeof(clan_info), 1, fp) != 0) {
				...
				strcpy((char *)&global_clan_info.
				       clan_name[x + 1], clan_info.clan_name);
				strcpy((char *)&global_clan_info.
				       clan_leader[x + 1],
...
void do_enrollclan(struct char_data *ch, char *argument, int cmd)
{
	...
	sprintf(log_buf, "[%s] has enrolled in the %s clan!", GET_NAME(victim),
		clan_info.clan_name);
	do_echo(ch, log_buf, 0);
	sprintf(log_buf, "[CLAN] %s has just become a member!!!",
		GET_NAME(victim));
...
bool update_clan_eggs(int clan, int eggs)
{
	...
	if ((fp = med_open(log_buf, "rb")) == NULL) {
		sprintf(log_buf, "## could not open  clan %d info in load_clan",
			clan);
		log_hd(log_buf);
		return (FALSE);
	}
	...
	if (fread(&clan_info, sizeof(clan_info), 1, fp) == 0) {
		med_close(fp);
		open_files--;
		sprintf(log_buf, "## could not read clan %d info in load_clan",
			clan);
		log_hd(log_buf);
	...
	if ((fp = med_open(log_buf, "wb")) == NULL) {
		sprintf(log_buf, "## could not save clan %d in save_clan",
			clan);
		log_hd(log_buf);
		return (FALSE);
	}
	...
	if (fwrite(&clan_info, sizeof(clan_info), 1, fp) == 0) {
		med_close(fp);
		open_files--;
		sprintf(log_buf, "## could not save clan %d info in save_clan",
			clan);
		log_hd(log_buf);
		return (FALSE);
	}
...
void do_removeclan(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!victim) {
		send_to_char("That person is not here!\n\r", ch);
		send_to_char
		    ("Will try and remove anyway, REMEMBER Use proper CASE with name.\n\r",
		     ch);
	} else {
		if (IS_NPC(victim)) {
			send_to_char("That is not a player!\n\r", ch);
	...
	open_files--;
	if (victim)
		victim->specials.clan = 0;
	sprintf(log_buf, "[%s] has been removed from the %s clan!", name,
		clan_info.clan_name);
	do_echo(ch, log_buf, 0);
	sprintf(log_buf, "[CLAN] %s has just been kicked out of the Clan!!!",
		name);
	tell_clan(ch->specials.clan, log_buf);
}
...
void do_showclans(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!argument[0]) {
		sprintf(clan_buf, "%s\n\r[   EGGS   ][##][%15s] %s\n\r",
			WHT(ch), "Clan Leader", "____Clan Name____");
		for (x = 0; x < MAX_CLANS; x++) {
		...
		sprintf(clan_buf, "%s%sClan Leader: %s\n\r\n\r", clan_buf,
			RED(ch), clan_info.clan_leader);

		for (x = 0; x < MAX_CLAN_MEMBERS; x++) {
			if (clan_info.members[x][0]) {
				sprintf(clan_buf, "%s%sClan Member: %s\n\r",
					clan_buf,
					GRN(ch), &clan_info.members[x][0]);
			} else
				break;
		}
		page_string(ch->desc, clan_buf, 1);
...
void do_lead_clan(struct char_data *ch, char *argument, int cmd)
{
	...
	if (clan < 0 || clan > MAX_CLANS) {
		send_to_char
		    ("Not a valid clan number must be 1 to MAX_CLANS.\n\r", ch);
		return;
	}
	...
	if (!clan)
		...
		log_hd(log_buf);
		send_to_char
		    ("Done, you are now also a leader of that clan!\n\r", ch);
	}
}
