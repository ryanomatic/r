...
void do_camp(struct char_data *ch, char *argument, int cmd)
{
	...
	for (stpR = world[ch->in_room]->room_afs; stpR; stpR = stpR->next) {
		if (stpR->type == RA_CAMP) {
			...
			global_color = 31;
			send_to_char
			    ("You throw more wood on the fire and get the campsite ready.\n\r",
			     ch);
			act("$n throws more wood on the fire.", TRUE, ch, 0, 0,
			    TO_ROOM);
			global_color = 0;
			GET_MOVE(ch) /= 2;
			return;
	...
	if (!IN_HOLO(ch)) {
		send_to_char
		    ("You realize you should only camp in the wilderness.\n\r",
		     ch);
		return;
	}
	...
	survey_list = stpS;
	stpS->description =
	    str_dup("you see pillar of smoke rising from a campfire.");
...
void DrawHoloMap(struct char_data *ch, int width, int length)
{
	...
	*buf = '\0';
	...
	if (!IS_NPC(ch) && IS_FLYING(ch)) {
		...
		sprintf(buf, "%s%s%s\n\r", YEL(ch), world[ch->in_room]->name,
			NRM(ch));
	}
	...
	for (iVertLoop = iBottom; iVertLoop <= iTop; iVertLoop++) {
		for (iHorzLoop = iLeft; iHorzLoop <= iRight; iHorzLoop++) {
			...
			if (Holo[iHorzLoop][iVertLoop] > 255) {
				iTemplate =
				    world[Holo[iHorzLoop][iVertLoop]]->holoroom;
				/* The You Are Here # Symbol */
				if (iHorzLoop == iChHere[0]
				    && iVertLoop == iChHere[1]) {
					cTemp = '#';
					if (bColor)
					...
					cTemp =
					    stpHoloRTemplates[iTemplate]->
					    cMapChar;
					/* if color, check if you need to change, if so, change colors */
					if (bColor)
						if (stpHoloRTemplates
						    [iTemplate]->siMapColor)
							if (stpHoloRTemplates
							    [iTemplate]->
							    siMapColor !=
							    iLastColor) {
				...
				if (bColor)
					if (stpHoloRTemplates[iTemplate]->
					    siMapColor)
						if (stpHoloRTemplates
						    [iTemplate]->siMapColor !=
						    iLastColor) {
...
int iHoloChDistance(struct char_data *ch, struct char_data *vict)
{
	...
	iDist = (int)dHoloDistance(HOLOX(ch),
				   HOLOY(ch), HOLOX(vict), HOLOY(vict));
...
void do_survey(struct char_data *ch, char *argument, int cmd)
{
	...
	*buf = '\0';
	...
	for (stpServ = survey_list; stpServ; stpServ = stpServ->next) {
		iDistance = (int)dHoloDistance((int)HOLOX(ch),
					       (int)HOLOY(ch),
					       (int)stpServ->iX,
					       (int)stpServ->iY);
		...
		if (iDistance <= stpServ->dist) {
			found = 1;
			iAngle = iHoloWhere((int)HOLOX(ch),
					    (int)HOLOY(ch),
					    (int)stpServ->iX,
					    (int)stpServ->iY, &iDistance);
			if (iAngle == -1)
				cDir = 13;
			else if (iAngle >= 360)
				cDir = 12;
			...
			if (cDir == 13)
				sprintf(buf, "In the immediate area, %s.\r\n",
					stpServ->description ? stpServ->
					description : "<NULL>");
			else
				sprintf(buf, "At %d o'clock, %s %s.\r\n", cDir,
					szaSurveyDists[(int)iMes],
					stpServ->description ? stpServ->
					description : "<NULL PLEASE REPORT>");
			send_to_char(buf, ch);
			/* Some God Stuff */
	...
	if (!found)
		send_to_char
		    ("Your survey of the area yields nothing special.\r\n", ch);
}
...
/******************************************************************/
		...
		if (world[Holo[x][y]]
		    && (world[Holo[x][y]]->holox != x
			|| world[Holo[x][y]]->holoy != y)) {
...
/******************************************************************/
	...
	if (!stpHoloRTemplates[Holo[x][y]]) {
		sprintf(log_buf, "## need to make HoloRoom %d at %d by %d",
			Holo[x][y], x, y);
		log_hd(log_buf);
		return (0);
	}
	...
	if (!iNextRoom) {	/*reuse mode */
		...
		if (stpLastHoloRoom && stpLastHoloRoom->holonext)
			stpLastHoloRoom->holonext = NULL;
		Holo[world[iRoom]->holox][world[iRoom]->holoy] =
		    world[iRoom]->holoroom;
		if (world[iRoom]->dir_option[NORTH])
			world[iRoom]->dir_option[NORTH] =
			    my_free(world[iRoom]->dir_option[NORTH]);
		if (world[iRoom]->dir_option[SOUTH])
			world[iRoom]->dir_option[SOUTH] =
			    my_free(world[iRoom]->dir_option[SOUTH]);
		if (world[iRoom]->dir_option[EAST])
			world[iRoom]->dir_option[EAST] =
			    my_free(world[iRoom]->dir_option[EAST]);
		if (world[iRoom]->dir_option[WEST])
			world[iRoom]->dir_option[WEST] =
			    my_free(world[iRoom]->dir_option[WEST]);
		if (world[iRoom]->dir_option[UP])
			world[iRoom]->dir_option[UP] =
			    my_free(world[iRoom]->dir_option[UP]);
		if (world[iRoom]->dir_option[DOWN])
			world[iRoom]->dir_option[DOWN] =
			    my_free(world[iRoom]->dir_option[DOWN]);
		...
		for (vict = world[iRoom]->people; vict; vict = next_v) {
			...
			if (IS_NPC(vict) && !IS_DEAD(vict)) {
				...
				sprintf(log_buf,
					"##HCode re-using a room with a player(%s)",
					GET_NAME(vict));
				log_hd(log_buf);
			}
		}
	...
	while ((world[iRoom]->description =
		stpHoloRTemplates[usiMap]->description[number(0, 9)]) == NULL) ;
	...
	world[iRoom]->class_restriction =
	    stpHoloRTemplates[usiMap]->class_restriction;
	world[iRoom]->level_restriction =
	    stpHoloRTemplates[usiMap]->level_restriction;
	world[iRoom]->mount_restriction =
	    stpHoloRTemplates[usiMap]->mount_restriction;
	world[iRoom]->move_mod = stpHoloRTemplates[usiMap]->move_mod;
	world[iRoom]->pressure_mod = stpHoloRTemplates[usiMap]->pressure_mod;
	world[iRoom]->temperature_mod =
	    stpHoloRTemplates[usiMap]->temperature_mod;
	...
	if (Holo[x - 1][y] > 255 || Holo[x - 1][y] == 215) {
		if (Holo[x - 1][y] == 215) {
			CREATE(world[iRoom]->dir_option[WEST],
			       struct room_direction_data, 1);
			world[iRoom]->dir_option[WEST]->to_room = igLINKW;
			CREATE(world[igLINKW]->dir_option[EAST],
			       struct room_direction_data, 1);
			world[igLINKW]->dir_option[EAST]->to_room = iRoom;
		} else {
			CREATE(world[iRoom]->dir_option[WEST],
			       struct room_direction_data, 1);
			world[iRoom]->dir_option[WEST]->to_room =
			    Holo[x - 1][y];
			CREATE(world[Holo[x - 1][y]]->dir_option[EAST],
			       struct room_direction_data, 1);
			world[Holo[x - 1][y]]->dir_option[EAST]->to_room =
			    iRoom;
		}
	}
	if (Holo[x + 1][y] > 255 || Holo[x + 1][y] == 215) {
		if (Holo[x + 1][y] == 215) {
			CREATE(world[iRoom]->dir_option[EAST],
			       struct room_direction_data, 1);
			world[iRoom]->dir_option[EAST]->to_room = igLINKE;
			CREATE(world[igLINKE]->dir_option[WEST],
			       struct room_direction_data, 1);
			world[igLINKE]->dir_option[WEST]->to_room = iRoom;
		} else {
			CREATE(world[iRoom]->dir_option[EAST],
			       struct room_direction_data, 1);
			world[iRoom]->dir_option[EAST]->to_room =
			    Holo[x + 1][y];
			CREATE(world[Holo[x + 1][y]]->dir_option[WEST],
			       struct room_direction_data, 1);
			world[Holo[x + 1][y]]->dir_option[WEST]->to_room =
			    iRoom;
		}
	}
	if (Holo[x][y - 1] > 255 || Holo[x][y - 1] == 215) {
		if (Holo[x][y - 1] == 215) {
			CREATE(world[iRoom]->dir_option[NORTH],
			       struct room_direction_data, 1);
			world[iRoom]->dir_option[NORTH]->to_room = igLINKN;
			CREATE(world[igLINKN]->dir_option[SOUTH],
			       struct room_direction_data, 1);
			world[igLINKN]->dir_option[SOUTH]->to_room = iRoom;
		} else {
			CREATE(world[iRoom]->dir_option[NORTH],
			       struct room_direction_data, 1);
			world[iRoom]->dir_option[NORTH]->to_room =
			    Holo[x][y - 1];
			CREATE(world[Holo[x][y - 1]]->dir_option[SOUTH],
			       struct room_direction_data, 1);
			world[Holo[x][y - 1]]->dir_option[SOUTH]->to_room =
			    iRoom;
		}
	}
	if (Holo[x][y + 1] > 255 || Holo[x][y + 1] == 215) {
		if (Holo[x][y + 1] == 215) {
			CREATE(world[iRoom]->dir_option[SOUTH],
			       struct room_direction_data, 1);
			world[iRoom]->dir_option[SOUTH]->to_room = igLINKS;
			CREATE(world[igLINKS]->dir_option[NORTH],
			       struct room_direction_data, 1);
			world[igLINKS]->dir_option[NORTH]->to_room = iRoom;
		} else {
			CREATE(world[iRoom]->dir_option[SOUTH],
			       struct room_direction_data, 1);
			world[iRoom]->dir_option[SOUTH]->to_room =
			    Holo[x][y + 1];
			CREATE(world[Holo[x][y + 1]]->dir_option[NORTH],
			       struct room_direction_data, 1);
			world[Holo[x][y + 1]]->dir_option[NORTH]->to_room =
			    iRoom;
		}
	}
...
void LoadHoloTemplateRooms(void)
{
	...
	fprintf(stderr,
		"HOLOCODE:  Loading and organizing medievia holotemplates.\n");
	fscanf(fl, " %s ", szTag);
	while (1) {
		if (szTag[0] == '$')
			break;
		...
		while (1) {
			...
			if (!strncasecmp(szTag, "NAM", 3)) {
				...
				stpHoloRTemplates[iR]->description[iDescs++] =
				    fread_string(fl);
			} else if (!strncasecmp(szTag, "!CL", 3)) {
				stpHoloRTemplates[iR]->class_restriction &=
				    NO_CLERIC;
				...
				stpHoloRTemplates[iR]->extra_flags |=
				    TRADE_NO_WAGON;
			} else if (!strncasecmp(szTag, "NOH", 3)) {
				stpHoloRTemplates[iR]->extra_flags |=
				    TRADE_NO_HORSE;
			} else if (!strncasecmp(szTag, "ARC", 3)) {
				stpHoloRTemplates[iR]->sector_type =
				    SECT_ARCTIC;
...
void HoloMobilePlacement(int iX, int iY, int iRoom)
{
	...
	iNumMobsPerSector = 0;
	for (x = 0; giaMobPlacement[world[iRoom]->sector_type][x] && x < 50;
	     x++) {
		if (GET_LEVEL
		    (mobs[giaMobPlacement[world[iRoom]->sector_type][x]]) >=
		    iMinLevel
		    &&
		    GET_LEVEL(mobs
			      [giaMobPlacement[world[iRoom]->sector_type][x]])
		    <= iMaxLevel) {
			iaMobs[iNumMobsPerSector++] =
			    giaMobPlacement[world[iRoom]->sector_type][x];
		}
	}
...
void HoloAct(int x, int y, int hx, int hy, int xxx, int yyy, char *szpText,
	     bool bInvis, struct char_data *stpCh, struct obj_data *stpObject,
	     struct char_data *stpVict, int iColor, char *szpSense)
{
	...
	for (stpS = descriptor_list; stpS; stpS = stpS->next) {
		global_color = iColor;
		if (stpS->character) {
			if (world[stpS->character->in_room] == NULL)
				return;

			if (world[stpS->character->in_room]->sector_type == NULL)
				return;

			if (world[stpS->character->in_room]->sector_type ==
			    SECT_INSIDE)
				continue;
			...
			if (cDir == -1)
				sprintf(szBuf,
					"In the immediate area, you %s %s.\r\n",
					szpSense, szpText);
			else
				sprintf(szBuf,
					"At %d o'clock, %s you %s %s.\r\n",
					cDir, szaSurveyDists[(int)iMes],
					szpSense, szpText);
